...
describe("API Endpoint: POST /api/reports/testCustomQuery", () => {
  ...
  beforeEach(() => {
    ...
    mockUnifiedAuthMiddleware.mockImplementation(
    ...
    // Setup route
    app.post(
      "/api/reports/testCustomQuery",
      mockUnifiedAuthMiddleware,
  ...
  describe("Successful Response Scenarios", () => {
    ...
    it("should return standardized PreviewResponse for Azure query", async () => {
      ...
      expect(response.body.data.testData[0].userPrincipalName).toBe(
        "john.doe@company.com",
      );
      expect(response.body.data.metadata.originalFormat).toBe(
        "graph_api_value",
      );
    });
  ...
  describe("Error Handling", () => {
    it("should return 401 when user is not authenticated", async () => {
      ...
      unauthApp.use((err: any, _req: any, res: any, _next: any) => {
        res.status(err.statusCode || 500).json({
          success: false,
          error: err.message || "Internal Server Error",
        });
    ...
    it("should handle malformed request bodies", async () => {
      mockPreviewService.executePreview.mockRejectedValue(
    ...
    it("should handle complex queries with multiple filters", async () => {
      ...
      const response: PreviewResponse = {
        ...
        data: {
          ...
          executionTime: 250,
          testData: [
            { sAMAccountName: "test", department: "IT", lastLogon: new Date() },
          ],
          rowCount: 1,
          isTestRun: true,
        },
  ...
  describe("Response Format Validation", () => {
    ...
    it("should preserve all metadata in response", async () => {
      ...
      expect(result.body.data.metadata.debugInfo.queryComplexity).toBe(
        "medium",
      );
    });
  });
