...
import {
  getAllGraphQueries,
  getGraphQuery,
  getGraphQueriesByCategory,
} from "@/queries/graph";
import { db } from "@/config/database";
import { logger } from "@/utils/logger";
import { asyncHandler, createError } from "@/middleware/error.middleware";
import { body } from "express-validator";
import { isAllowedQueryTable } from "@/config/query-allowlist";
...
export class QueryController {
   ...
   */
  buildAndExecuteQuery = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const {
        dataSource,
        select,
      ...
      if (!req.user.isAdmin) {
        throw createError(
          "Admin access required for dynamic query building",
          403,
        );
      }

      if (dataSource === "postgres" && from && !isAllowedQueryTable(from)) {
        throw createError(`Access to table '${from}' is not permitted`, 403);
      }
   ...
   */
  getQueryDefinitions = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
            ...
            accessibleDefinitions.push({
              ...
              cache: def.cache
                ? {
                    ...
                    ttlSeconds: def.cache.ttlSeconds,
                  }
                : null,
            });
          }
  ...
  getSchema = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (dataSource === "postgres" && !req.user.isAdmin) {
        throw createError(
          "Admin access required for PostgreSQL schema discovery",
          403,
        );
      }
   ...
   */
  validateQuery = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const { queryDef, parameters = {} } = req.body;

      try {
        const validation = await this.queryService.validateQuery(
          queryDef,
          parameters,
        );
  ...
  clearCache = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (!req.user.isAdmin) {
        throw createError(
          "Admin permissions required for cache management",
          403,
        );
      }
      ...
      try {
        ...
        logger.info(`Cache cleared by ${req.user.username}`, {
          queryId: queryId || "all",
        });
        ...
        res.json({
          ...
          data: {
            message: queryId
              ? `Cache cleared for query ${queryId}`
              : "All query caches cleared",
   ...
   */
  executeGraphQuery = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const { queryId, parameters = {}, options = {} } = req.body;
      ...
      try {
        ...
        if (!queryDef) {
          throw createError(
            `Graph query definition not found: ${queryId}`,
            404,
          );
        }
   ...
   */
  getGraphQueryDefinitions = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        const definitions = queries.map((q) => ({
          ...
          parameters: q.parameters
            ? Object.entries(q.parameters).map(([name, def]) => ({
                name,
                type: def.type,
   ...
   */
  getGraphQueryHistory = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
   ...
   */
  executeGraphBatch = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        const results = await this.graphQueryExecutor.executeBatch(queries, {
          userId: req.user.id,
          options: {
            includeCount: options.includeCount ?? true,
            pageSize: options.pageSize || 100,
  ...
  private async checkQueryAccess(user: any, queryDef: any): Promise<boolean> {
    ...
    if (queryDef.access.roles && queryDef.access.roles.length > 0) {
      ...
      const hasRole = queryDef.access.roles.some((role: string) =>
    ...
    if (queryDef.access.permissions && queryDef.access.permissions.length > 0) {
      ...
      const hasPermission = queryDef.access.permissions.some((perm: string) =>
    ...
    return {
      user: [
        ...
        { name: "mail", type: "string", description: "Email Address" },
        {
          name: "userType",
          type: "string",
          description: "User Type (Member/Guest)",
        },
        {
          name: "accountEnabled",
          type: "boolean",
        ...
        {
          name: "signInActivity.lastSignInDateTime",
          type: "date",
...
export const buildQueryValidation = [
  ...
  body("select").isArray({ min: 1 }).withMessage("Select fields are required"),
  body("from")
    .isString()
    .notEmpty()
