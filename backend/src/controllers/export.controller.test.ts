...
jest.mock("@/queues/report.queue", () => ({
  addReportToQueue: jest.fn().mockResolvedValue({ id: "job-123" }),
  reportQueue: {
...
describe("ExportController", () => {
  ...
  describe("exportReport", () => {
    ...
    it("should export custom report successfully", async () => {
      ...
      (db.query as jest.Mock)
        .mockResolvedValueOnce({ rows: [mockCustomTemplate] })
      ...
      expect(db.query).toHaveBeenCalledWith(
        "SELECT created_by, is_public FROM custom_report_templates WHERE id = $1",
        ["custom-123"],
      );
    ...
    it("should handle custom template not found", async () => {
      ...
      (db.query as jest.Mock).mockResolvedValue({ rows: [] });
      (createError as jest.Mock).mockReturnValue(
        new Error("Custom report template not found"),
      );
      ...
      expect(createError).toHaveBeenCalledWith(
        "Custom report template not found",
        404,
      );
      expect(mockNext).toHaveBeenCalledWith(expect.any(Error));
    });
    ...
    it("should handle access denied to private custom template", async () => {
      ...
      (db.query as jest.Mock).mockResolvedValue({
        rows: [mockPrivateTemplate],
      });
      (createError as jest.Mock).mockReturnValue(
        new Error("Access denied to this report"),
      );
    ...
    it("should handle export service errors", async () => {
      ...
      (exportService.exportData as jest.Mock).mockRejectedValue(
  ...
  describe("queueExport", () => {
    ...
    it("should queue export successfully", async () => {
      ...
      await exportController.queueExport(
        mockReq as Request,
        mockRes as Response,
        mockNext,
      );
    ...
    it("should queue custom template export", async () => {
      ...
      (db.query as jest.Mock).mockResolvedValue({
        rows: [{ created_by: 1, is_public: true }],
      });
    ...
    it("should handle queue errors", async () => {
      ...
      (addReportToQueue as jest.Mock).mockRejectedValue(
  ...
  describe("downloadFile", () => {
    ...
    it("should download Excel file successfully", async () => {
      ...
      await exportController.downloadFile(
        mockReq as Request,
        mockRes as Response,
        mockNext,
      );
    ...
    it("should handle invalid filename with illegal characters", async () => {
      ...
      (createError as jest.Mock).mockReturnValue(
        new Error("Invalid filename - contains illegal characters"),
      );
    ...
    it("should handle path traversal with clean filename", async () => {
      ...
      (createError as jest.Mock).mockReturnValue(
        new Error("Invalid filename - path traversal attempt"),
      );
    ...
    it("should handle file not found on disk", async () => {
      ...
      (createError as jest.Mock).mockReturnValue(
        new Error("File not found on server"),
      );
    ...
    it("should use correct export path based on environment", async () => {
      ...
      expect(fs.open).toHaveBeenCalledWith(
        "/custom/exports/test-report.xlsx",
        "r",
      );
    });
  });
  ...
  describe("exportHistoryResults", () => {
    ...
    beforeEach(() => {
      (db.query as jest.Mock).mockResolvedValue({ rows: [mockHistoryRecord] });
      (exportService.exportDataWithFormatting as jest.Mock).mockResolvedValue(
        mockExportResult,
      );
    });
    ...
    it("should export history results in Excel format", async () => {
      ...
      await exportController.exportHistoryResults(
        mockReq as Request,
        mockRes as Response,
        mockNext,
      );
    ...
    it("should handle history record not found", async () => {
      ...
      (db.query as jest.Mock).mockResolvedValue({ rows: [] });
      (createError as jest.Mock).mockReturnValue(
        new Error("History record not found or access denied"),
      );
      ...
      expect(createError).toHaveBeenCalledWith(
        "History record not found or access denied",
        404,
      );
      expect(mockNext).toHaveBeenCalledWith(expect.any(Error));
    });
    ...
    it("should handle no results found", async () => {
      ...
      (createError as jest.Mock).mockReturnValue(
        new Error("No results found for this history record"),
      );
      ...
      expect(createError).toHaveBeenCalledWith(
        "No results found for this history record",
        404,
      );
      expect(mockNext).toHaveBeenCalledWith(expect.any(Error));
    });
  ...
  describe("getJobStatus", () => {
    it("should return job status", async () => {
      ...
      await exportController.getJobStatus(
        mockReq as Request,
        mockRes as Response,
        mockNext,
      );
    ...
    it("should handle errors", async () => {
      ...
      jest
        .spyOn(exportController, "getJobStatus")
  ...
  describe("cleanupExports", () => {
    ...
    it("should cleanup old exports successfully", async () => {
      ...
      await exportController.cleanupExports(
        mockReq as Request,
        mockRes as Response,
        mockNext,
      );
  ...
  describe("Error Handling", () => {
    it("should handle unexpected errors in exportReport", async () => {
      ...
      (reportExecutor.executeReport as jest.Mock).mockRejectedValue(
