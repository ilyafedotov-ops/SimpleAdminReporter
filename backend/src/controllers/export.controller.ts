...
export class ExportController {
  ...
  async exportReport(req: Request, res: Response, _next: NextFunction) {
    try {
      ...
      if (customTemplateId) {
        await this.verifyCustomTemplateAccess(
          customTemplateId,
          req.user!.id,
          req.user!.isAdmin,
        );
  ...
  /**
   * Verify the current user can access a custom report template.
   */
  private async verifyCustomTemplateAccess(
    ...
    if (!template.is_public && template.created_by !== userId && !isAdmin) {
  ...
  async downloadFile(req: Request, res: Response, _next: NextFunction) {
    try {
      ...
      if (
        !requestedFilePath.startsWith(resolvedExportPath + path.sep) &&
  ...
  async exportHistoryResults(req: Request, res: Response, _next: NextFunction) {
    try {
      ...
      res.setHeader(
        "Content-Type",
        contentTypes[format as string] || "application/octet-stream",
      );
      res.setHeader(
        "Content-Disposition",
  ...
  async getJobStatus(req: Request, res: Response, _next: NextFunction) {
    try {
      ...
      const { reportQueue } = await import("@/queues/report.queue");
      ...
      if (job.data.userId !== req.user!.id && !req.user!.isAdmin) {
        throw createError("Access denied to this export job", 403);
      }
