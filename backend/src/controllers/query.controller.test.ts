...
describe("QueryController", () => {
  ...
  beforeEach(() => {
    ...
    (
      QueryDefinitionRegistry as jest.MockedClass<
        typeof QueryDefinitionRegistry
      >
    ).mockImplementation(() => mockQueryRegistry);
      ...
      () =>
        ({
          buildQuery: jest.fn().mockReturnValue({
            ...
            sql: "SELECT name, category FROM report_templates WHERE is_active = $1 LIMIT $2",
            parameters: [
  ...
  describe("POST /api/query/build", () => {
    it("should build and execute dynamic query successfully", async () => {
      ...
      const mockResult = {
        success: true,
        data: [{ name: "test-template", category: "ad" }],
        metadata: {
          executionTime: 100,
          rowCount: 1,
          queryId: "temp_123456",
  ...
  describe("GET /api/query/definitions", () => {
    it("should return available query definitions", async () => {
      ...
      const response = await request(app).get("/api/query/definitions");
    ...
    it("should filter definitions by data source", async () => {
      ...
      const response = await request(app).get(
        "/api/query/definitions?dataSource=postgres",
      );
    ...
    it("should search definitions by query parameter", async () => {
      ...
      const response = await request(app).get(
        "/api/query/definitions?search=user",
      );
  ...
  describe("GET /api/query/stats", () => {
    it("should return statistics for specific query", async () => {
      ...
      const response = await request(app).get("/api/query/stats/test_query");
    ...
    it("should accept date range parameters", async () => {
      ...
      const response = await request(app).get(
        "/api/query/stats/test_query?startDate=2024-01-01T00:00:00.000Z&endDate=2024-01-31T23:59:59.999Z",
      );
    ...
    it("should return general statistics when no queryId provided", async () => {
      ...
      const response = await request(app).get("/api/query/stats");
  ...
  describe("GET /api/query/health", () => {
    it("should return healthy status when all services are up", async () => {
      ...
      const response = await request(app).get("/api/query/health");
    ...
    it("should handle health check errors", async () => {
      mockQueryService.testConnection.mockRejectedValue(
    ...
    it("should return system metrics successfully", async () => {
      ...
      mockDb.query = jest
        ...
        .mockResolvedValueOnce({
          // query breakdown query (first in controller)
          rows: [
            {
              query_id: "popular_query",
              total_executions: 50,
        ...
        .mockResolvedValueOnce({
          // performance summary query (second in controller)
          rows: [
            {
              unique_queries: 5,
              total_executions: 100,
      ...
      const response = await request(app).get("/api/query/metrics");
  ...
  describe("Input Validation", () => {
    ...
    it("should validate build query required fields", async () => {
      ...
      (
        QueryBuilder as jest.MockedClass<typeof QueryBuilder>
      ).mockImplementationOnce(() => mockQueryBuilder as any);

      const response = await request(app).post("/api/query/build").send({
