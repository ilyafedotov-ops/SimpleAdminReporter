...
export class Database {
  ...
  private constructor() {
    ...
    if (process.env.DATABASE_URL) {
      ...
      if (!password && !allowPasswordless) {
        throw new Error(
          "Database password is missing in DATABASE_URL. Please include password in the connection string.",
        );
      }
      ...
      if (!password && !allowPasswordless) {
        throw new Error(
          "Database password is missing. Please set DB_PASSWORD or POSTGRES_PASSWORD environment variable.",
        );
      }
  ...
  public async query(text: string, params?: any[]): Promise<any> {
    ...
    try {
      ...
      client = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error("Failed to acquire database connection from pool"),
              ),
            10000,
          ),
        ),
      ...
      if ((error as any).code === "57014") {
        throw new Error(
          "Query timeout: The query took too long to execute (>25s)",
        );
      }
