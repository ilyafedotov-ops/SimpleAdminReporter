  ...
  describe("loginValidation", () => {
    ...
    describe("Valid Login Data", () => {
      ...
      it("should pass validation with all valid authSource values", async () => {
        const validAuthSources = ["ad", "local"];
    ...
    describe("Username Validation", () => {
      it("should fail validation with missing username", async () => {
        ...
        expect(errors.some((error: any) => error.path === "username")).toBe(
          true,
        );
        expect(
          errors.some((error: any) => error.msg === "Username is required"),
        ).toBe(true);
      });
      ...
      it("should fail validation with username too short", async () => {
        ...
        expect(
          errors.some(
            (error: any) =>
              error.path === "username" &&
              error.msg === "Username must be 3-50 characters",
          ),
        ).toBe(true);
      });
    ...
    describe("Password Validation", () => {
      ...
      it("should fail validation with password too long", async () => {
        ...
        expect(
          errors.some(
            (error: any) =>
              error.path === "password" && error.msg === "Password too long",
          ),
        ).toBe(true);
      });
    ...
    describe("AuthSource Validation", () => {
      it("should fail validation with invalid authSource", async () => {
        ...
        expect(
          errors.some(
            (error: any) =>
              error.path === "authSource" &&
              error.msg === "Invalid authentication source",
          ),
        ).toBe(true);
      });
      ...
      it("should prevent SQL injection via authSource", async () => {
        ...
        const errors = result.array();
        expect(errors.some((error: any) => error.path === "authSource")).toBe(
          true,
        );
      });
    });
    ...
    describe("Display Name Validation", () => {
      it("should fail validation with missing display name", async () => {
        ...
        expect(
          errors.some(
            (error: any) =>
              error.path === "displayName" &&
              error.msg === "Display name is required",
          ),
        ).toBe(true);
      });
      ...
      it("should fail validation with display name too long", async () => {
        ...
        expect(
          errors.some(
            (error: any) =>
              error.path === "displayName" &&
              error.msg === "Display name must be 1-100 characters",
          ),
        ).toBe(true);
      });
    ...
    describe("Email Validation", () => {
      it("should fail validation with missing email", async () => {
        ...
        expect(
          errors.some(
            (error: any) =>
              error.path === "email" && error.msg === "Valid email is required",
          ),
        ).toBe(true);
      });
      ...
      it("should fail validation with invalid email format", async () => {
        ...
        for (const email of invalidEmails) {
          ...
          const errors = result.array();
          expect(errors.some((error: any) => error.path === "email")).toBe(
            true,
          );
        }
      });
    ...
    describe("Admin Flag Validation", () => {
      ...
      it("should fail validation with non-boolean isAdmin values", async () => {
        ...
        for (const isAdmin of nonBooleanValues) {
          ...
          if (!result.isEmpty()) {
            const errors = result.array();
            const hasIsAdminError = errors.some(
              (error: any) =>
                (error.path === "isAdmin" || error.param === "isAdmin") &&
                (error.msg === "isAdmin must be a boolean" ||
                  error.msg.includes("boolean") ||
        ...
        expect(
          errors.some(
            (error: any) =>
              ...
              error.msg === "New password is required",
          ),
        ).toBe(true);
      });
      ...
      it("should fail validation with new password shorter than 8 characters", async () => {
        ...
        expect(
          errors.some(
            (error: any) =>
              error.path === "newPassword" &&
              error.msg === "New password must be at least 8 characters",
          ),
        ).toBe(true);
      });
  ...
  describe("Integration Security Tests", () => {
    describe("Cross-Validation Security", () => {
      it("should maintain consistent validation behavior across all validators", async () => {
        ...
        const createResult = await runValidation(
          createUserValidation,
          req,
          res,
        );
        ...
        const changeResult = await runValidation(
          changePasswordValidation,
          req,
          res,
        );
    ...
    describe("Error Message Security", () => {
      it("should not expose sensitive information in error messages", async () => {
        ...
        expect(errorMessages).not.toMatch(
          /database|sql|internal|system|server|config/i,
        );
