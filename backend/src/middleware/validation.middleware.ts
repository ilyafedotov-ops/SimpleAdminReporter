...
export const sanitizeInput = (
  ...
  try {
    ...
    // Basic XSS protection - HTML-escape string values before handlers use them.
...
export const validatePagination = (
  ...
  try {
    ...
    if (offset < 0) {
      return next(
        createError("Offset must be greater than or equal to 0", 400),
      );
    }
    ...
    try {
      ...
      if (file) {
        ...
        if (options.maxSize && file.size > options.maxSize) {
          return next(
            createError(
              `File too large. Maximum size: ${options.maxSize} bytes`,
              400,
            ),
          );
        }
