...
describe("CredentialEncryption", () => {
  ...
  const mockEncryptionKey =
    "test-encryption-key-that-is-very-long-and-secure-12345678";
  ...
  beforeEach(() => {
    ...
    (crypto.randomInt as jest.Mock).mockImplementation((_max: number) => 0);
  ...
  describe("decrypt", () => {
    ...
    beforeEach(() => {
      ...
      (crypto.pbkdf2Sync as jest.Mock).mockReturnValue(
        Buffer.from("derived-key-32-bytes-long-1234567"),
      );
    });
    ...
    it("should handle insufficient data length", () => {
      ...
      expect(() => encryption.decrypt(shortData)).toThrow(
        "Failed to decrypt credential",
      );
    });
  ...
  describe("decryptWithSalt", () => {
    ...
    it("should throw error when salt is invalid hex", () => {
      ...
      expect(() =>
        encryption.decryptWithSalt(encryptedData, invalidSalt),
    ...
    it("should throw error when decryption fails", () => {
      ...
      expect(() => encryption.decryptWithSalt(encryptedData, saltHex)).toThrow(
        "Failed to decrypt credential with provided salt",
      );
    });
  });
  ...
  describe("rotateKey", () => {
    ...
    it("should handle rotation errors gracefully", () => {
      ...
      expect(() => encryption.rotateKey(oldKey, newKey, encryptedData)).toThrow(
        "Failed to decrypt credential",
      );
    });
  });
  ...
  describe("generateSecurePassword", () => {
    ...
    it("should generate secure random password with default length", () => {
      (crypto.randomInt as jest.Mock).mockImplementation(() => 10);

      const password = encryption.generateSecurePassword();

      expect(crypto.randomInt).toHaveBeenCalledWith(passwordCharset.length);
      expect(crypto.randomInt).toHaveBeenCalledTimes(32);
    ...
    it("should generate password with custom length", () => {
      ...
      (crypto.randomInt as jest.Mock).mockImplementation(() => 11);

      const password = encryption.generateSecurePassword(customLength);

      expect(crypto.randomInt).toHaveBeenCalledTimes(customLength);
    ...
    it("should use crypto.randomInt to avoid modulo bias", () => {
      const password = encryption.generateSecurePassword(1);
    ...
    it("should handle large lengths", () => {
      ...
      expect(crypto.randomInt).toHaveBeenCalledTimes(largeLength);
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle null and undefined inputs gracefully", () => {
      ...
      expect(() => encryption.decrypt(null as any)).toThrow(
        "Failed to decrypt credential",
      );
      expect(() => encryption.decrypt(undefined as any)).toThrow(
        "Failed to decrypt credential",
      );
    });
      ...
      const promises = Array(10)
        ...
        .map((_, i) => {
          ...
          const mockCipher = {
            ...
            getAuthTag: jest
              .fn()
              .mockReturnValue(Buffer.from(`tag-${i}`.padEnd(16, "0"))),
          };
...
describe("getCredentialEncryption singleton", () => {
  beforeEach(() => {
    ...
    process.env.CREDENTIAL_ENCRYPTION_KEY =
      "test-key-32-characters-long-123456";
...
describe("Integration Tests", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CREDENTIAL_ENCRYPTION_KEY =
      "test-integration-key-that-is-very-long-and-secure-123456789";
    process.env.CREDENTIAL_ENCRYPTION_SALT =
      "abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890";
  ...
  it("should perform complete encrypt-decrypt cycle", () => {
    const originalText =
      "This is a sensitive password with special chars: !@#$%^&*()";

    // Setup mocks for encryption
    const mockSalt = Buffer.from(
      "abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890",
      "hex",
    );
    const mockIv = Buffer.from("1234567890123456", "ascii");
    const mockAuthTag = Buffer.from("authtagauthtagau", "ascii");
    const mockDerivedKey = Buffer.from(
      "derivedkey1234567890123456789012",
      "ascii",
    );
  ...
  it("should generate unique encrypted outputs for same input", () => {
    ...
    (crypto.randomBytes as jest.Mock)
      .mockReturnValueOnce(
        Buffer.from("salt1234567890123456789012345678901", "ascii"),
      )
      .mockReturnValueOnce(Buffer.from("iv123456789012345", "ascii"))
      .mockReturnValueOnce(
        Buffer.from("salt9876543210987654321098765432109", "ascii"),
      )
  ...
  it("should handle various data types and sizes", () => {
    const testCases = ["", "a", "short password"];
  ...
  it("should generate secure passwords with expected characteristics", () => {
    (crypto.randomInt as jest.Mock).mockReturnValue(26);

    const password = encryption.generateSecurePassword(50);

    expect(password).toHaveLength(50);
    expect(typeof password).toBe("string");
    expect(password).toBe("A".repeat(50));
    expect(crypto.randomInt).toHaveBeenCalledTimes(50);
  });
});
