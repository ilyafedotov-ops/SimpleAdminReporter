...
describe("LDAP Utilities", () => {
  ...
  describe("Windows FileTime Conversions", () => {
    ...
    it("should convert days ago to Windows FileTime", () => {
      ...
      const daysDiff = Math.round(
        (now.getTime() - fileTimeAsDate!.getTime()) / (1000 * 60 * 60 * 24),
      );
    ...
    it("should convert hours ago to Windows FileTime", () => {
      ...
      const hoursDiff = Math.round(
        (now.getTime() - fileTimeAsDate!.getTime()) / (1000 * 60 * 60),
      );
  ...
  describe("buildFilterComponent", () => {
    it("should build equals filter", () => {
      expect(buildFilterComponent("name", "equals", "John")).toBe(
        "(displayName=John)",
      );
    });

    it("should build contains filter", () => {
      expect(buildFilterComponent("name", "contains", "oh")).toBe(
        "(displayName=*oh*)",
      );
    });

    it("should build startsWith filter", () => {
      expect(buildFilterComponent("name", "startsWith", "Jo")).toBe(
        "(displayName=Jo*)",
      );
    });
    ...
    it("should build not_exists filter", () => {
      expect(buildFilterComponent("mail", "not_exists", "")).toBe(
        "(!(mail=*))",
      );
    });
    ...
    it("should build older_than filter", () => {
      ...
      expect(filter).toMatch(/^\(lastLogonTimestamp<=\d+\)$/);
    });

    it("should throw error for unknown operator", () => {
      expect(() => buildFilterComponent("mail", "unknown", "value")).toThrow(
  ...
  describe("buildComplexFilter", () => {
    it("should combine filters with AND", () => {
      ...
      expect(result).toBe(
        "(&(objectClass=user)(department=IT)(title=*Manager*))",
      );
    });
  ...
  describe("User Account Control", () => {
    it("should check if account is disabled", () => {
      ...
      expect(
        isAccountDisabled(
          UAC_FLAGS.ACCOUNT_DISABLED | UAC_FLAGS.NORMAL_ACCOUNT,
        ),
      ).toBe(true);
