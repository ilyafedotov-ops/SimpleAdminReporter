...
describe("Preview Data Extractor - Error Scenarios and Stress Tests", () => {
  describe("Memory and Performance Stress Tests", () => {
    it.skip("should handle extremely large arrays without memory issues", () => {
  ...
  describe("Concurrent Access and Thread Safety", () => {
    ...
    it.skip("should handle concurrent processing of different data types", async () => {
      const testCases = [
        [{ id: 1 }, { id: 2 }],
        { data: [{ id: 3 }], count: 1 },
  ...
  describe("Error Recovery and Graceful Degradation", () => {
    it("should handle objects that throw on property access", () => {
      ...
      expect(() => extractPreviewData(throwingObject)).toThrow(
    ...
    it("should handle objects with getters that return different values on each call", () => {
      ...
      expect(result).toHaveProperty("data");
      expect(result).toHaveProperty("rowCount");
      expect(result.metadata.originalFormat).toMatch(
        /query_result|single_object/,
      );
    });
