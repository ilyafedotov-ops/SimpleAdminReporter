...
const skipIfNoDb = () => {
  ...
  const hasDb =
    dbUrl && !dbUrl.includes("undefined") && dbUrl.includes("postgresql");
...
describe("Cookie Authentication System", () => {
  ...
  beforeEach(() => {
    ...
    app.post("/api/auth/login", loginRateLimiter, (req, res, _next) =>
      unifiedAuthController.login(req, res, _next),
    );
    app.post(
      "/api/auth/logout",
      authEndpointsRateLimiter,
      authMiddleware,
      (req, res, _next) => unifiedAuthController.logout(req, res, _next),
    );
    app.get(
      "/api/auth/profile",
      authEndpointsRateLimiter,
      authMiddleware,
      (req, res, _next) => unifiedAuthController.getProfile(req, res, _next),
    );
    app.get("/api/auth/csrf", authEndpointsRateLimiter, (req, res, _next) =>
      unifiedAuthController.getCSRFToken(req, res, _next),
    );

    // Protected test route; global CSRF middleware protects cookie requests.
    app.post("/api/test/protected", authMiddleware, (req, res) => {
  ...
  describe("Session Management", () => {
    ...
    it.skip("should remove session on logout", async () => {
      ...
      // Logout
      await request(app).post("/api/auth/logout").set("Cookie", authCookies);
