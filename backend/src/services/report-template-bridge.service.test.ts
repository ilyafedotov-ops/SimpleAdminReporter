...
import {
  ReportTemplateBridgeService,
  reportTemplateBridge,
} from "./report-template-bridge.service";
...
describe("ReportTemplateBridgeService", () => {
  ...
  const mockQueryDefinition: LDAPQueryDefinition = {
    ...
    name: "Inactive Users",
    description:
      "Find users who have not logged in for a specified number of days",
    category: "users",
    query: {
      scope: "sub",
      filter:
        "(&(objectClass=user)(objectCategory=person)(lastLogonTimestamp>=1))",
      attributes: [
        "sAMAccountName",
        "displayName",
        "mail",
  ...
  describe("getQueryDefinitionByReportType", () => {
    ...
    it("should return query definition if reportType is already a query ID", () => {
      ...
      const result = service.getQueryDefinitionByReportType(
        "some_direct_query_id",
      );
    ...
    it("should throw error if query definition not found for mapped query ID", () => {
      ...
      const mockError = new Error(
        "Query definition not found for: inactive_users",
      ) as any;
  ...
  describe("transformParameters", () => {
    it("should transform parameters using parameter mapping", () => {
      ...
      const result = service.transformParameters(
        "ad_inactive_users",
        oldParams,
      );
    ...
    it("should handle all parameter mappings", () => {
      const testCases = [
        {
          reportType: "ad_password_expiry",
          params: { days: 30 },
          expected: { days: 30 },
        },
        {
          reportType: "ad_recent_lockouts",
          params: { hours: 24 },
          expected: { hours: 24 },
        },
        {
          reportType: "ad_password_changes",
          params: { hours: 48 },
          expected: { hours: 48 },
        },
      ];
  ...
  describe("getAllQueryDefinitions", () => {
    ...
    it("should return all query definitions with proper structure", () => {
      ...
      expect(result).toHaveLength(14); // Total number of mapped report types
  ...
  describe("getDataSourceByReportType (private method)", () => {
    ...
    it("should return azure for azure_ prefixed types", () => {
      ...
      service.getAvailableReportTypes = jest
        .fn()
        .mockReturnValue(["azure_test_report"]);
      service.getQueryDefinitionByReportType = jest
        .fn()
        .mockReturnValue(azureQueryDef);
    ...
    it("should return o365 for o365_ prefixed types", () => {
      ...
      service.getAvailableReportTypes = jest
        .fn()
        .mockReturnValue(["o365_test_report"]);
      service.getQueryDefinitionByReportType = jest
        .fn()
        .mockReturnValue(o365QueryDef);
    ...
    it("should return ad as default fallback", () => {
      ...
      service.getAvailableReportTypes = jest
        .fn()
        .mockReturnValue(["test_report"]);
      service.getQueryDefinitionByReportType = jest
        .fn()
        .mockReturnValue(queryDefWithoutPrefix);
  ...
  describe("integration scenarios", () => {
    it("should handle complete workflow for existing report type", () => {
      ...
      // Get query ID
      expect(service.getQueryIdByReportType("ad_inactive_users")).toBe(
        "inactive_users",
      );

      // Transform parameters
      const params = { days: 30, customParam: "test" };
      const transformedParams = service.transformParameters(
        "ad_inactive_users",
        params,
      );
