...
describe("SocketService", () => {
  ...
  describe("initialize", () => {
    ...
    it("should setup authentication middleware with token from cookies", async () => {
      socketService.initialize(mockHttpServer);
      ...
      const mockSocket = {
        handshake: {
          ...
          headers: {
            cookie:
              "csrf_token=csrf-token; access_token=cookie-token; theme=dark",
          },
        },
      };
      const mockNext = jest.fn();
    ...
    it("should setup logs namespace", () => {
      ...
      const io = getSocketIO(socketService);
      expect(io.namespaces.has("/socket/logs")).toBe(true);

      const logsNamespace = io.namespaces.get("/socket/logs");
  ...
  describe("logs namespace connection handling", () => {
    beforeEach(() => {
      socketService.initialize(mockHttpServer);
      const io = getSocketIO(socketService);
      mockLogsNamespace = io.of("/socket/logs");
    });
    ...
    it("should handle socket connection to logs namespace", () => {
      ...
      const connectionHandler =
        mockLogsNamespace.eventHandlers.get("connection");
      ...
      const activeSockets = (socketService as unknown as SocketServicePrivate)
        .activeSockets;
    ...
    it("should handle subscription to log types", () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "User 1 subscribed to audit logs",
      );
      expect(logger.info).toHaveBeenCalledWith(
        "User 1 subscribed to system logs",
      );
    });
    ...
    it("should handle unsubscription from log types", () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "User 1 unsubscribed from audit logs",
      );
      expect(logger.info).toHaveBeenCalledWith(
        "User 1 unsubscribed from system logs",
      );
    });
  ...
  describe("log event listeners", () => {
    ...
    it("should handle audit log events", () => {
      ...
      const auditLogHandler = (logEventEmitter.on as jest.Mock).mock.calls.find(
    ...
    it("should handle system log events", () => {
      ...
      const systemLogHandler = (
        logEventEmitter.on as jest.Mock
      ).mock.calls.find((call) => call[0] === "system_log")?.[1];
    ...
    it("should handle generic log events", () => {
      ...
      const newLogHandler = (logEventEmitter.on as jest.Mock).mock.calls.find(
        (call) => call[0] === "newLog",
      )?.[1];
  ...
  describe("matchesFilters", () => {
    ...
    it("should match event type filter", () => {
      ...
      const nonMatchResult = (socketService as any).matchesFilters(log, {
        eventType: "logout",
      });
    ...
    it("should match search filter in multiple fields", () => {
      ...
      expect(
        (socketService as any).matchesFilters(log, { search: "user" }),
      ).toBe(true);
      expect(
        (socketService as any).matchesFilters(log, { search: "login" }),
      ).toBe(true);
      expect(
        (socketService as any).matchesFilters(log, { search: "testuser" }),
      ).toBe(true);
      expect(
        (socketService as any).matchesFilters(log, { search: "auth" }),
      ).toBe(true);
      expect(
        (socketService as any).matchesFilters(log, { search: "ldap" }),
      ).toBe(true);
      expect(
        (socketService as any).matchesFilters(log, { search: "notfound" }),
      ).toBe(false);
    });
    ...
    it("should match date range filters", () => {
      ...
      expect(
        (socketService as any).matchesFilters(logWithCreatedAt, filters),
      ).toBe(true);
      ...
      expect((socketService as any).matchesFilters(log, filtersOutside)).toBe(
        false,
      );
    });
    ...
    it("should match start date only", () => {
      ...
      expect(
        (socketService as any).matchesFilters(log, {
          startDate: "2025-01-01T00:00:00Z",
        }),
      ).toBe(true);
      expect(
    ...
    it("should handle missing searchable fields gracefully", () => {
      ...
      expect(
        (socketService as any).matchesFilters(log, { search: "test" }),
      ).toBe(false);
    });
    ...
    it("should match all filters when multiple are provided", () => {
      ...
      expect(
        (socketService as any).matchesFilters(log, filtersWithWrongLevel),
      ).toBe(false);
    });
  });
  ...
  describe("emitToUsers", () => {
    ...
    it("should emit to specific users", () => {
      const io = getSocketIO(socketService);
      const logsNamespace = io.of("/socket/logs");
  ...
  describe("broadcast", () => {
    ...
    it("should broadcast to all connected clients", () => {
      ...
      expect(emitSpy).toHaveBeenCalledWith("announcement", {
        message: "Server maintenance",
      });
    });
  ...
  describe("getStats", () => {
    ...
    it("should return connection statistics", () => {
      ...
      // Mock some rooms
      logsNamespace.adapter.rooms.set(
        "logs:audit",
      ...
      activeSockets.set(
        "socket-1",
        createAuthenticatedMockSocket("socket-1", 1),
      );
      activeSockets.set(
        "socket-2",
        createAuthenticatedMockSocket("socket-2", 2),
      );
  ...
  describe("shutdown", () => {
    ...
    it("should gracefully shutdown Socket.IO server", () => {
      const io = getSocketIO(socketService);
      const _logsNamespace = io.of("/socket/logs"); // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  describe("error handling and edge cases", () => {
    it("should handle setupLogsNamespace when io is null", () => {
      ...
      expect(() => {
        (
          uninitializedService as unknown as SocketServicePrivate
        ).setupLogsNamespace();
      }).not.toThrow();
    });
    ...
    it("should handle filters with undefined properties", () => {
      ...
      const result = (socketService as any).matchesFilters(
        log,
    ...
    it("should handle malformed date strings in filters", () => {
      ...
      expect(() => {
        (socketService as unknown as SocketServicePrivate).matchesFilters(
          log,
          filters,
        );
      }).not.toThrow();
