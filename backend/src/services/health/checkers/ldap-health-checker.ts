...
export class LDAPHealthChecker extends BaseHealthChecker {
  ...
  protected async performCheck(
    ...
    const placeholderServers = new Set([
      "placeholder",
      "dc.example.com",
      "example.com",
    ]);
    const isExampleDomain =
      normalizedServer === "example.com" ||
      normalizedServer.endsWith(".example.com");

    // Check if using placeholder values - treat as not configured
    if (
      placeholderServers.has(normalizedServer) ||
      isExampleDomain ||
    ...
    try {
      ...
      logger.debug(
        `LDAP health check: Testing TCP connection to ${server}:${port}`,
      );
      ...
      if (isPortOpen) {
        return this.createHealthyResult(Date.now() - context.startTime, {
          server: server,
          port: port,
