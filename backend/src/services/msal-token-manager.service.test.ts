import {
  MsalTokenManager,
  msalTokenManager,
} from "./msal-token-manager.service";
import {
...
const mockAzureCredentialService = azureCredentialService as jest.Mocked<
  typeof azureCredentialService
>;
...
describe("MsalTokenManager", () => {
  ...
  describe("Constructor", () => {
    ...
    it("should throw error when required environment variables are missing", () => {
      ...
      expect(() => new MsalTokenManager()).toThrow(
        "Required MSAL configuration is missing",
      );
    });
  });
  ...
  describe("getAppOnlyToken", () => {
    ...
    it("should acquire new token when cache is empty", async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "App-only token acquired successfully",
      );
    });
  ...
  describe("getDelegatedToken", () => {
    ...
    it("should return cached token when available and valid", async () => {
      ...
      expect(result).toBe(cachedToken);
      expect(mockRedis.getJson).toHaveBeenCalledWith(
        `msal:token:user:${userId}:${mockScopes.join(",")}`,
      );
    });
    ...
    it("should return stored token when not cached", async () => {
      ...
      expect(mockAzureCredentialService.getCredentials).toHaveBeenCalledWith(
        userId,
      );
    });
    ...
    it("should throw error when no credentials found", async () => {
      ...
      await expect(
        tokenManager.getDelegatedToken(userId, mockScopes),
  ...
  describe("getOnBehalfOfToken", () => {
    ...
    it("should acquire OBO token successfully", async () => {
      ...
      const result = await tokenManager.getOnBehalfOfToken(
        userAccessToken,
        mockScopes,
      );
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        "On-behalf-of token acquired successfully",
      );
    });
    ...
    it("should handle null response from MSAL client", async () => {
      ...
      await expect(
        tokenManager.getOnBehalfOfToken(userAccessToken, mockScopes),
  ...
  describe("getCachedToken", () => {
    it("should return token when cache hit and token is valid", async () => {
      ...
      const result = await (tokenManager as any).getCachedToken(
        "test-cache-key",
      );
  ...
  describe("clearUserTokenCache", () => {
    it("should clear all tokens for a specific user", async () => {
      ...
      expect(mockRedis.invalidatePattern).toHaveBeenCalledWith(
        `msal:token:user:${userId}:*`,
      );
      expect(mockLogger.info).toHaveBeenCalledWith(
        `Token cache cleared for user ${userId}`,
      );
    });
  });
  ...
  describe("Concurrent Access Scenarios", () => {
    it("should handle concurrent getAppOnlyToken requests", async () => {
      ...
      mockRedis.getJson.mockResolvedValueOnce(null).mockResolvedValue({
        accessToken: "concurrent-token",
      ...
      const promises = Array(5)
        .fill(null)
        .map(() => tokenManager.getAppOnlyToken(mockScopes));
    ...
    it("should handle concurrent getDelegatedToken requests", async () => {
      ...
      const promises = Array(3)
        .fill(null)
        .map(() => tokenManager.getDelegatedToken(userId, mockScopes));
      const results = await Promise.all(promises);
  ...
  describe("Error Recovery Scenarios", () => {
    ...
    it("should handle Azure credential service failures", async () => {
      ...
      mockRedis.getJson.mockResolvedValue(null);
      mockAzureCredentialService.getCredentials.mockRejectedValue(
    ...
    it("should handle crypto service failures during token refresh", async () => {
      ...
      mockCryptoService.encryptToken.mockRejectedValue(
  ...
  describe("Token Validation and Edge Cases", () => {
    ...
    it("should handle very short token lifetimes", async () => {
      ...
      await (tokenManager as any).cacheToken(
        "short-lived-key",
        shortLivedResponse,
      );

      const ttl = mockRedis.setJson.mock.calls[0][2];
      expect(ttl).toBeGreaterThanOrEqual(52);
      expect(ttl).toBeLessThanOrEqual(54);
    });
