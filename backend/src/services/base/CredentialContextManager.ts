...
export class CredentialContextManager {
  ...
  private credentialCache: Map<
    string,
    { credentials: ServiceCredentials; timestamp: number }
  > = new Map();
  ...
  private async decryptCredentials(
    ...
    // The row comes from `SELECT * FROM service_credentials`, so columns are
    // snake_case and the salt lives in `encryption_salt` (v1 payloads embed
    // their own salt; legacy payloads rely on the stored salt).
    const salt: string | undefined = credential.encryption_salt;

    if (salt === "NEEDS_REGENERATION") {
      throw new CredentialError(
        "Credentials need to be re-entered due to missing encryption salt",
      );
    }

    const decryptField = (value: string, label: string): string => {
      try {
        if (value.startsWith("v1:")) {
          // v1 format has an embedded salt
          return encryption.decrypt(value);
        }
        if (salt && salt !== "legacy") {
          // Legacy format with a stored salt
          return encryption.decryptWithSalt(value, salt);
        }
        throw new CredentialError(
          `Cannot decrypt legacy ${label} without salt`,
        );
      } catch (error) {
        if (error instanceof CredentialError) {
          throw error;
        }
        throw new CredentialError(`Failed to decrypt ${label}`, error as Error);
      }
    };

    // Decrypt password (AD/local)
    if (credential.encrypted_password) {
      credentials.password = decryptField(
    ...
    if (credential.encrypted_client_secret) {
      credentials.clientSecret = decryptField(
        credential.encrypted_client_secret,
        "client secret",
      );
      credentials.tenantId = credential.tenant_id;
      credentials.clientId = credential.client_id;
    }
