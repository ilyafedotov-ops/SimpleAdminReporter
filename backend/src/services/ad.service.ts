...
export class ADService extends BaseDataSourceService {
  ...
  ): Promise<QueryResult<StandardADUser>> {
   ...
   */
  protected async getConnectionOptions(
    context?: CredentialContext,
    ...
    if (!credentials) {
      throw new DataSourceError(
        "No credentials available for AD connection",
        "NO_CREDENTIALS",
      );
    }
    ...
    try {
      ...
      this.logger.debug("Attempting LDAP bind", {
        ...
        bindDN: options.bindDN
          ? "***" + options.bindDN.substring(options.bindDN.length - 10)
          : null,
      ...
      // Bind to AD
      await client.bind(
        options.bindDN as string,
        options.bindCredentials as string,
      );
  ...
  protected getConnectionPoolKey(options: ConnectionOptions): string {
    ...
    const credentialSource = this.credentialContext?.useSystemCredentials
      ? "system"
      : this.credentialContext?.userId
        ? `user-${this.credentialContext.userId}`
        : "default";
   ...
   */
  protected async isConnectionValid(
  ...
  private async executeLDAPSearch(
    ...
    if (!baseDN || !/^[A-Za-z]+\s*=/u.test(baseDN)) {
      if (baseDN) {
        this.logger.warn(
          `Invalid baseDN provided ('${baseDN}'), falling back to default '${this.baseDN}'.`,
        );
      }
      baseDN = this.baseDN;
    }
  ...
  async getUsersWithExpiringPasswords(
      ...
      filter: buildComplexFilter(LDAP_FILTERS.USER, [
        {
          ...
          operator: "not_equals",
          value: UAC_FLAGS.DONT_EXPIRE_PASSWORD,
        },
      ]),
      attributes: [...LDAP_ATTRIBUTES.USER],
    };
    ...
    return ((result as any)?.data).filter((user: any) => {
      ...
      const daysUntilExpiry =
        (maxPasswordAge - passwordAge) / (24 * 60 * 60 * 1000);
   ...
   */
  async getUsersWithNeverExpiringPasswords(
    context?: CredentialContext,
  ): Promise<StandardADUser[]> {
    const query: ADQuery = {
  ...
  async executeCustomQuery(
    ...
    if (customQuery.filter && typeof customQuery.filter === "string") {
      ...
      const conditions = customQuery.filters.map((f: any) => ({
        ...
        operator: f.operator,
        value:
          parameters[f.field] !== undefined ? parameters[f.field] : f.value,
      }));
    ...
    const query: ADQuery = {
      ...
      filter,
      attributes:
        customQuery.fields?.map((f: any) => resolveFieldAlias(f.name)) ||
        LDAP_ATTRIBUTES.USER,
  ...
  async authenticateUser(username: string, password: string): Promise<boolean> {
    try {
      ...
      const serverUrl =
        this.formatLdapUrl(this.config.server || process.env.AD_SERVER) ||
        "ldap://localhost:389";
      ...
      const domain =
        process.env.AD_DOMAIN || this.config.domain || "domain.local";
      ...
      try {
        ...
        this.logger.warn(
          `AD authentication failed for ${username}: ${err.message}`,
        );
...
export function resetADService(): void {
  if (instance) {
    instance
      .closeAllConnections()
      .catch((err) => logger.error("Error closing AD connections:", err));
    instance = null;
  }
}
