...
jest.mock("../utils/ldap-utils", () => ({
  convertLDAPToUser: jest.fn((user: any) => ({
    username:
      user.attributes?.sAMAccountName || user.sAMAccountName || "test.user",
    displayName:
      user.attributes?.displayName || user.displayName || "Test User",
...
describe("ADService", () => {
  ...
  describe("Connection Management", () => {
    ...
    it("should format LDAP URL correctly", () => {
      ...
      expect(formatLdapUrl("server.domain.local")).toBe(
        "ldap://server.domain.local:389",
      );
      expect(formatLdapUrl("ldap://server.domain.local")).toBe(
        "ldap://server.domain.local",
      );
      expect(formatLdapUrl("ldaps://server.domain.local")).toBe(
  ...
  describe("Report Methods", () => {
    ...
    it("should get users with expiring passwords", async () => {
        ...
        searchEntries: [
          {
            ...
            passwordLastSet: (
              (eightlyDaysAgo.getTime() + 11644473600000) *
              10000
            ).toString(),
            userAccountControl: 512,
          },
      ...
      const mockConvertLDAPToUser =
        require("../utils/ldap-utils").convertLDAPToUser;
  ...
  describe("Custom Query Execution", () => {
    it("should execute custom query with filters", async () => {
      const customQuery = {
        ...
        filters: [{ field: "department", operator: "equals", value: "IT" }],
    ...
    it("should handle custom query with parameters", async () => {
      const customQuery = {
        filters: [{ field: "title", operator: "contains", value: null }],
      };
  ...
  describe("Security and Input Validation", () => {
    ...
    it("should handle special characters in passwords", async () => {
      const specialPassword = "complex-test-password-special";
