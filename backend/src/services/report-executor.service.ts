...
export class ReportExecutorService {
  ...
  async executeReport(
    ...
    try {
      ...
      logger.info(
        `Executing report: ${template.name} (${template.category}) for user ${userId}`,
      );
      ...
      if (credentialId) {
        ...
        credential = await credentialsService.getCredential(
          credentialId,
          userId,
        );
        if (!credential) {
          throw new Error(
            `Credential with ID ${credentialId} not found for user`,
          );
        }
        if (credential.serviceType !== template.category) {
          throw new Error(
            `Credential type mismatch: credential is for ${credential.serviceType} but report requires ${template.category}`,
          );
        }
      } else {
        // Try to get default credential for the service
        credential = await credentialsService.getDefaultCredential(
          userId,
          template.category,
        );
        if (!credential) {
          logger.info(
            `No default ${template.category} credential found for user ${userId}, will use system defaults`,
          );
        }
      }
      ...
      if (credential) {
        try {
          decryptedCreds = await credentialsService.getDecryptedCredential(
            credential.id,
            userId,
          );
          logger.info(
            `Using credential ${credential.id} (${credential.credentialName}) for report execution`,
          );
        } catch (decryptError) {
          logger.error(
        ...
        logger.warn(
          `No user credential found for ${template.category}, using system defaults`,
        );
      }
      ...
      try {
        const ldapQueryDef =
          reportTemplateBridge.getQueryDefinitionByReportType(
        ...
        if (ldapQueryDef) {
          queryDef = {
            ...
            parameters: Object.entries(ldapQueryDef.parameters || {}).map(
      ...
      if (queryDef && this.queryService) {
        ...
        const result: QueryResult<any> = await this.queryService.executeQuery(
          queryDef,
          queryContext,
        );
        ...
        try {
            ...
            query: {
              fields: template.field_mappings
                ? Object.keys(template.field_mappings).map((field) => ({
                    name: field,
                    displayName:
                      template.field_mappings[field]?.displayName || field,
                  }))
                : [],
              filters: [],
          ...
          const previewResponse =
            await previewService.executePreview(previewRequest);

          if (!previewResponse.success || !previewResponse.data) {
            throw new Error(
              previewResponse.error?.message ||
                "Preview service execution failed",
            );
          }
          ...
          logger.warn(
            `PreviewService failed for template ${templateId}, falling back to direct service execution:`,
            previewError,
          );
          ...
          switch (template.category) {
            case "ad":
              data = await this.executeADReport(
                template,
                finalParameters,
                decryptedCreds,
                userId,
              ...
              break;
            case "azure":
              data = await this.executeAzureReport(
                template,
                finalParameters,
                decryptedCreds,
                userId,
              ...
              break;
            case "o365":
              data = await this.executeO365Report(
                template,
                finalParameters,
                decryptedCreds,
              );
      ...
      const executionTime = Date.now() - startTime;
      logger.error(
        `Report execution failed for user ${userId}: ${(error as Error).message}`,
      );
      ...
      try {
        ...
        logger.error(
          "Failed to save execution history for failed report:",
          saveError,
        );
      }
  ...
  private async executeADReport(
    ...
    if (credentials && credentials.username && credentials.encryptedPassword) {
      try {
        ...
        logger.error(
          "Failed to create AD service with user credentials:",
          error,
        );
    ...
    if (template.query_template) {
      ...
      const calculatedParams = this.calculateReportParameters(
        template.report_type,
        parameters,
      );
      ...
      let finalResult = Array.isArray(result)
        ? result
        : result.entries || (result as any)?.data || [];

      // Apply post-processing filters if needed
      if (
        parameters.postFilter === "passwordChangeCutoff" &&
        calculatedParams.passwordChangeCutoff
      ) {
        ...
        finalResult = finalResult.filter((user: any) => {
          if (user.passwordLastSet) {
            try {
              ...
              logger.debug(
                `Failed to parse passwordLastSet for user ${user.sAMAccountName}:`,
                e,
              );
              return false;
            }
        ...
        logger.info(
          `Filtered ${Array.isArray(result) ? result.length : 0} users to ${finalResult.length} based on password change cutoff (cutoff: ${new Date(Number(cutoffTimestamp) / 10000 - 11644473600000).toISOString()})`,
        );
      }
  ...
  private async executeAzureReport(
    ...
    if (isGraphQuery) {
      ...
      const { GraphQueryExecutor } =
        await import("@/services/graph-query-executor.service");
      ...
      if (!queryDef) {
        throw new Error(
          `Graph query definition not found for ${template.report_type}`,
        );
      }
    ...
    if (template.query_template) {
      ...
      if (!azureConfig.tenantId) {
        throw new Error(
          "AZURE_TENANT_ID environment variable is not configured",
        );
      }

      // Replace template variables with environment config
      if (query.endpoint) {
        query.endpoint = query.endpoint.replace(
          "{{tenantId}}",
  ...
  private async executeO365Report(
    ...
    if (template.query_template) {
      ...
      logger.info(
        `Executing O365 Graph API query for report ${template.name}:`,
        { query },
      );
   ...
   */
  private async createUserADService(
    ...
    try {
      ...
      const __credentialContext = {
        credentials: {
          ...
          // `encryptedPassword` already holds the decrypted plaintext here:
          // getDecryptedCredential() (credentials.service.ts) decrypts before this point.
   ...
   */
  private async createUserAzureService(
    _credentials: EncryptedCredential,
  ): Promise<any> {
    ...
    try {
      ...
      logger.error(
        "Failed to create Azure service with user credentials:",
        error,
      );
      throw error;
    }
   ...
   */
  private async createUserO365Service(
    _credentials: EncryptedCredential,
  ): Promise<any> {
    ...
    try {
      ...
      logger.error(
        "Failed to create O365 service with user credentials:",
        error,
      );
      throw error;
    }
    ...
    switch (reportType) {
      ...
      case "inactive_computers":
        ...
        if (parameters.days !== undefined) {
          ...
          const windowsFileTime = (
            (cutoffDate.getTime() + 11644473600000) *
            10000
          ).toString();
      ...
      case "password_expiry":
        ...
        if (parameters.days !== undefined) {
          ...
          const cutoffDate = new Date();
          cutoffDate.setDate(
            cutoffDate.getDate() + parseInt(parameters.days) - maxPasswordAge,
          );
          const windowsFileTime = (
            (cutoffDate.getTime() + 11644473600000) *
            10000
      ...
      case "recent_password_changes":
        ...
        if (parameters.hours !== undefined) {
          const cutoffDate = new Date();
          cutoffDate.setHours(
            cutoffDate.getHours() - parseInt(parameters.hours),
          );
          const windowsFileTime = (
            (cutoffDate.getTime() + 11644473600000) *
            10000
    ...
    try {
      ...
      if (status === "completed" && execution.results !== undefined) {
        await db.query(
          ...
          [historyId, JSON.stringify(execution.results || [])],
        );
      }
      ...
      if (execution.credentialId) {
        logger.info(
          `Report executed with credential ${execution.credentialId} for user ${userId}`,
        );
      }
