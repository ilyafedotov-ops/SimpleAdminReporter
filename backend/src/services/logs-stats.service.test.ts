...
describe("LogsStatsService", () => {
  ...
  describe("getAuditStats", () => {
    ...
    it("should handle database errors", async () => {
      ...
      await expect((logsStatsService as any).getAuditStats(24)).rejects.toThrow(
        "Connection failed",
      );
    });
  });
  ...
  describe("getMetrics", () => {
    ...
    it("should handle database query errors", async () => {
      ...
      await expect(logsStatsService.getMetrics()).rejects.toThrow(
  ...
  describe("getCleanupStats", () => {
    ...
    it("should handle negative retention days gracefully", async () => {
      ...
      const timeDiff = Math.abs(
        result.cutoffDate.getTime() - futureDate.getTime(),
      );
    ...
    it("should handle database query errors during cleanup stats", async () => {
      ...
      await expect(logsStatsService.getCleanupStats(7)).rejects.toThrow(
        "Audit table error",
      );
    });
  ...
  describe("performCleanup", () => {
    ...
    it("should handle database errors during cleanup", async () => {
      ...
      await expect(logsStatsService.performCleanup(7)).rejects.toThrow(
        "System table cleanup failed",
      );
    });
    ...
    it("should use correct cutoff date calculation", async () => {
      ...
      expect(mockDbQuery.mock.calls[0][1]?.[0]).toEqual(
        mockDbQuery.mock.calls[1][1]?.[0],
      );
    });
  ...
  describe("Edge Cases and Error Scenarios", () => {
    ...
    it("should handle database connection timeout", async () => {
      ...
      await expect(logsStatsService.getLogStats()).rejects.toThrow(
        "Connection timeout",
      );
    });
  ...
  describe("Performance and Large Dataset Scenarios", () => {
    it("should handle large result sets within LIMIT constraints", async () => {
      ...
      const largeAuditStats = Array(20)
        .fill(null)
        .map((_, i) => ({
          event_type: `type_${i}`,
      ...
      const largeSystemStats = Array(20)
        .fill(null)
        .map((_, i) => ({
