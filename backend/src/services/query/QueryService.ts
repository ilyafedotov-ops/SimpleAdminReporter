...
export class QueryService {
  ...
  async executeQuery<T>(
    ...
    try {
      ...
      const validation = await this.validator.validateQuery(
        queryDef,
        parameters,
      );
      if (!validation.valid) {
      ...
      switch (queryDef.dataSource) {
        case "postgres":
          result = await this.executePostgresQuery(
            queryDef,
            processedParams,
            options,
          );
          break;
        case "ad":
          result = await this.executeADQuery(
            queryDef,
            processedParams,
            options,
          );
          break;
        case "azure":
          result = await this.executeAzureQuery(
            queryDef,
            processedParams,
            options,
          );
          break;
        case "o365":
          result = await this.executeO365Query(
            queryDef,
            processedParams,
            options,
          );
        ...
        default:
          throw createError(
            `Unsupported data source: ${queryDef.dataSource}`,
            400,
          );
      }
    ...
    try {
      ...
      if (options.timeout || queryDef.constraints?.timeoutMs) {
        const timeoutMs = Math.max(
          1,
          Math.min(
            Number(options.timeout || queryDef.constraints!.timeoutMs!),
            300_000,
          ),
        );
      ...
      // Report SQL definitions are validated by QueryValidator and constrained by the query allowlist.
      ...
      if (maxResults && data.length > maxResults) {
        logger.warn(
          `Query ${queryDef.id} returned ${data.length} rows, limiting to ${maxResults}`,
        );
  ...
  private async executeADQuery<T>(
    ...
    try {
      ...
      if (options?.credentialId && this.credentialManager) {
        ...
        const __credentials = await this.credentialManager.getCredentials(
          "ad",
          {
      ...
      try {
        ...
        throw new Error(
          `Invalid LDAP query configuration for query ${queryDef.id}. Expected JSON with type: 'ldap'`,
        );
      }
      ...
      Object.entries(parameterObject).forEach(([key, value]) => {
        ...
        if (typeof adQuery.filter === "string") {
          adQuery.filter = adQuery.filter.replace(
            new RegExp(`{{${key}}}`, "g"),
            String(value),
          );
        }
        ...
        if (typeof adQuery.baseDN === "string") {
          adQuery.baseDN = adQuery.baseDN.replace(
            new RegExp(`{{${key}}}`, "g"),
            String(value),
          );
        }
      ...
      if (
        typeof adQuery.baseDN === "string" &&
        adQuery.baseDN.includes("{{baseDN}}")
      ) {
        const systemBaseDN = process.env.AD_BASE_DN || "DC=domain,DC=local";
        adQuery.baseDN = adQuery.baseDN.replace(/{{baseDN}}/g, systemBaseDN);
        logger.debug(
          `Replaced {{baseDN}} with ${systemBaseDN} in query ${queryDef.id}`,
        );
      }

      // Also replace {{baseDN}} in filter if present
      if (
        typeof adQuery.filter === "string" &&
        adQuery.filter.includes("{{baseDN}}")
      ) {
        const systemBaseDN = process.env.AD_BASE_DN || "DC=domain,DC=local";
  ...
  private async executeAzureQuery<T>(
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Invalid Azure query configuration for query ${queryDef.id}. Expected JSON with type: 'azure'`,
        );
      }
      ...
      if (azureQuery.graphOptions.filter) {
        Object.entries(parameterObject).forEach(([key, value]) => {
          azureQuery.graphOptions.filter =
            azureQuery.graphOptions.filter.replace(
              new RegExp(`{{${key}}}`, "g"),
              String(value),
            );
        });
  ...
  private async executeO365Query<T>(
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Invalid O365 query configuration for query ${queryDef.id}. Expected JSON with type: 'o365'`,
        );
      }
  ...
  async getQueryStats(
    ...
    try {
      ...
      logger.debug(
        `Query statistics requested for ${queryId} - returning empty stats`,
      );
