...
export class QueryBuilder {
  ...
  select(fields: string | string[]): QueryBuilder {
    if (typeof fields === "string") {
      ...
      throw createError(
        "Select fields must be string or array of strings",
        400,
      );
    }
  ...
  join(
    ...
    this.joinClauses.push(
      `${type} JOIN ${this.escapeIdentifier(table)} ON ${onCondition}`,
    );
    return this;
  }
  ...
  groupBy(fields: string | string[]): QueryBuilder {
    if (typeof fields === "string") {
      ...
      throw createError(
        "GROUP BY fields must be string or array of strings",
        400,
      );
    }
  ...
  private buildSelectClause(): string {
    const fields = this.selectFields.map((field) => {
      ...
      if (this.isAggregateFunction(field) || this.isExpression(field)) {
  ...
  private buildWhereCondition(condition: WhereCondition): string {
    ...
    const isAggregateField = this.isAggregateFunction(condition.field);
    const field = isAggregateField
      ? condition.field
      ...
      return parts
        .map((part) => {
          ...
          if (!cleaned || cleaned.length === 0) {
            throw createError(
              "Invalid identifier part after sanitization",
              400,
            );
          }
          return `"${cleaned}"`;
   ...
   */
  private isAggregateFunction(field: string): boolean {
  ...
  private validateFieldName(field: string): void {
    ...
    // Check for SQL injection patterns FIRST — this must run even for the
    // aggregate/expression escape-hatch below, otherwise a crafted "expression"
    // could smuggle `; DROP`, `--`, or `/* */` past all validation.
    ...
    // Allow aggregate functions and expressions (still subject to the
    // dangerous-pattern check above; column allowlisting is enforced separately
    // via assertAllowedQueryColumns / query-allowlist.ts).
    if (this.isAggregateFunction(field) || this.isExpression(field)) {
      return;
    }
  ...
  static buildSelect(
    ...
    const builder = new QueryBuilder().select(fields).from(table);
