...
export class QueryValidator {
    ...
    try {
      ...
      if (queryDef.parameters) {
        if (Array.isArray(queryDef.parameters)) {
          ...
          this.validateLDAPStyleParameters(
            queryDef.parameters as any,
            parameters,
            errors,
            warnings,
          );
  ...
  private validateQueryDefinition(
    ...
    if (queryDef.id && !/^[a-z0-9_]+$/i.test(queryDef.id)) {
      errors.push(
        "Query ID must contain only alphanumeric characters and underscores",
      );
    }
    ...
    if (queryDef.constraints) {
      if (
        queryDef.constraints.maxResults &&
        queryDef.constraints.maxResults > 50000
      ) {
        warnings.push("MaxResults over 50,000 may cause performance issues");
      }
      if (
        queryDef.constraints.timeoutMs &&
        queryDef.constraints.timeoutMs > 300000
      ) {
        warnings.push(
          "Query timeout over 5 minutes may cause connection issues",
        );
      }
    }
  }
      ...
      if (paramDefs[key]) {
        ...
        if (paramDef.type && value !== null && value !== undefined) {
          ...
          if (expectedType === "number" && actualType !== "number") {
            errors.push(
              `Parameter ${key} should be a number, got ${actualType}`,
            );
          } else if (expectedType === "string" && actualType !== "string") {
            errors.push(
              `Parameter ${key} should be a string, got ${actualType}`,
            );
          } else if (expectedType === "boolean" && actualType !== "boolean") {
            errors.push(
              `Parameter ${key} should be a boolean, got ${actualType}`,
            );
          }
        }
      }
  ...
  private validateParameterRules(
    ...
    if (param.type === "string") {
      if (rules.min !== undefined && value.length < rules.min) {
        errors.push(
          `Parameter ${param.name} must be at least ${rules.min} characters`,
        );
      }
      if (rules.max !== undefined && value.length > rules.max) {
        errors.push(
          `Parameter ${param.name} must be at most ${rules.max} characters`,
        );
      }
    }

    // Pattern validation for strings. Only allow vetted validation regexes so
    // query definitions cannot inject expensive or unsafe expressions.
    if (param.type === "string" && rules.pattern) {
      const regex = this.getSafeValidationPattern(rules.pattern);
      if (!regex) {
        errors.push(
          `Parameter ${param.name} uses an unsupported validation pattern`,
        );
        return;
      }
      if (!regex.test(value)) {
    ...
    if (rules.enum && !rules.enum.includes(value)) {
      errors.push(
        `Parameter ${param.name} must be one of: ${rules.enum.join(", ")}`,
      );
    }
  }

  /**
   * Compile only the known-safe validation patterns used by bundled query definitions.
   */
  private getSafeValidationPattern(pattern: string): RegExp | null {
    switch (pattern) {
      case "^[a-zA-Z0-9._-]+$":
        return /^[a-zA-Z0-9._-]+$/;
      case "^[a-zA-Z\\s]+$":
  ...
  private validateSQLSecurity(
    ...
    for (const op of dangerousOperations) {
      if (upperSQL.includes(op)) {
        errors.push(
          `SQL contains potentially dangerous operation: ${op.trim()}`,
        );
      }
    }
    ...
    if (!paramPlaceholders || paramPlaceholders.length === 0) {
      warnings.push(
        "SQL does not use parameterized queries - potential security risk",
      );
    }
  }
  ...
  private validatePerformance(
    ...
    for (const table of largeTables) {
      if (sql.includes(`FROM ${table}`) && !sql.includes("WHERE")) {
        warnings.push(
          `Query on large table ${table} without WHERE clause may be slow`,
        );
      }
    }
    ...
    if (sql.includes("SELECT *")) {
      warnings.push(
        "SELECT * may retrieve unnecessary data and impact performance",
      );
    }
