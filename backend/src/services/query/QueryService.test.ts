...
describe("QueryService", () => {
  ...
  beforeEach(() => {
    ...
    (
      QueryValidator as jest.MockedClass<typeof QueryValidator>
    ).mockImplementation(
      () =>
        ({
          validateQuery: jest
    ...
    (
      ParameterProcessor as jest.MockedClass<typeof ParameterProcessor>
    ).mockImplementation(
      () =>
        ({
          processParameters: jest
            .fn()
            .mockImplementation((params: any[], values: any) =>
              params
                .map((p: any, _i: any) =>
                  values[p.name] !== undefined ? values[p.name] : null,
                )
                .filter((v: any) => v !== null),
            ),
        }) as any,
    ...
    (
      ResultTransformer as jest.MockedClass<typeof ResultTransformer>
    ).mockImplementation(
      () =>
        ({
  ...
  describe("executeQuery", () => {
    it("should execute a PostgreSQL query successfully", async () => {
      ...
      const queryDef = {
        ...
        parameters: [{ name: "id", type: "number" as const, required: true }],
  ...
  describe("getQueryStats", () => {
    it("should retrieve query statistics successfully", async () => {
      ...
      expect(result.note).toContain(
        "Direct query executions are not persisted",
      );
    });
  ...
  describe("clearCache", () => {
    it("should clear cache successfully", async () => {
      ...
      await expect(
        queryService.clearCache("test_query"),
