...
async function seedDatabase() {
  ...
  try {
    ...
    if (templateCount > 0) {
      logger.warn(
        `Database already contains ${templateCount} report templates. Skipping seed to avoid duplicates.`,
      );
      logger.info(
        "To force re-seeding, manually truncate the report_templates table first.",
      );
      return;
    }

    // Read the SQL seed file
    const seedFilePath = path.join(getDatabaseRoot(), "seed-templates.sql");
    ...
    const fieldMetadata = await db.query(
