...
router.get(
  ...
  param("source")
    .isIn(["ad"])
    .withMessage("Schema discovery currently only supports AD"),
  query("refresh")
    .optional()
    .isBoolean()
  ...
  query("credentialId")
    .optional()
    .isNumeric()
    .withMessage("Credential ID must be a number"),
...
router.get(
  ...
  query("includePublic")
    .optional()
    .isBoolean()
    .withMessage("includePublic must be boolean"),
  reportsController.getCustomReports,
);

// Debug endpoint removed for security - use admin logs API instead
...
router.post(
  ...
  body("query.fields")
    .isArray({ min: 1 })
    .withMessage("At least one field required"),
  body("parameters")
    .optional()
    .isObject()
    .withMessage("Parameters must be an object"),
...
queryRouter.post(
  ...
  requireAuth,
  validateRequest([
    body("queryDef").isObject().withMessage("Query definition is required"),
    body("queryDef.id")
      .isString()
      .notEmpty()
      .withMessage("Query ID is required"),
    body("queryDef.sql").isString().notEmpty().withMessage("SQL is required"),
    body("parameters")
      .optional()
      .isObject()
      .withMessage("Parameters must be an object"),
...
queryRouter.get(
  ...
  requireAuth,
  validateRequest([
    param("queryId").isString().notEmpty().withMessage("Query ID is required"),
    query("parameters")
      .optional()
      .isObject()
      .withMessage("Parameters must be an object"),
...
queryRouter.delete("/cache", requireAuth, queryController.clearCache);
...
queryRouter.get("/health", async (req, res) => {
  try {
    ...
    const overallHealthy = Object.values(healthCheck.services).some(
      (status) => status,
    );
...
queryRouter.get("/metrics", async (req, res) => {
  try {
    ...
    const [totalQueries, recentQueries, queryStats, errorStats] =
      await Promise.all([
    ...
    const metrics = {
      ...
      last24Hours: {
        ...
        averageExecutionTime: parseFloat(
          queryStats.rows[0]?.avg_execution_time || 0,
        ),
...
exportRouter.post(
  ...
  validateRequest([
    param("customTemplateId")
      .isUUID()
      .withMessage("Invalid custom template ID"),
    body("format")
      .optional()
      .isIn(["excel", "csv", "pdf"])
      .withMessage("Invalid format"),
...
exportRouter.get(
  ...
  validateRequest([
    param("filename")
      .matches(/^[a-zA-Z0-9_.-]+\.(xlsx|csv|pdf)$/)
...
exportRouter.get(
  "/job/:jobId",
  validateRequest([param("jobId").isNumeric().withMessage("Invalid job ID")]),
...
router.get("/favorites", requireAuth, reportsController.getFavorites);
