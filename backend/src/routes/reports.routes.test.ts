...
jest.mock("@/controllers/reports.controller", () => ({
  reportsController: {
    getTemplates: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { templates: [] } }),
    ),
    executeTemplate: jest.fn((req, res) =>
      res.status(200).json({ success: true, executionId: "test-execution-id" }),
    ),
    getFields: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { fields: [] } }),
    ),
    discoverSchema: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { schema: {} } }),
    ),
    createCustomReport: jest.fn((req, res) =>
      res.status(201).json({ success: true, reportId: "new-report-id" }),
    ),
    getCustomReports: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { reports: [] } }),
    ),
    getCustomReport: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { report: {} } }),
    ),
    updateCustomReport: jest.fn((req, res) =>
      res.status(200).json({ success: true, message: "Report updated" }),
    ),
    deleteCustomReport: jest.fn((req, res) =>
      res.status(200).json({ success: true, message: "Report deleted" }),
    ),
    executeCustomReport: jest.fn((req, res) =>
      res
        .status(200)
        .json({ success: true, executionId: "custom-execution-id" }),
    ),
    testCustomQuery: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { results: [] } }),
    ),
    getReportStats: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { stats: {} } }),
    ),
    getReportHistory: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { history: [] } }),
    ),
    getReportExecution: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { execution: {} } }),
    ),
    getReportResults: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { results: [] } }),
    ),
    getFavorites: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { favorites: [] } }),
    ),
    addToFavorites: jest.fn((req, res) =>
      res.status(200).json({ success: true, message: "Added to favorites" }),
    ),
    removeFromFavorites: jest.fn((req, res) =>
      res
        .status(200)
        .json({ success: true, message: "Removed from favorites" }),
    ),
    previewCustomReport: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { preview: [] } }),
    ),
    previewTemplate: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { preview: [] } }),
    ),
    deleteReportExecution: jest.fn((req, res) =>
      res.status(200).json({ success: true, message: "Deleted" }),
    ),
    bulkDeleteReportExecutions: jest.fn((req, res) =>
...
jest.mock("@/controllers/query.controller", () => ({
  queryController: {
    executeQuery: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { results: [] } }),
    ),
    buildAndExecuteQuery: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { results: [] } }),
    ),
    getQueryDefinitions: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { definitions: [] } }),
    ),
    getSchema: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { schema: {} } }),
    ),
    validateQuery: jest.fn((req, res) => res.status(200).json({ valid: true })),
    getCachedResult: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { cached: true } }),
    ),
    getQueryStats: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { stats: {} } }),
    ),
    clearCache: jest.fn((req, res) =>
      res.status(200).json({ success: true, message: "Cache cleared" }),
    ),
    executeGraphQuery: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { results: [] } }),
    ),
    getGraphQueryDefinitions: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { definitions: [] } }),
    ),
    getGraphQueryHistory: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { history: [] } }),
    ),
    executeGraphBatch: jest.fn((req, res) =>
      res.status(200).json({ success: true, data: { results: [] } }),
    ),
  },
...
jest.mock("@/controllers/export.controller", () => ({
  ExportController: jest.fn().mockImplementation(() => ({
    exportReport: jest.fn((req, res) =>
      res
        .status(200)
        .json({ success: true, downloadUrl: "http://test.com/download" }),
    ),
    queueExport: jest.fn((req, res) =>
      res.status(202).json({ success: true, jobId: "export-job-123" }),
    ),
    exportHistoryResults: jest.fn((req, res) =>
      res.status(200).json({
        success: true,
        downloadUrl: "http://test.com/history-download",
      }),
    ),
    downloadFile: jest.fn((req, res) => res.status(200).send("File content")),
    getJobStatus: jest.fn((req, res) =>
      res.status(200).json({ success: true, status: "completed" }),
    ),
    cleanupExports: jest.fn((req, res) =>
      res.status(200).json({ success: true, cleanedCount: 5 }),
    ),
  })),
}));
...
jest.mock("@/middleware/auth-wrapper", () => {
  ...
  return {
    requireAuth: jest.fn(
      (req: MockRequest, res: MockResponse, _next: MockNext) => {
...
jest.mock("@/middleware/validation.middleware", () => ({
  validateRequest: jest.fn(
    () => (req: MockRequest, res: MockResponse, next: MockNext) => next(),
  ),
...
jest.mock("express-validator", () => {
  ...
  const createValidatorChain = () => {
    const middleware = (req: MockRequest, res: MockResponse, next: MockNext) =>
      next();
...
describe("Reports Routes", () => {
  ...
  describe("Pre-built Report Templates", () => {
    ...
    describe("POST /api/reports/execute/:templateId", () => {
      ...
      it("should apply rate limiting to template execution", async () => {
        ...
        expect(mockAuthWrapper.userRateLimit).toHaveBeenCalledWith(
          RATE_LIMITS.TEMPLATE_EXECUTION,
        );
      });
      ...
      it("should audit template execution", async () => {
        ...
        expect(mockAuthWrapper.auditAction).toHaveBeenCalledWith(
  ...
  describe("Custom Reports", () => {
    ...
    describe("POST /api/reports/custom", () => {
      ...
      it("should apply rate limiting to custom report creation", async () => {
        ...
        expect(mockAuthWrapper.userRateLimit).toHaveBeenCalledWith(
          RATE_LIMITS.CUSTOM_REPORT_CREATION,
        );
      });
    ...
    describe("PUT /api/reports/custom/:reportId", () => {
      ...
      it("should require resource access for updates", async () => {
        ...
        expect(mockAuthWrapper.requireResourceAccess).toHaveBeenCalledWith(
          resourceCheckers.customReport,
        );
      });
  ...
  describe("Error Handling and Edge Cases", () => {
    it("should handle controller errors gracefully", async () => {
      (reportsController.getTemplates as jest.Mock).mockImplementationOnce(
        (_req: any, _res: any) => {
          throw new Error("Controller error");
        },
      );

      await request(app).get("/api/reports/templates").expect(500);
    });
    ...
    it("should handle invalid UUID parameters", async () => {
      await request(app).get("/api/reports/custom/invalid-uuid").expect(200); // Validation passes in mock
    });
  ...
  describe("Security and Access Control", () => {
    it("should apply authentication to protected routes", async () => {
      ...
      for (const route of protectedRoutes) {
        ...
        const method = route.method as keyof typeof req &
      ...
      expect(mockAuthWrapper.requireAuth).toHaveBeenCalledTimes(
    ...
    it("should apply admin authentication to admin routes", async () => {
      ...
      expect(mockAuthWrapper.requireAdmin).toHaveBeenCalledTimes(
        adminRoutes.length,
      );
    });
    ...
    it("should apply rate limiting to resource-intensive operations", async () => {
      ...
      expect(mockAuthWrapper.userRateLimit).toHaveBeenCalledTimes(
        rateLimitedRoutes.length,
      );
    });
    ...
    it("should audit sensitive operations", async () => {
      ...
      expect(mockAuthWrapper.auditAction).toHaveBeenCalledTimes(
        auditedRoutes.length,
      );
    });
  });
