...
jest.mock("@/middleware/rate-limit.middleware", () => ({
  ...
  authEndpointsRateLimiter: jest.fn((_req: any, _res: any, next: any) =>
    next(),
  ),
}));
...
describe("Auth Routes Integration", () => {
  ...
  describe("POST /api/auth/login", () => {
    it("should handle successful login", async () => {
      (unifiedAuthController.login as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
    ...
    it("should handle validation errors", async () => {
      ...
      const response = await request(app).post("/api/auth/login").send({}); // Missing required fields
  ...
  describe("POST /api/auth/refresh", () => {
    it("should refresh valid token", async () => {
      (unifiedAuthController.refresh as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
  ...
  describe("POST /api/auth/logout", () => {
    it("should logout successfully", async () => {
      (unifiedAuthController.logout as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
  ...
  describe("GET /api/auth/profile", () => {
    it("should return user profile", async () => {
      (unifiedAuthController.getProfile as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: mockUser,
  ...
  describe("PUT /api/auth/profile", () => {
    it("should update profile", async () => {
      ...
      (unifiedAuthController.updateProfile as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
  ...
  describe("GET /api/auth/verify", () => {
    it("should verify valid token", async () => {
      (unifiedAuthController.verifyToken as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
  ...
  describe("POST /api/auth/change-password", () => {
    it("should change password successfully", async () => {
      (unifiedAuthController.changePassword as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
  ...
  describe("POST /api/auth/create-user", () => {
    it("should create user (admin)", async () => {
      ...
      (unifiedAuthController.createUser as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(201).json({
            success: true,
            data: { id: 2, ...newUser },
  ...
  describe("GET /api/auth/test-connections", () => {
    it("should test authentication connections", async () => {
      ...
      (unifiedAuthController.testConnections as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: connections,
  ...
  describe("Azure AD Routes", () => {
    it("should get Azure config", async () => {
      ...
      (
        azureAuthController.getAzurePublicConfig as jest.Mock
  ...
  describe.skip("Azure AD legacy routes (removed)", () => {
    it("placeholder", () => {});
  });

  describe.skip("OAuth Routes (removed)", () => {
    it("placeholder", () => {});
  });
