...
jest.mock("@/middleware/auth-wrapper", () => ({
  ...
  requireRole: jest.fn(
    (roles: string[]) => (_req: any, _res: any, next: any) => {
...
describe("Logs Routes Integration", () => {
  ...
  beforeEach(() => {
    ...
    Object.values(logsController).forEach((handler) => {
      if (jest.isMockFunction(handler)) {
        (handler as jest.Mock).mockImplementation((_req: any, res: any) => {
  ...
  describe.skip("GET /api/logs/test (legacy route removed)", () => {
    it("should return database test results", async () => {
      ...
      const response = await request(app).get("/api/logs/test").expect(200);
    ...
    it("should handle database test errors", async () => {
      ...
      const response = await request(app).get("/api/logs/test").expect(500);
    ...
    it("should log request information", async () => {
      ...
      await request(app).get("/api/logs/test").expect(200);
  ...
  describe("GET /api/logs", () => {
    ...
    it("should handle logs retrieval errors", async () => {
      ...
      const response = await request(app).get("/api/logs").expect(500);
  ...
  describe("GET /api/logs/stats", () => {
    it("should get log statistics", async () => {
      (logsController.getLogStats as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: mockStats,
    ...
    it("should handle stats errors", async () => {
      ...
      const response = await request(app).get("/api/logs/stats").expect(500);
  ...
  describe("GET /api/logs/realtime", () => {
    it("should get real-time logs", async () => {
      ...
      const response = await request(app).get("/api/logs/realtime").expect(200);
  ...
  describe("GET /api/logs/export (Admin Only)", () => {
    it("should export logs for admin users", async () => {
      ...
      (logsController.exportLogs as jest.Mock).mockImplementation(
        (_req, res) => {
    ...
    it("should reject non-admin users from export", async () => {
      ...
      const response = await request(app).get("/api/logs/export").expect(403);
  ...
  describe("GET /api/logs/metrics (Admin Only)", () => {
    it("should get metrics for admin users", async () => {
      ...
      (logsController.getMetrics as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: mockMetrics,
    ...
    it("should reject non-admin users from metrics", async () => {
      ...
      const response = await request(app).get("/api/logs/metrics").expect(403);
  ...
  describe("GET /api/logs/metrics/queries (Admin Only)", () => {
    it("should get query performance metrics", async () => {
      ...
      (logsController.getQueryMetrics as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: queryMetrics,
  ...
  describe("GET /api/logs/metrics/queries/export (Admin Only)", () => {
    it("should export query metrics as CSV", async () => {
      ...
      const csvData =
        "timestamp,executionTime,queryType,cacheHit\n2025-01-01,150,audit_logs,true";

      (logsController.exportQueryMetrics as jest.Mock).mockImplementation(
        (_req, res) => {
          res.setHeader("Content-Type", "text/csv");
  ...
  describe("GET /api/logs/websocket/stats (Admin Only)", () => {
    it("should get WebSocket statistics", async () => {
      ...
      (logsController.getWebSocketStats as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: wsStats,
  ...
  describe("GET /api/logs/materialized-views/stats (Admin Only)", () => {
    it("should get materialized view statistics", async () => {
      ...
      (logsController.getMaterializedViewStats as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: mvStats,
  ...
  describe("POST /api/logs/materialized-views/refresh (Admin Only)", () => {
    it("should refresh materialized views", async () => {
      ...
      (logsController.refreshMaterializedViews as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
  ...
  describe("POST /api/logs/cleanup (Admin Only)", () => {
    it("should cleanup old logs", async () => {
      ...
      (logsController.cleanupOldLogs as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
    ...
    it("should validate cleanup parameters", async () => {
      ...
      await request(adminApp).post("/api/logs/cleanup").send({ days: -1 }); // Invalid parameter
  ...
  describe("GET /api/logs/search/fulltext", () => {
    it("should perform full-text search", async () => {
      ...
      (logsController.searchLogs as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: searchResults,
  ...
  describe("GET /api/logs/search/fuzzy", () => {
    it("should perform fuzzy search", async () => {
      ...
      (logsController.fuzzySearchLogs as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: fuzzyResults,
      ...
      const response = await request(app)
        .get(
          "/api/logs/search/fuzzy?type=audit&field=username&term=testusr&threshold=0.8",
        )
        .expect(200);
    ...
    it("should handle fuzzy search with different thresholds", async () => {
      ...
      const response = await request(app)
        .get(
          "/api/logs/search/fuzzy?type=audit&field=username&term=test&threshold=0.5",
        )
        .expect(200);
    ...
    it("should validate fuzzy search parameters", async () => {
      await request(app).get("/api/logs/search/fuzzy"); // Missing required parameters
  ...
  describe("GET /api/logs/:id", () => {
    it("should get specific log details", async () => {
      ...
      (logsController.getLogDetails as jest.Mock).mockImplementation(
        (_req, res) => {
          res.status(200).json({
            success: true,
            data: logDetails,
      ...
      const response = await request(app).get("/api/logs/1").expect(200);
    ...
    it("should handle non-existent log ID", async () => {
      ...
      const response = await request(app).get("/api/logs/99999").expect(404);

      expect(response.body.error).toBe("Log entry not found");
    });

    it("should validate log ID parameter", async () => {
      await request(app).get("/api/logs/invalid-id");
  ...
  describe("Authentication and Authorization", () => {
    it("should require authentication for all routes", async () => {
      ...
      const response = await request(noAuthApp).get("/api/logs").expect(401);
  ...
  describe("Rate Limiting", () => {
    it("should apply query rate limiting", async () => {
      ...
      const requests = Array(5)
        .fill(null)
        .map(() => request(app).get("/api/logs").expect(200));
    ...
    it("should apply export rate limiting", async () => {
      ...
      await request(adminApp).get("/api/logs/export").expect(200);
  ...
  describe("Input Validation", () => {
    it("should validate log query parameters", async () => {
      await request(app).get("/api/logs?invalidParam=value");

      // LogsValidator.validateLogQuery should have been called
    });

    it("should validate search parameters", async () => {
      await request(app).get("/api/logs/search/fulltext?invalid=true");
    ...
    it("should validate export parameters", async () => {
      ...
      await request(adminApp).get("/api/logs/export?format=invalid");
  ...
  describe("Error Handling", () => {
    it("should handle controller errors gracefully", async () => {
      (logsController.getLogs as jest.Mock).mockImplementation(
        (_req, _res, next) => {
    ...
    it.skip("should handle database connection errors (legacy /test route removed)", async () => {
    ...
    it("should handle validation errors", async () => {
      ...
      await request(app).get("/api/logs?invalid=param");
    });
  });
