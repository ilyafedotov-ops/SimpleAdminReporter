...
jest.mock("@/auth/controllers/unified-auth.controller", () => ({
  unifiedAuthController: {
    ...
    logoutAll: jest.fn((req, res) =>
      res
        .status(200)
        .json({ success: true, message: "All sessions logged out" }),
    ),
    getProfile: jest.fn((req, res) =>
      res
        .status(200)
        .json({ success: true, user: { id: 1, username: "testuser" } }),
    ),
    ...
    testConnections: jest.fn((req, res) =>
...
jest.mock("@/auth/controllers/azure-auth.controller", () => ({
  azureAuthController: {
    getAzurePublicConfig: jest.fn((req, res) =>
      res.status(200).json({ config: "test" }),
    ),
    generateAuthUrl: jest.fn((req, res) =>
      res.status(200).json({ authUrl: "https://login.microsoft.com" }),
    ),
    exchangeToken: jest.fn((req, res) =>
      res.status(200).json({ success: true, token: "test-token" }),
    ),
    storeToken: jest.fn((req, res) =>
      res.status(200).json({ success: true, message: "Token stored" }),
    ),
    getAzureUserInfo: jest.fn((req, res) =>
...
jest.mock("@/auth/controllers/azure-oauth-url.controller", () => ({
  azureOAuthURLController: {
    generateAuthUrl: jest.fn((req, res) =>
      res.status(200).json({ authUrl: "https://oauth.url" }),
    ),
  },
}));
...
jest.mock("@/auth/controllers/azure-oauth.controller", () => ({
  azureOAuthController: {
    authorize: jest.fn((req, res) =>
      res.redirect("https://microsoft.com/oauth"),
    ),
    callback: jest.fn((req, res) => res.status(200).json({ success: true })),
    checkStatus: jest.fn((req, res) =>
...
jest.mock("@/middleware/rate-limit.middleware", () => {
  ...
  return {
    ...
    authEndpointsRateLimiter: jest.fn((req: any, res: any, next: any) =>
      next(),
    ),
    // Expose the tracker for testing
    __executionTracker: executionTracker,
...
describe("Auth Routes", () => {
  ...
  describe("POST /api/auth/login", () => {
    ...
    it("should apply rate limiting to login endpoint", async () => {
      ...
      expect(
        rateLimitMiddleware.__executionTracker.loginRateLimiterCalls,
      ).toBeGreaterThan(0);
    });
    ...
    it("should audit login attempts", async () => {
      ...
      const loginAttemptCall = calls.find(
        (call: any) =>
    ...
    it("should handle login with missing credentials", async () => {
      await request(app).post("/api/auth/login").send({}).expect(200); // Validation mock passes everything
    });
  ...
  describe("POST /api/auth/logout-all", () => {
    ...
    it("should audit logout-all actions", async () => {
      ...
      // Check if the audit middleware was executed with expected parameters
      const calls = authWrapper.__executionTracker.auditActionCalls;
      const logoutAllCall = calls.find(
        (call: any) =>
          call.action === "logout_all_sessions" &&
          call.category === "authentication",
      );
  ...
  describe("PUT /api/auth/profile", () => {
    ...
    it("should audit profile updates", async () => {
      ...
      // Check if the audit middleware was executed with expected parameters
      const calls = authWrapper.__executionTracker.auditActionCalls;
      const profileUpdateCall = calls.find(
        (call: any) =>
          call.action === "update_profile" &&
          call.category === "user_management",
      );
  ...
  describe("GET /api/auth/verify", () => {
    ...
    it("should handle verification without token", async () => {
      const response = await request(app).get("/api/auth/verify").expect(200);
  ...
  describe("POST /api/auth/change-password", () => {
    ...
    it("should audit password changes", async () => {
      ...
      // Check if the audit middleware was executed with expected parameters
      const calls = authWrapper.__executionTracker.auditActionCalls;
      const passwordChangeCall = calls.find(
        (call: any) =>
  ...
  describe("GET /api/auth/test-connections", () => {
    ...
    it("should audit connection testing", async () => {
      ...
      // Check if the audit middleware was executed with expected parameters
      const calls = authWrapper.__executionTracker.auditActionCalls;
      const connectionTestCall = calls.find(
        (call: any) =>
          call.action === "test_auth_connections" &&
          call.category === "system_administration",
      );
  ...
  describe.skip("Azure OAuth Routes (legacy endpoints removed)", () => {
  ...
  describe("Security and Rate Limiting", () => {
    it("should apply rate limiting to sensitive endpoints", async () => {
      ...
      expect(authEndpointsRateLimiter).toHaveBeenCalledTimes(
        rateLimitedEndpoints.length,
      );
    });
  ...
  describe("Error Handling", () => {
    it("should handle controller errors gracefully", async () => {
      ...
      (unifiedAuthController.login as jest.Mock).mockImplementationOnce(
        (_req: any, _res: any) => {
    ...
    it("should handle validation errors", async () => {
      const { loginValidation } = require("@/validation/auth.validation");
      loginValidation.mockImplementationOnce(
  ...
  describe("Request Body Validation", () => {
    it("should handle large request bodies", async () => {
      ...
      await request(app).post("/api/auth/login").send(largePayload).expect(200);
    });
  ...
  describe("HTTP Methods and Headers", () => {
    it("should reject unsupported HTTP methods", async () => {
      await request(app).patch("/api/auth/login").expect(404);

      await request(app).delete("/api/auth/profile").expect(404);
    });
    ...
    it("should handle special characters in parameters", async () => {
      await request(app)
        ...
        .send({
          ...
          password: "complex-test-password",
        })
        .expect(200);
    });
  });
  ...
  describe("Integration with Authentication System", () => {
    it("should pass user context correctly through middleware", async () => {
      ...
      (unifiedAuthController.getProfile as jest.Mock).mockImplementationOnce(
        (req: any, res: any) => {
    ...
    it("should handle admin-only endpoints correctly", async () => {
      ...
      (
        unifiedAuthController.testConnections as jest.Mock
