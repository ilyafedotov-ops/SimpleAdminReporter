...
const loginValidation = [
  ...
  body("authSource")
    .optional()
    .isIn(["ad", "local"])
    .withMessage("Invalid auth source"),
];
...
router.get(
  "/csrf",
  authEndpointsRateLimiter,
  unifiedAuthController.getCSRFToken,
);
...
router.get(
  ...
  authEndpointsRateLimiter,
  requireAuth,
  azureAuthController.generateAuthUrl,
);
router.post(
  "/azure/token",
  authEndpointsRateLimiter,
  requireAuth,
  azureAuthController.exchangeToken,
);
router.post(
  "/azure/store-token",
  authEndpointsRateLimiter,
  requireAuth,
  azureAuthController.storeToken,
);
router.get(
  "/azure/userinfo",
  authEndpointsRateLimiter,
  requireAuth,
  azureAuthController.getAzureUserInfo,
);
