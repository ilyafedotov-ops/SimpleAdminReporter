  ...
  authorize = async (req: Request, res: Response) => {
    try {
      ...
      if (!userId || typeof userId !== "number") {
        throw createError(
          "User must be authenticated and have a valid ID to initiate OAuth flow",
          401,
        );
      }
  ...
  callback = async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        logger.error("Invalid userId in OAuth state:", {
          stateData,
          userIdValue,
        });
        throw createError(
          "User authentication required. Please ensure you are logged in before authorizing Azure AD.",
          401,
        );
      }
      ...
      try {
        ...
        // Encrypt tokens
        const encryptedAccessToken = await cryptoService.encryptToken(
