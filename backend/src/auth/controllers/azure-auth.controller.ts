...
export class AzureAuthController {
   ...
   */
  getAzurePublicConfig = asyncHandler(
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        authUrl.searchParams.append(
          "redirect_uri",
          `${process.env.BASE_URL || "http://localhost:5000"}/auth/azure/callback`,
        );
        ...
        logger.info(
          `Generated Azure auth URL for user ${req.user.username} with PKCE`,
        );
  ...
  exchangeToken = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        this.validateAzureIdToken(tokenData.id_token);

        // Clean up PKCE data
        await redisClient.del(pkceKey);

        // Encrypt and store tokens
        const encryptedAccess = await cryptoService.encryptToken(
          tokenData.access_token,
          req.user.id,
        );
        const encryptedRefresh = tokenData.refresh_token
          ? await cryptoService.encryptToken(
        ...
        await unifiedAuthService.storeAzureCredentials(
          req.user.id,
          {
        ...
        logger.info(
          `Azure tokens securely stored for user ${req.user.username}`,
        );
  ...
  private validateAzureIdToken(idToken?: string): void {
    if (!idToken) {
      throw createError("Azure ID token missing from token response", 400);
    }
    ...
    if (!payload || typeof payload !== "object") {
      throw createError("Invalid Azure ID token payload", 400);
    }

    const expectedTenant = process.env.AZURE_TENANT_ID;
    const expectedClient = process.env.AZURE_CLIENT_ID;
    const tenantId = String(payload.tid || "");
    const audience = String(payload.aud || "");
    const issuer = String(payload.iss || "");

    if (expectedTenant && tenantId !== expectedTenant) {
      throw createError("Azure token tenant mismatch", 403);
    }

    if (expectedClient && audience !== expectedClient) {
      throw createError("Azure token audience mismatch", 403);
    }

    if (expectedTenant && !issuer.includes(expectedTenant)) {
      throw createError("Azure token issuer mismatch", 403);
    }
  }
  ...
  storeToken = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        // Encrypt token
        const encryptedToken = await cryptoService.encryptToken(
          accessToken,
          req.user.id,
        );
        ...
        logger.info(
          `Azure token securely stored for user ${req.user.username}`,
        );
