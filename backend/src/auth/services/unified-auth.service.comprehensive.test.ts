...
describe("UnifiedAuthenticationService - Comprehensive Tests", () => {
  ...
  beforeEach(() => {
    ...
    (failedLoginTracker.recordFailedAttempt as jest.Mock).mockResolvedValue(
      undefined,
    );
    (failedLoginTracker.clearFailedAttempts as jest.Mock).mockResolvedValue(
      undefined,
    );
  ...
  describe("authenticate method", () => {
    describe("Happy Path Scenarios", () => {
      test("should successfully authenticate AD user", async () => {
        ...
        const mockServiceFactory =
          require("@/services/service.factory").serviceFactory;
        mockServiceFactory.getADService = jest
          .fn()
          .mockResolvedValue(mockADService);
      ...
      test.skip("should successfully authenticate Azure AD user", async () => {
        // Arrange
        const loginRequest: LoginRequest = {
        ...
        const mockServiceFactory =
          require("@/services/service.factory").serviceFactory;
        mockServiceFactory.getAzureService = jest
          .fn()
          .mockResolvedValue(mockAzureService);
        ...
        expect(mockAzureService.getUser).toHaveBeenCalledWith(
          "azureuser@company.com",
        );
      });

      test.skip("should successfully authenticate local user", async () => {
        // Arrange
        const loginRequest: LoginRequest = {
          username: "localuser",
      ...
      test.skip("should handle cookie mode authentication", async () => {
        // Arrange
        const loginRequest: LoginRequest = {
          username: "cookieuser",
    ...
    describe("Error Conditions", () => {
      test("should handle account lockout", async () => {
        ...
        // Act & Assert
        await expect(
          service.authenticate(loginRequest, mockRequest as Request),
        ).rejects.toThrow();
      ...
      test("should handle invalid credentials", async () => {
        ...
        jest
          .spyOn(serviceFactoryModule.serviceFactory, "getADService")
          .mockResolvedValue(mockADService);
      ...
      test("should handle service errors", async () => {
        ...
        const mockADService = {
          authenticateUser: jest
            .fn()
            .mockRejectedValue(new Error("LDAP connection failed")),
        };
    ...
    describe("Security Edge Cases", () => {
      ...
      test("should handle user enumeration attempts", async () => {
        ...
        await expect(
          service.authenticate(existingUserRequest, mockRequest as Request),
        ).rejects.toThrow("Invalid credentials");

        (db.query as jest.Mock).mockResolvedValue({ rows: [] });

        await expect(
          service.authenticate(nonExistingUserRequest, mockRequest as Request),
        ).rejects.toThrow("Invalid credentials");
  ...
  describe("refreshAccessToken method", () => {
    describe("Happy Path Scenarios", () => {
      test.skip("should successfully refresh valid JWT token", async () => {
        // Arrange
      ...
      test.skip("should handle token refresh in cookie mode", async () => {
        // Arrange
        const mockRefreshPayload: RefreshTokenPayload = {
          userId: 2,
    ...
    describe("Error Conditions", () => {
      test("should handle expired refresh token", async () => {
        ...
        // Act & Assert
        await expect(
          service.refreshAccessToken("expired-token", mockRequest as Request),
        ).rejects.toThrow();
      ...
      test("should handle invalid refresh token", async () => {
        ...
        // Act & Assert
        await expect(
          service.refreshAccessToken("invalid-token", mockRequest as Request),
        ).rejects.toThrow();
      ...
      test("should handle blacklisted refresh token", async () => {
        ...
        (tokenBlacklist.isTokenBlacklisted as jest.Mock).mockResolvedValue(
          true,
        );
      ...
      test("should handle token reuse attack", async () => {
        ...
        expect(redis.del).toHaveBeenCalledWith(
          "token_family:compromised-family",
        );
      });

      test.skip("should handle missing token family", async () => {
        // Arrange
        const mockPayload = {
          userId: 5,
        ...
        // Act & Assert
        await expect(
          service.refreshAccessToken("orphaned-token", mockRequest as Request),
        ).rejects.toThrow();
    ...
    describe("Security Edge Cases", () => {
      test("should prevent token family manipulation", async () => {
        ...
        // Act & Assert
        await expect(
          service.refreshAccessToken("malicious-token", mockRequest as Request),
        ).rejects.toThrow();
      });
    });
  });
  ...
  describe("verifyAccessToken method", () => {
    describe("Happy Path Scenarios", () => {
      test("should successfully verify valid JWT access token", async () => {
        ...
        expect(tokenBlacklist.isTokenBlacklisted).toHaveBeenCalledWith(
          "valid-access-token",
        );
      });
      ...
      test("should skip blacklist check when requested", async () => {
        ...
        // Act
        const result = await service.verifyAccessToken("skip-blacklist-token", {
    ...
    describe("Error Conditions", () => {
      test("should handle expired access token", async () => {
        ...
        // Act & Assert
        await expect(
          service.verifyAccessToken("expired-access-token"),
        ).rejects.toThrow();
      ...
      test("should handle invalid access token", async () => {
        ...
        // Act & Assert
        await expect(
          service.verifyAccessToken("invalid-access-token"),
        ).rejects.toThrow();
      ...
      test("should handle blacklisted access token", async () => {
        ...
        // Act & Assert
        await expect(
          service.verifyAccessToken("blacklisted-access-token"),
        ).rejects.toThrow();
      ...
      test("should handle session not found", async () => {
        ...
        // Act & Assert
        await expect(
          service.verifyAccessToken("no-session-token"),
        ).rejects.toThrow();
      ...
      test("should handle user not found", async () => {
        ...
        // Act & Assert
        await expect(
          service.verifyAccessToken("nonexistent-user-token"),
        ).rejects.toThrow();
  ...
  describe("logout method", () => {
    ...
    test("should handle logout service errors", async () => {
      ...
      // Act & Assert
      await expect(
        service.logout(sessionId, undefined, mockRequest as Request),
  ...
  describe("logoutAllSessions method", () => {
    ...
    test("should handle errors during logout all", async () => {
      ...
      // Act & Assert
      await expect(service.logoutAllSessions(userId)).rejects.toThrow(
        "Database error",
      );
    });
  });
  ...
  describe("IP Address Extraction", () => {
    test("should extract IP from X-Forwarded-For header", async () => {
      ...
      // Act
      await expect(
        service.authenticate(loginRequest, requestWithForwarded),
      ).rejects.toThrow();
    ...
    test("should extract IP from X-Real-IP header when X-Forwarded-For not available", async () => {
      ...
      // Act
      await expect(
        service.authenticate(loginRequest, requestWithRealIp),
      ).rejects.toThrow();
    ...
    test("should fallback to connection remote address", async () => {
      ...
      // Act
      await expect(
        service.authenticate(loginRequest, requestWithConnectionIp),
      ).rejects.toThrow();
  ...
  describe("testAuthConnections method", () => {
    test("should test all authentication connections", async () => {
      ...
      const mockO365Service = {
        testConnection: jest
          .fn()
          .mockRejectedValue(new Error("O365 unavailable")),
      };
      ...
      const { UnifiedAuthenticationService: FreshService } =
        await import("./unified-auth.service");
  ...
  describe("User Cache Management", () => {
    test("should cache user data for performance", async () => {
      ...
      await service.verifyAccessToken("cache-token-1", {
        skipBlacklistCheck: true,
      });

      // Act - Second call should use cache
      await service.verifyAccessToken("cache-token-2", {
        skipBlacklistCheck: true,
      });
