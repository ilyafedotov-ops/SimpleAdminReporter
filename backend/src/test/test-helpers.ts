...
/** Shared E2E/local test credentials with a valid bcrypt hash */
export const E2E_TEST_PASSWORD = "testpassword";
export const E2E_TEST_PASSWORD_HASH =
  "$2b$10$HNzAKQh51agjPNxM1VPOU.aKJpVHLEYdsX53FoKRjxzV8ypWRmtSK";
...
export async function initializeTestDatabase(): Promise<Pool> {
  ...
  // All retries failed
  logger.error(
    "Failed to connect to test database after all retries:",
    lastError,
  );
  try {
    await testPool.end();
...
export async function createTestSchema(pool: Pool): Promise<void> {
  ...
  try {
    ...
    const { getDatabaseRoot } = await import("@/utils/database-path");
    const schemaPath = path.join(getDatabaseRoot(), "init.sql");
...
export async function seedTestData(
  pool: Pool,
): Promise<{ userId: number; adminUserId: number }> {
  const client = await pool.connect();
  try {
    // Use serializable isolation to prevent deadlocks
    await client.query("BEGIN ISOLATION LEVEL SERIALIZABLE");

    // Create stable test users for E2E and integration tests
    const userResult = await client.query(
      `
      INSERT INTO users (username, email, display_name, auth_source, is_admin, is_active, password_hash, created_at)
      VALUES 
        ($1, $2, 'Test User', 'local', false, true, $3, NOW()),
        ($4, $5, 'Admin User', 'local', true, true, $3, NOW())
    ...
    const userId =
      userResult.rows.find((row) => row.username === "testuser")?.id ??
      userResult.rows[0].id;
    const adminUserId =
      userResult.rows.find((row) => row.username === "adminuser")?.id ??
      userResult.rows[1].id;
...
export async function cleanupTestDatabase(pool: Pool): Promise<void> {
  ...
  try {
    ...
    logger.info("Test database cleaned up", {
      ...
      tablesCleaned: tablesToClean.filter((t) => existingTables.includes(t))
        .length,
    });
  } catch (error) {
    await client.query("ROLLBACK");
...
export async function teardownTestContext(context: TestContext): Promise<void> {
  ...
  try {
    ...
    logger.warn(
      "Failed to cleanup test database during teardown:",
      error as Error,
    );
  }
...
export function createTestToken(
  ...
  const {
    createTestToken: unifiedCreateTestToken,
  } = require("@/auth/test-utils/auth-test.utils");
