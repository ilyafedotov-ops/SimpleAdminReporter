...
export class TestDataManager {
  ...
  async createTestDataset(): Promise<TestDataset> {
    ...
    try {
      ...
      const reportHistory = await this.createTestReportHistory(
        client,
        users,
        templates,
      );
  ...
  async cleanupTestDataset(_dataset?: TestDataset): Promise<void> {
    ...
    try {
      ...
      await client.query(
        `DELETE FROM notifications WHERE created_by IN (SELECT id FROM users WHERE username LIKE 'e2e_%')`,
      );
      await client.query(
        `DELETE FROM report_history WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'e2e_%')`,
      );
      await client.query(
        `DELETE FROM custom_report_templates WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'e2e_%')`,
      );
      await client.query(
        `DELETE FROM service_credentials WHERE user_id IN (SELECT id FROM users WHERE username LIKE 'e2e_%')`,
      );
      await client.query(
    ...
    const users: TestUser[] = [
      {
        ...
        isAdmin: false,
        isActive: true,
        passwordHash: E2E_TEST_PASSWORD_HASH,
      },
      {
    ...
    for (const cred of credentials) {
      ...
      `,
        [
          ...
          cred.tenantId || null,
          cred.clientId || null,
          cred.encryptedClientSecret || null,
          cred.server || null,
          cred.baseDN || null,
          cred.isActive,
          cred.isDefault,
   ...
   */
  private async createTestReportTemplates(
    client: any,
  ): Promise<TestReportTemplate[]> {
    const templates = [
      {
    ...
    for (const template of templates) {
      const queryConfig = JSON.parse(template.queryTemplate);
      const result = await client.query(
        `
        INSERT INTO report_templates (name, description, category, data_source, report_type, query_config, query_template, field_mappings, is_active, created_at)
        VALUES ($1, $2, $3, $4, $4, $5::jsonb, $5::jsonb, '{}'::jsonb, $6, NOW())
      ...
      `,
        [
          ...
          template.category,
          template.reportType,
          JSON.stringify({ ...queryConfig, reportType: template.reportType }),
          template.isActive,
        ],
      );
   ...
   */
  private async createTestReportHistory(
    client: any,
    users: TestUser[],
    templates: TestReportTemplate[],
  ): Promise<TestReportHistory[]> {
    const regularUser = users.find((u) => u.username === "e2e_regular_user")!;
    const adminUser = users.find((u) => u.username === "e2e_admin_user")!;
    ...
    if (!template1.id || !template2.id) {
      throw new Error(
        "Template IDs are required for creating test report history",
      );
    }
   ...
   */
  private async createTestAuditLogs(
    client: any,
    users: TestUser[],
    ...
    for (const log of systemLogs) {
      const result = await client.query(
        ...
        INSERT INTO system_logs (
          level, log_level, source, service, category, message, details, metadata, correlation_id, created_at, timestamp
        )
        VALUES ($1, $1, $2, $2, $3, $4, $5, $5, $6, $7, $7)
        ON CONFLICT DO NOTHING
        RETURNING id
      `,
        [
   ...
   * Create test custom reports
   */
  private async createTestCustomReports(
    client: any,
    users: TestUser[],
  ): Promise<TestCustomReport[]> {
    const regularUser = users.find((u) => u.username === "e2e_regular_user")!;
    const adminUser = users.find((u) => u.username === "e2e_admin_user")!;
    ...
    for (const report of customReports) {
      const result = await client.query(
        ...
        INSERT INTO custom_report_templates (
          user_id, created_by, name, description, category, source, query, is_active, is_public, created_at
        )
        VALUES ($1, $1, $2, $3, $4, $5, $6::jsonb, $7, $8, NOW())
        RETURNING id
      `,
        [
          report.userId,
          ...
          report.description,
          report.category,
          report.category === "security" ? "ad" : report.category,
  ...
  async verifyTestDataIntegrity(): Promise<TestDataIntegrity> {
    ...
    try {
      const results = await Promise.all([
        client.query(
          `SELECT COUNT(*) as count FROM users WHERE username LIKE 'e2e_%'`,
        ),
        client.query(
          `SELECT COUNT(*) as count FROM service_credentials WHERE credential_name LIKE 'E2E %'`,
        ),
        client.query(
          `SELECT COUNT(*) as count FROM report_templates WHERE name LIKE 'E2E %'`,
        ),
        client.query(
          `SELECT COUNT(*) as count FROM audit_logs WHERE correlation_id LIKE 'e2e-%'`,
        ),
        client.query(
          `SELECT COUNT(*) as count FROM system_logs WHERE correlation_id LIKE 'e2e-%'`,
        ),
        client.query(
          `SELECT COUNT(*) as count FROM custom_report_templates WHERE name LIKE 'E2E %'`,
        ),
      ]);
