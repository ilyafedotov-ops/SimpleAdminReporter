...
const corsOptions = {
  origin: (
    ...
    // Allow requests with no origin only outside production
    if (!origin) {
...
app.use(async (req, res, next) => {
  try {
    const { dbPoolMonitor, ensureDbCleanup } =
      await import("@/middleware/db-pool-monitor.middleware");
...
// Body parsing middleware - default limit with stricter per-route overrides elsewhere
app.use(express.json({ limit: process.env.REQUEST_BODY_LIMIT || "5mb" }));
app.use(
  express.urlencoded({
    extended: true,
    limit: process.env.REQUEST_BODY_LIMIT || "5mb",
  }),
);
...
// CSRF protection is mounted unconditionally after cookie parsing so every
// cookie-authenticated, state-changing route is protected. The middleware
// itself skips bearer-token clients, safe methods, and auth bootstrap routes.
...
process.on("unhandledRejection", (reason, promise) => {
  ...
  if (reason && typeof reason === "object" && "code" in reason) {
    ...
    if (
      ["ECONNRESET", "ECONNREFUSED", "ETIMEDOUT", "EHOSTUNREACH"].includes(
        errorCode,
      )
    ) {
  ...
  if (
    reasonStr.includes("CRITICAL") ||
    reasonStr.includes("DATABASE") ||
    reasonStr.includes("REDIS")
  ) {
    logger.error(
      "Critical system error detected, initiating graceful shutdown",
    );
  ...
  try {
    ...
    try {
      ...
      if (configResult.errors.length > 0) {
        ...
        logger.warn(
          "Continuing with degraded functionality due to configuration issues",
        );
      } else {
      ...
      logger.warn(
        "Continuing without configuration service - using environment variables directly",
      );
    }
    ...
    server.listen(PORT, () => {
      ...
      logger.info(
        `WebSocket support enabled at ws://localhost:${PORT}/socket/logs`,
      );
    });
  } catch (_error) {
