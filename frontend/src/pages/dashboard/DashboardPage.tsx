...
import {
  ...
  selectReports,
  selectFavoriteReports,
  fetchFavoriteReportsAsync,
...
const StatCard: React.FC<StatCardProps> = React.memo(
  ({ target, title, subtitle, icon: Icon, color, bgGlow, darkMode }) => {
...
const DashboardPage: React.FC = () => {
  ...
  const reportCategories = reportStats?.reportsBySource
    ? Object.entries(reportStats.reportsBySource).map(
        ([name, value], index) => ({
          name:
            name === "ad"
              ? "Active Directory"
  ...
  const reportTemplates = React.useMemo(() => {
    ...
    return templates.map((template: any) => ({
      ...
      avgTime: template.avgExecutionTime
        ? `${Math.round(template.avgExecutionTime / 1000)}s`
        : "N/A",
    }));
  }, [templates]);
  ...
  const scheduledReports = [
    {
      ...
      template: "Inactive Users Report",
    ...
    },
    {
      ...
      template: "License Usage Analysis",
    ...
    },
    {
      ...
      template: "Security Permissions Audit",
  ...
  const handleGenerateReport = (template: any) => {
    ...
    const queryDef: QueryDefinition = {
      ...
      dataSource:
        template.dataSource ||
        (template.category?.toLowerCase() === "ad"
          ? "ad"
          : template.category?.toLowerCase() === "azure"
            ? "azure"
            : template.category?.toLowerCase() === "o365"
              ? "o365"
              : template.category?.toLowerCase() === "users"
                ? "ad"
                : template.category?.toLowerCase() === "computers"
                  ? "ad"
                  : template.category?.toLowerCase() === "groups"
                    ? "ad"
                    : "ad"),
      ...
      >
        <div>
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            >
              Generate comprehensive reports for Active Directory, Azure AD, and
              Office 365
            </p>
            ...
            {hasErrors && (
              ...
              >
                <strong>Connection Error:</strong> Unable to load some data from
                the backend. Please check your connection.
              </div>
            )}
          </div>
          ...
          >
            ...
            >
              ...
              {reportStats?.recentExecutions &&
              reportStats.recentExecutions.length > 0 ? (
                          ...
                          >
                            ...
                            {execution.executionTimeMs && (
                              <>
                                <span>•</span>
                                <span>
                                  {(execution.executionTimeMs / 1000).toFixed(
                                    1,
                                  )}
                                  s
                                </span>
          ...
          <div>
            ...
            >
              {favoritesLoading ? (
                ...
                >
                  You haven't marked any reports as favorites yet. Star your
                  frequently used reports to see them here.
                </div>
              ) : (
                favoriteReports.slice(0, 6).map((template) => {
                  const IconComponent = getIconForCategory(
                    template.category || template.dataSource || "general",
                  );
                  return (
                    <div
                      key={template.id}
