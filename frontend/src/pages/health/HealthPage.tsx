...
const HealthPage: React.FC = () => {
  ...
  const [systemHealth, setSystemHealth] = useState<OverallHealthStatus | null>(
    null,
  );
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <InfoCircleOutlined style={{ color: "#8c8c8c", fontSize: "24px" }} />
        );
    }
  };
  ...
  const renderServiceCard = (
    ...
    >
      <div style={{ textAlign: "center" }}>
        ...
        <Title
          level={5}
          style={{ margin: 0, color: darkMode ? "#f3f4f6" : "#1f2937" }}
        >
          {title}
        </Title>
          ...
          <Col span={8}>
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ marginTop: "8px" }}>
                <Text type="secondary">
                  {system.memory?.free
                    ? `${(system.memory.free / 1024 / 1024 / 1024).toFixed(1)}GB free`
                    : "N/A"}
                </Text>
              </div>
            </div>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <Progress
                ...
                strokeColor={{
                  "0%": "#fa8c16",
                  "100%":
                    (system.disk?.percentage || 0) > 80 ? "#ff4d4f" : "#ffa940",
                }}
                format={(percent) => `${percent}%`}
              />
              <div style={{ marginTop: "8px" }}>
                <Text type="secondary">
                  {system.disk?.free
                    ? `${(system.disk.free / 1024 / 1024 / 1024).toFixed(1)}GB free`
                    : "N/A"}
                </Text>
              </div>
            </div>
  ...
  const renderHealthSummary = () => {
    ...
    const healthPercentage =
      totalChecks > 0 ? (healthyCount / totalChecks) * 100 : 0;
            ...
            >
              <List
                ...
                dataSource={[
                  {
                    label: "Export Path",
                    value: systemHealth.checks.storage.details.path,
                  },
                  {
                    label: "Files Count",
                    value: systemHealth.checks.storage.details.fileCount,
                  },
                  {
                    label: "Total Size",
                    value: systemHealth.checks.storage.details.totalSize,
                  },
                ]}
                renderItem={(item) => (
                  <List.Item>
