...
const LogsPage: React.FC = React.memo(() => {
  ...
  const {
    ...
    logTypes: filters.activeTab === "all" ? ["combined"] : [filters.activeTab],
    filters: {
  ...
  // Audit log columns
  const auditColumns = useMemo<any[]>(
    () => [
      {
      ...
      },
      {
        ...
        render: (type: string) => {
          ...
          const IconComponent =
            iconMap[
              logsService.getLogIcon({
                event_type: type,
              } as AuditLog) as keyof typeof iconMap
            ] || FileText;
          return (
      ...
      },
      {
        ...
        render: (username: string, record: AuditLog) =>
          username || `User ${(record as any).user_id}` || "System",
      },
      {
  ...
  // System log columns
  const systemColumns = useMemo<any[]>(
    () => [
      {
        dataIndex: "timestamp",
        title: "Timestamp",
        ...
        render: (level: string) => {
          ...
          const IconComponent =
            iconMap[
              logsService.getLogIcon({
                level,
              } as SystemLog) as keyof typeof iconMap
            ] || FileText;
          return (
            <Tag color={color} icon={<IconComponent size={14} />}>
              {level.toUpperCase()}
          ...
          return (
            <div>
              ...
              {(record as any).status_code && (
                <Tag
                  color={
                    (record as any).status_code >= 400 ? "error" : "success"
                  }
                >
                  {(record as any).status_code}
                </Tag>
  ...
  const combinedColumns = useMemo<any[]>(
    () => [
      {
      ...
      },
      {
        ...
        render: (_: unknown, record: Record<string, unknown>) => {
          if ((record as any)._type === "audit") {
            const color = logsService.getEventTypeColor(
              (record as any).event_type,
            );
            return (
              <Tag color={color}>
                {(record as any).event_type.toUpperCase()}
              </Tag>
            );
          } else {
            const color = logsService.getLogLevelColor((record as any).level);
            return (
              <Tag color={color}>{(record as any).level.toUpperCase()}</Tag>
            );
          }
        },
      },
      {
        ...
        render: (_: unknown, record: Record<string, unknown>) => {
          if ((record as any)._type === "audit") {
            return (record as any).event_action
              .replace(/_/g, " ")
              .toUpperCase();
          } else {
    ...
    >
            ...
            {realtimeLogs.length === 0 ? (
              ...
              realtimeLogs.map((log, index) => (
                ...
                >
                  ...
                  >
                    <Tag
                      color={log.log_type === "audit" ? "blue" : "green"}
                      style={{ fontSize: "11px" }}
                    >
      ...
      >
        ...
        >
          ...
          <Button icon={<Filter size={16} />} onClick={handleRefresh}>
            Apply Filters
          </Button>
            ...
            items={[
              {
                ...
                children: (
                  <EnhancedDataTable
                    data={combinedData as Record<string, unknown>[]}
                    ...
                    onExport={async (_rows, format) => {
                      await handleExport(format === "excel" ? "csv" : format);
                    }}
                  />
                ),
              },
              ...
              },
              {
                ...
                children: (
                  <EnhancedDataTable
                    data={systemLogs as unknown as Record<string, unknown>[]}
      ...
      >
        ...
        >
          ...
          >
            ...
            <h4 style={{ margin: 0, color: darkMode ? "white" : "#1f2937" }}>
              Total Logs
            </h4>
          </div>
          <p
