...
const ReportsPageV2: React.FC = () => {
    ...
    if (storedResults) {
      ...
      if (parsedResults.results && parsedResults.queryName) {
        ...
        const reportResult: ReportResult = {
          ...
          reportName: parsedResults.queryName,
          source: parsedResults.results?.metadata?.dataSource || "unknown",
          executedAt: parsedResults.executedAt || new Date().toISOString(),
          rowCount: parsedResults.results?.data?.length || 0,
          executionTimeMs: parsedResults.results?.metadata?.executionTime || 0,
          columns: parsedResults.results?.data?.[0]
            ? Object.keys(parsedResults.results.data[0])
            : [],
          metadata: {
            templateName: parsedResults.queryName,
            dataSource:
              parsedResults.results?.metadata?.dataSource || "unknown",
  ...
  useEffect(() => {
    if (
      location.state &&
      (location.state as any).selectedTemplateId &&
  ...
  const handleExecuteReport = async (
    ...
    try {
      const queryDef =
        selectedTemplate || definitions.find((def) => def.id === queryId);
      if (!queryDef) {
      ...
      if (result) {
        const executionResult = result as unknown as QueryExecutionResult;

        // Store the execution ID for export purposes
        setLastExecutionId(
          executionResult.executionId || executionResult.id || null,
        );

        // Transform execution result to ReportResult format for ReportDataTable
        const reportResult: ReportResult = {
          id: executionResult.id || "",
          reportName: queryDef.name,
          source: queryDef.dataSource || "unknown",
          executedAt: executionResult.executedAt || new Date().toISOString(),
          rowCount: executionResult.result?.data?.length || 0,
          executionTimeMs: executionResult.result?.metadata?.executionTime || 0,
          columns: executionResult.result?.data?.[0]
            ? Object.keys(executionResult.result.data[0])
            : [],
          metadata: {
            ...
            rowCount: executionResult.result?.data?.length || 0,
            cached: executionResult.result?.metadata?.cached || false,
          },
      ...
      message.error(
        `Failed to execute report: ${(error as any)?.message || String(error)}`,
      );
    } finally {
      ...
      >
        ...
        >
          <div>
            ...
            {reportResults && (
              ...
              >
                {reportResults.metadata?.templateName} - Executed{" "}
                {formatDate(reportResults.executedAt)}
              </p>
            )}
          </div>
          <button
        ...
        >
          ...
          {statsLoading ? (
            ...
            dashboardStats.recentExecutions.length > 0 ? (
            <div
                ...
                .map((execution, index) => (
                  ...
                  >
                    <div>
                      ...
                      >
                        {execution.reportCategory} •{" "}
                        {formatDate(execution.generatedAt)}
                      </div>
                    </div>
                    <div
                      style={{
            ...
            >
              ...
              />
              <p style={{ fontSize: "16px", margin: 0 }}>
                No recent report executions found
              </p>
            </div>
          )}
        </div>
