...
import { ReportExecution, ReportResult } from "@/types";
...
const ReportHistoryPage: React.FC = () => {
      ...
      try {
        ...
        if (execResponse.success && execResponse.data) {
          ...
          const resultsResponse =
            await reportsService.getReportResults(reportId);
          if (resultsResponse.success && resultsResponse.data) {
            setReportResults({
              ...
              reportName:
                execResponse.data.report_name ||
                execResponse.data.template_name ||
                "Report",
              executedAt:
                execResponse.data.executed_at || execResponse.data.executedAt,
    ...
    const fetchHistory = async () => {
      ...
      try {
        const response = await reportsService.getReportHistory({
          ...
          source:
            selectedFormat !== "all" ? (selectedFormat as any) : undefined,
        });
  ...
  const filteredExecutions = allExecutions.filter((execution) => {
    ...
    if (selectedPeriod !== "all") {
      ...
      const executionDate = execution.generated_at
        ? new Date(execution.generated_at)
  ...
  const handleBulkDownload = async () => {
    try {
      ...
      if (successCount > 0) {
        message.success(
          `Downloaded ${successCount} reports successfully${skippedCount > 0 ? ` (${skippedCount} skipped)` : ""}`,
        );
      }
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to delete ${selectedRows.length} selected reports? This action cannot be undone.`,
    );
    ...
    try {
      ...
      message.success(
        `Successfully deleted ${result.data?.deleted || selectedRows.length} reports`,
      );
  ...
  const handleDownload = async (
    ...
    try {
      ...
      message.error(
        `Failed to ${format === "excel" ? "export" : "download"} report`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const tableData = useMemo(() => {
    return filteredExecutions.map((execution) => ({
      ...
      reportName:
        ...
        execution.custom_template_name ||
        definitions.find((d) => d.id === execution.queryId)?.name ||
        execution.report_name ||
        execution.queryId ||
        "Unknown Report",
      template:
        execution.template_category ||
        execution.custom_template_source ||
        definitions.find((d) => d.id === execution.queryId)?.category ||
        "Query",
      generatedAt:
      ...
      format: (
        execution.result?.metadata.dataSource ||
        execution.format ||
        "JSON"
      ).toUpperCase(),
      rowCount:
        execution.result?.metadata.rowCount || execution.result_count || 0,
      executionTime:
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {new Date(
                      reportResults.execution.generatedAt ||
                        reportResults.execution.executedAt,
                    ).toLocaleString()}
                  </span>
                </div>
              </div>
              ...
              >
                <button
                  onClick={() =>
                    handleShare(reportResults.execution?.id || reportId)
                  }
                  style={{
                    display: "inline-flex",
                    alignItems: "center",
            ...
            {reportResults.message && reportResults.results.length === 0 ? (
              <>
                ...
                >
                  ...
                  >
                    The report was executed successfully but the detailed
              ...
              <ReportViewer
                ...
                data={{
                  ...
                  resultCount:
                    reportResults.resultCount || reportResults.results.length,
                  executionTime: reportResults.execution?.executionTimeMs,
                ...
                onDownload={(format) => {
                  ...
                  if (executionId) {
                    if (format === "csv") {
                      handleDownload(
                        { id: executionId, results: reportResults.results },
                        "csv",
                      );
                ...
                onCopyId={() =>
                  handleCopyId(reportResults.execution?.id || reportId)
                }
              />
            ) : (
              <>
                  ...
                  >
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={
                          selectedRows.length === paginatedExecutions.length &&
              ...
              {false ? (
                ...
                <EnhancedDataTable
                  ...
                  formatCellValue={(value, key, record) => {
                    ...
                    return defaultFormatCellValue(value, key);
                  }}
                  enableRowSelection={true}
                  onRowSelect={(selectedRows) => {
                    setSelectedRows(selectedRows.map((row) => String(row.id)));
                  }}
                  showExport={false}
                ...
                <button
                  ...
                  onClick={async () => {
                    if (selectedRows.length > 0) {
                      ...
                      setLoading(true);
                      for (const id of selectedRows) {
                        const execution = databaseHistory.find(
                          (e) => e.id === id,
                        );
                        if (execution) {
                      ...
                      setLoading(false);
                      message.success(
                        `Exported ${selectedRows.length} reports to Excel`,
                      );
                    } else {
                ...
                <button
                  ...
                  onClick={async () => {
                    if (selectedRows.length > 0) {
                      ...
                      setLoading(false);
                      message.success(
                        `Exported ${selectedRows.length} reports to CSV`,
                      );
                    } else {
                      message.warning("Please select reports to export");
                    }
