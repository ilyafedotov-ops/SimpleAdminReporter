...
const ReportBuilderPage: React.FC = () => {
  ...
  const { executeDynamic: executeDynamicQuery, executionHistory } =
    useQueryExecution();
  const { availableFields, fieldsLoading } = useAppSelector(
    (state) => state.reports,
  );
      ...
      reportsService
        ...
        .then((response) => {
          if (response.success && (response as any).data) {
            ...
            setSelectedSource(
              report.source as "ad" | "azure" | "o365" | "postgres",
            );
            ...
            const dynamicQuery: DynamicQuerySpec = {
              ...
              from: report.source,
              where:
                report.query.filters?.map((f: ReportFilter) => ({
                  field: f.field,
                  operator: f.operator,
                  value: f.value,
                })) || [],
              groupBy: report.query.groupBy ? [report.query.groupBy] : [],
              orderBy: report.query.orderBy?.[0],
  ...
  useEffect(() => {
    ...
    if (selectedSource && selectedSource !== "postgres") {
      const hasFields =
        availableFields[selectedSource] &&
        availableFields[selectedSource].length > 0;
      ...
      if (!hasFields && !loadingInitiated && !fieldsLoading) {
        ...
        dispatch(
          fetchAvailableFieldsAsync(selectedSource as "ad" | "azure" | "o365"),
        );
      }
    }
  ...
  const handleSaveQuery = async (
    ...
    try {
      ...
      if (query.dataSource === "postgres") {
        message.warning(
          "PostgreSQL queries cannot be saved as custom reports at this time",
        );
        setSavedQuery(query);
        return;
      }
      ...
      const customQuery: CustomReportQuery = {
        fields: query.select.map((fieldName) => {
          const field = availableFields[query.dataSource]?.find(
      ...
      let response;
      if (isEditing && reportId && editingReport) {
      ...
      if (response.success) {
        ...
        message.error(
          "Failed to save query: " + (response.error || "Unknown error"),
        );
      }
    } catch (error: unknown) {
      message.error(
  ...
  const handleExecuteQuery = async (
    ...
    if (query.dataSource !== "postgres") {
      ...
      if (response.success && response.data) {
        ...
        const errorMessage =
          response.error?.message || "Query execution failed";
      ...
      >
        <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          <div style={{ marginBottom: 32 }}>
            <Title
              level={2}
              style={{ color: darkMode ? "white" : "#1f2937", margin: 0 }}
            >
              {isEditing
                ? `Edit Report: ${editingReport?.name || "Loading..."}`
            ...
            >
              {isEditing
                ? "Modify your custom report query"
                : "Build custom queries with visual tools or SQL-like syntax"}
            </Text>
          </div>
          ...
          >
            <Title
              level={4}
              style={{ color: darkMode ? "white" : "#1f2937", marginBottom: 8 }}
            >
                ...
                return (
                  <Col key={source} xs={12} sm={12} md={6} lg={6}>
                    <Card
                      ...
                      onClick={() =>
                        handleSourceChange(
                          source as "ad" | "azure" | "o365" | "postgres",
                        )
                      }
                      style={{
                        height: "100%",
                              ...
                              try {
                                ...
                                if (result) {
                                  const rowCount =
                                    ...
                                    (result as any).data?.rowCount ??
                                    0;
                                  message.success(
                                    `Query executed successfully! Found ${rowCount} records.`,
                                  );
                      ...
                      <Col>
                        <Space>
                          {exec.status === "completed" && exec.result && (
                            <Text>
                              {exec.result.result.metadata.rowCount} rows
                            </Text>
                          )}
                          <Text
                            type={
