...
type LegacyReportTemplate = ReportTemplate & {
  query_template?: string | Record<string, unknown>;
  required_parameters?: string | string[];
  report_type?: string;
...
const TemplateGalleryPage: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(
      setCurrentPage({ page: "template-gallery", title: "Template Gallery" }),
    );
    dispatch(
      setBreadcrumbs([
  ...
  const getServiceTypeFromCategory = (
    category: string,
  ): "ad" | "azure" | "o365" | undefined => {
    const categoryLower = category.toLowerCase();
    if (categoryLower === "ad") return "ad";
    if (categoryLower === "azuread" || categoryLower === "azure")
      return "azure";
  ...
  const templateToQueryDefinition = (
    template: ReportTemplate,
  ): QueryDefinition => {
    console.log("DEBUG: Converting template to QueryDefinition", template);
    try {
      const legacyTemplate = template as LegacyReportTemplate;
      const dataSource =
        template.category === "AD"
          ? "ad"
          : template.category === "AzureAD"
            ? "azure"
            : template.category === "O365"
              ? "o365"
              : "ad";

      const queryTemplate =
        typeof legacyTemplate.query_template === "string"
          ? JSON.parse(legacyTemplate.query_template)
          : legacyTemplate.query_template || template.queryTemplate || {};

      const requiredParams =
        typeof legacyTemplate.required_parameters === "string"
          ? JSON.parse(legacyTemplate.required_parameters)
          : legacyTemplate.required_parameters ||
      ...
      return {
        id: legacyTemplate.report_type || template.reportType || template.id,
        name: template.name,
        description: template.description || "",
        category: template.category,
        dataSource,
        parameters: Object.entries(parameters).map(([name, param]) => ({
          name,
  ...
  const customReportToQueryDefinition = (
  ...
  const renderPrebuiltTemplates = () => {
    ...
    const groupedTemplates = templates.reduce(
      (acc: Record<string, ReportTemplate[]>, template) => {
                      ...
                      <Card.Meta
                        ...
                        description={
                          <div>
                            ...
                            {(template as LegacyReportTemplate)
  ...
  const renderCustomTemplates = () => {
    ...
    if (customReportsError) {
      return (
        <Card>
          <Text type="danger">
            Error loading custom templates: {customReportsError}
          </Text>
        </Card>
      );
    }
  ...
  return (
    <div className="page-container">
      ...
      <Text type="secondary" style={{ marginBottom: 24, display: "block" }}>
        Browse and use pre-built report templates and community-shared custom
        reports
      </Text>
