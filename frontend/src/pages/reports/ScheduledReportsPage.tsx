...
const ScheduledReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(
      setCurrentPage({ page: "scheduled-reports", title: "Scheduled Reports" }),
    );
    dispatch(
      setBreadcrumbs([
  ...
  const fetchTemplates = async () => {
    try {
      const [templatesRes, customRes] = await Promise.all([
        ...
        reportsService.getCustomReports({ isPublic: true }),
      ]);
        ...
        onOk: async () => {
          try {
            ...
            if (
              ...
              ((error as any)?.message || String(error)).includes("404")
            ) {
  ...
  const handleViewHistory = async (scheduleId: string) => {
    ...
    try {
      const response =
        await scheduledReportsService.getScheduleHistory(scheduleId);
      if (response.success && (response as any).data) {
  ...
  const handleRunNow = async (schedule: ReportSchedule) => {
    ...
    try {
      ...
      if (schedule.template_id) {
        ...
        await reportsService.executeCustomReport(
          schedule.custom_template_id,
          schedule.parameters || {},
        );
      }
    ...
    try {
      ...
      if (editingSchedule) {
        const updateData: UpdateScheduleDto = {
          ...
          recipients:
            values.recipients && values.recipients.trim().length > 0
              ? values.recipients
                  .split(",")
                  .map((e: string) => e.trim())
                  .filter(Boolean)
        ...
        const response = await scheduledReportsService.updateSchedule(
          editingSchedule.id,
          updateData,
        );
        ...
        const createResponse =
          await scheduledReportsService.createSchedule(createData);
  ...
  const getStatusColor = (schedule: ReportSchedule) => {
    ...
    if (
      schedule.last_run &&
      new Date(schedule.last_run) > new Date(Date.now() - 24 * 60 * 60 * 1000)
    ) {
      return "#10b981";
    }
    return "#3b82f6";
  ...
  const getStatusIcon = (schedule: ReportSchedule) => {
    if (!schedule.is_active)
      return <Power style={{ width: "16px", height: "16px" }} />;
  ...
  return (
    ...
    >
      <div
        style={{ padding: "24px", position: "relative" as const, zIndex: 1 }}
      >
        {/* Header */}
          ...
          >
            {loading ? (
              ...
              >
                ...
                >
                  Click "Create Schedule" above to automate your report
                  generation
                </p>
              </div>
            ) : (
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {scheduledReportsService.getScheduleDescription(
                        ...
                        {schedule.next_run && (
                          ...
                          >
                            ...
                            >
                              Next:{" "}
                              {scheduledReportsService.getNextRunDescription(
                                schedule.next_run,
                              )}
                            </span>
                          </div>
                    ...
                    <div style={{ position: "relative" as const }}>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setActionMenuOpen(
                            actionMenuOpen === schedule.id ? null : schedule.id,
                          );
                        }}
                        style={{
                          padding: "8px",
                    ...
                    return (
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: "Please select a custom report",
                          },
                        ]}
                      >
