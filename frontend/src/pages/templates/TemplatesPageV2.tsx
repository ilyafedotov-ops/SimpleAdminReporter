  ...
  ad: [
    {
      ...
      name: "Users",
      icon: Users,
      description: "User account reports",
    },
    {
      id: "groups",
      name: "Groups",
      icon: FolderOpen,
      description: "Group management reports",
    },
    {
      id: "security",
      name: "Security",
    ...
    },
    {
      ...
      name: "Computers",
      icon: Database,
      description: "Computer and server reports",
    },
  ],
  azure: [
    {
      ...
      name: "Users",
      icon: Users,
      description: "Azure AD user reports",
    },
    {
      id: "security",
      name: "Security",
      icon: Shield,
      description: "Security and risk reports",
    },
    {
      id: "apps",
      name: "Applications",
      icon: Grid3X3,
      description: "App registrations and usage",
    },
    {
      id: "computers",
      name: "Computers",
      icon: Database,
      description: "Device management reports",
    },
  ],
  o365: [
    {
    ...
    },
    {
      ...
      name: "Applications",
      icon: Grid3X3,
      description: "Email and mailbox reports",
    },
    {
      id: "usage",
      name: "Usage",
...
const TemplatesPageV2: React.FC = () => {
  ...
  const [selectedTemplate, setSelectedTemplate] =
    useState<QueryDefinition | null>(null);
    ...
    try {
      ...
      const [templatesResponse, customResponse] = await Promise.all([
        reportsService.getReportTemplates(
          forceRefresh ? { _t: Date.now() } : undefined,
        ),
      ...
      if (templatesResponse.success && templatesResponse.data) {
        const builtInTemplates = (templatesResponse.data.definitions || []).map(
          (def: QueryDefinition) => ({
            ...def,
            isCustom: false,
      ...
      if (customResponse.success && customResponse.data) {
        ...
        const customReports = customResponse.data || [];
        const customTemplates = customReports.map(
          (custom: any) =>
            ({
              id: custom.id,
              name: custom.name,
  ...
  const filteredTemplates = useMemo(() => {
    ...
    if (selectedSubcategory !== "all") {
      filtered = filtered.filter(
        (def) => def.subcategory === selectedSubcategory,
      );
    }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avgTime:
        templates.reduce((sum, t) => sum + (t.avgExecutionTime || 0), 0) /
        (templates.length || 1),
      categories: {
    ...
    try {
      ...
      if (template?.isCustom) {
        ...
        const result = await reportsService.executeCustomReport(
          queryId,
          parameters as Record<string, string | number | boolean | string[]>,
        ...
        setRecentTemplates((prev) =>
          [queryId, ...prev.filter((id) => id !== queryId)].slice(0, 5),
        );
        ...
        return {
          ...
          result: {
            ...
            metadata: {
              ...
              executionTime:
                resultData?.executionTimeMs || resultData?.executionTime || 0,
              cached: false,
              dataSource:
                resultData?.source || template?.dataSource || "unknown",
            },
          },
          cached: false,
        };
      ...
      try {
        ...
        if (response.success && (response as any).data) {
          const favoriteIds = new Set<string>(
            (response as any).data.map((fav: any) => fav.id),
          );
  ...
  const formatCellValue = (value: unknown, columnKey: string): string => {
    ...
    if (
      (columnKey === "lastLogonTimestamp" ||
        columnKey === "pwdLastSet" ||
        columnKey === "accountExpires" ||
        columnKey === "badPasswordTime" ||
        columnKey === "lockoutTime" ||
        columnKey === "lastLogon") &&
    ...
    if (
      (columnKey === "whenCreated" || columnKey === "whenChanged") &&
      typeof value === "string"
    ) {
  ...
  const renderResultsTable = () => {
      ...
      if (format === "excel" && selectedResult?.id) {
        ...
        try {
          ...
          if (selectedResult.history_id) {
            await reportsService.exportHistoryResults(
              selectedResult.history_id,
              "excel",
            );
          } else {
            // For fresh executions, we need to use the export endpoint
            const templateId =
              selectedResult.template_id || selectedResult.custom_template_id;
            if (templateId) {
              const response = await fetch(
                `${apiService.getBaseURL()}/reports/export/${selectedResult.custom_template_id ? "custom" : "report"}/${templateId}`,
                {
                  method: "POST",
                  credentials: "include",
        ...
        const csv = [
          ...
          ...data.map((row: any) =>
            columns
              .map((col) => `"${formatCellValue(row[col], col)}"`)
              .join(","),
          ),
        ].join("\n");
    ...
    return (
      <EnhancedDataTable
        data={resultData.results as Record<string, unknown>[]}
        ...
        quickFilters={customQuickFilters as any}
              ...
              >
                {[
                  ...
                  <button
                    key={tab.id}
                    onClick={() =>
                      setSelectedTab(tab.id as "system" | "custom" | "results")
                    }
                    style={{
                      padding: "10px 20px",
                      borderRadius: "8px",
                      background:
                        selectedTab === tab.id ? "#4a5568" : "transparent",
                      color:
                        selectedTab === tab.id
                          ? "white"
                          : darkMode
          ...
          <div style={{ display: "flex", gap: "24px" }}>
                ...
                >
                  ...
                  {[
                    { id: "all", label: "All Sources", count: stats.total },
                    {
                      ...
                      icon: UserCheck,
                      count: stats.categories.ad,
                    },
                    {
                      id: "azure",
                      label: "Azure AD",
                      icon: Cloud,
                      count: stats.categories.azure,
                    },
                    {
                      id: "o365",
                      label: "Office 365",
                      icon: Mail,
                      count: stats.categories.o365,
                    },
                  ].map((cat) => {
                    const Icon = cat.icon;
                    return (
                    ...
                    <div>
                      ...
                      >
                        <div>
                          ...
                          >
                            {selectedResult.report_name ||
                              selectedResult.template_name}{" "}
                            - Executed{" "}
                            {dayjs(selectedResult.executed_at).format(
                              "MMM D, YYYY h:mm A",
                            )}
                          </p>
                        </div>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {(selectedResult.execution_time_ms / 1000).toFixed(
                              1,
                            )}
                            s
                          </div>
                        ...
                        >
                          <div style={{ display: "flex", gap: "16px" }}>
                            ...
                            >
                              ...
                              >
                                {template.avgExecutionTime
                                  ? `${(template.avgExecutionTime / 1000).toFixed(1)}s`
                                  : "N/A"}
                              </span>
                            </div>
                          </div>
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: "flex", gap: "16px" }}>
                            ...
                            >
                              {template.avgExecutionTime
                                ? `${(template.avgExecutionTime / 1000).toFixed(1)}s avg`
                                : "No data"}
                            </span>
                            {template.lastExecuted && (
                              ...
                              >
                                Last run{" "}
                                {dayjs(template.lastExecuted).fromNow()}
                              </span>
                            )}
                          </div>
                        </div>
            ...
            {showDetails && selectedTemplate && (
              ...
              >
                ...
                >
                  {(() => {
                    const Icon = getIconForDataSource(
                  ...
                  <div>
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          {selectedTemplate.avgExecutionTime
                    ...
                    <div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            Last executed{" "}
                            {dayjs(selectedTemplate.lastExecuted).fromNow()}
                          </div>
                          <div
                            style={{
                              fontSize: "12px",
                          ...
                          >
                            {dayjs(selectedTemplate.lastExecuted).format(
                              "MMM D, YYYY h:mm A",
                            )}
                          </div>
                        </div>
