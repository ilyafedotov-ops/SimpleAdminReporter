...
const DraggableColumnHeader: React.FC<DraggableColumnHeaderProps> = ({
  ...
  >({
    ...
    hover(item: DragItem, monitor) {
      ...
      const hoverClientX =
        (clientOffset as { x: number }).x - hoverBoundingRect.left;
...
export interface EnhancedColumn<T = any> extends Omit<
  ColumnsType<T>[0],
  "filters"
> {
  dataIndex: string;
...
>({
  ...
  React.useEffect(() => {
    ...
    setColumnOrder((prev) => {
      ...
      const filtered = prev.filter((dataIndex) =>
        newDataIndexes.includes(dataIndex),
      );
      const missing = newDataIndexes.filter(
        (dataIndex) => !filtered.includes(dataIndex),
      );
  ...
  const filteredAndSortedData = useMemo(() => {
        ...
        filtered = filtered.filter((row) => {
          ...
          switch (filter.type) {
            ...
            case "number": {
              ...
              if (filterNum.min !== undefined && numValue < filterNum.min)
                return false;
              if (filterNum.max !== undefined && numValue > filterNum.max)
                return false;
              return true;
            }
            ...
            case "date": {
              const dateValue = dayjs(
                value as string | number | Date | dayjs.Dayjs,
              );
            ...
            case "dateRange": {
              ...
              return (
                date.isAfter(dayjs(rangeValue[0])) &&
                date.isBefore(dayjs(rangeValue[1]))
              );
            }
    ...
    if (sortConfig.field && sortConfig.order) {
      const column = enhancedColumns.find(
        (col) => col.dataIndex === sortConfig.field,
      );
      filtered.sort((a, b) => {
  ...
  const tableColumns = useMemo(() => {
    return enhancedColumns
      ...
      .map((col, index) => {
        ...
        const isDateColumn =
          col.filterType === "date" || col.filterType === "dateRange";
          ...
          sortMenuItems.push(
            {
              ...
              label: "Sort A → Z",
              icon:
                sortConfig.field === col.dataIndex &&
                sortConfig.order === "ascend" &&
                sortConfig.method === "default" ? (
                  <CheckOutlined />
                ) : null,
              onClick: () =>
                setSortConfig({
            ...
            },
            {
              ...
              icon:
                sortConfig.field === col.dataIndex &&
                sortConfig.method === "caseSensitive" ? (
                  <CheckOutlined />
                ) : null,
              onClick: () =>
                setSortConfig({
            ...
            return (
              <div style={{ padding: 8, minWidth: 200 }}>
                ...
                {col.filterType === "number" && (
                  <Space direction="vertical" style={{ width: "100%" }}>
                    <InputNumber
                      ...
                      onChange={(min) => {
                        const current =
                          (selectedKeys[0] as NumberFilterValue) || {};
                ...
                {col.filterType === "dateRange" && (
                  <RangePicker
                    defaultValue={
                      selectedKeys[0] as unknown as
                        [dayjs.Dayjs, dayjs.Dayjs] | undefined
                    }
                    onChange={(dates) =>
                      handleConfirm(dates as DateRangeFilterValue)
                    }
                ...
                {col.filterType === "date" && (
                  <DatePicker
                    defaultValue={
                      selectedKeys[0] as unknown as dayjs.Dayjs | undefined
                    }
                ...
                {col.filterType === "boolean" && (
                  <Select
                    defaultValue={
                      selectedKeys[0] as unknown as boolean | undefined
                    }
                    onChange={(value) => handleConfirm(value)}
                    style={{ marginBottom: 8, width: "100%" }}
                    allowClear
        ...
        if (!column.render && formatCellValue) {
          column.render = (value: unknown, record: any) =>
            formatCellValue(value, col.dataIndex, record);
        }
          ...
          enhancedColumns
            ...
            .map((col) => {
              ...
              return formatCellValue
                ? formatCellValue(value, col.dataIndex, row)
                : String(value || "");
            }),
        );
        const csv = [
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div
        className={`enhanced-data-table ${darkMode ? "dark" : "light"}`}
        ...
        <div className="table-toolbar">
          <div className="toolbar-left">
            ...
            {(activeFilterCount > 0 || searchText) && (
              <Button icon={<ClearOutlined />} onClick={clearAllFilters} danger>
