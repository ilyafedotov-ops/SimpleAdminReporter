...
describe("MainLayout", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(uiSlice.initializeUI).mockReturnValue({
      type: "ui/initializeUI",
    } as unknown);
    vi.mocked(uiSlice.toggleDarkMode).mockReturnValue({
      type: "ui/toggleDarkMode",
    } as unknown);
    vi.mocked(uiSlice.toggleSidebar).mockReturnValue({
      type: "ui/toggleSidebar",
    } as unknown);

    // Mock auth slice actions
    vi.mocked(authSlice.logoutAsync).mockReturnValue({
    ...
    vi.mocked(useAppSelector).mockImplementation((selector: unknown) => {
      ...
      if (
        selectorString.includes("auth") ||
        selectorString.includes("state => state.auth")
      ) {
    ...
    it("should render main layout with all components", () => {
      ...
      expect(
        screen.getByPlaceholderText(
          "Search reports, templates, or schedules...",
        ),
      ).toBeInTheDocument();
  ...
  describe("navigation", () => {
    ...
    it("should open the profile menu when clicking user avatar", async () => {
      renderWithProviders(<MainLayout />);
    ...
    it("should show active state for current page", () => {
      ...
      for (const button of dashboardButtons) {
        ...
        if (
          buttonElement
            ?.getAttribute("style")
            ?.includes("background: rgb(74, 85, 104)")
        ) {
      ...
      expect(activeButton?.getAttribute("style")).toContain(
  ...
  describe("search functionality", () => {
    ...
    it("should handle search errors gracefully", async () => {
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      vi.mocked(searchService.globalSearch).mockRejectedValue(
