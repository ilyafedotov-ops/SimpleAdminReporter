...
const formatMetricValue = (value: unknown): number => {
...
export const QueryPerformanceVisualization: React.FC<
  QueryPerformanceVisualizationProps
> = ({ metrics, className = "" }) => {
  ...
  const performanceTrends = useMemo(() => {
    ...
    return performanceData.map((item, index) => {
      ...
      const trend =
        item.executionTime > previous.executionTime
          ? "up"
          : item.executionTime < previous.executionTime
            ? "down"
            : "stable";
  ...
  const insights = useMemo(() => {
    ...
    const avgExecutionTime =
      performanceData.reduce((sum, item) => sum + item.executionTime, 0) /
      performanceData.length;
    const cacheHitRate =
      (performanceData.filter((item) => item.cacheHit).length /
        performanceData.length) *
      100;
    const slowQueries = performanceData.filter(
      (item) => item.executionTime > avgExecutionTime * 2,
    ).length;
                ...
                <Pie
                  ...
                  label={({ payload }) => {
                    const data = payload as
                      { name?: string; percentage?: string } | undefined;
                    return data
                      ? `${data.name || ""} (${data.percentage || "0"}%)`
                      : "";
                  }}
                >
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={performanceTrends}>
              ...
              <RechartsTooltip
                ...
                formatter={(value) => [
                  `${formatMetricValue(value).toFixed(2)}`,
                  "Efficiency",
                ]}
              />
              <Area
      ...
      >
        <Space direction="vertical" style={{ width: "100%" }}>
          {insights.map((insight, index) => {
            ...
            const recs =
              recommendations[insight.title as keyof typeof recommendations] ||
              [];
