...
interface CredentialFormProps {
  ...
  onCancel: () => void;
  onSubmit: (
    values: CreateCredentialDto | UpdateCredentialDto,
  ) => Promise<void>;
  loading?: boolean;
}
...
const CredentialForm: React.FC<CredentialFormProps> = ({
  ...
  const [serviceType, setServiceType] = React.useState<"ad" | "azure" | "o365">(
    "ad",
  );
  ...
  const handleOAuthLogin = async () => {
    try {
      ...
      const credentialName =
        form.getFieldValue("credentialName") || "Azure AD OAuth";
      ...
      if (axiosError.response?.status === 401) {
        ...
        message.error(
          "OAuth endpoint not found. Please check the API configuration.",
        );
      } else if (axiosError.response?.data?.message) {
        message.error(
          `OAuth error: ${(axiosError.response?.data as { message?: string })?.message}`,
        );
        ...
        {serviceType === "ad" && (
          <>
            ...
            <Form.Item
              ...
              extra={
                credential ? "Leave blank to keep current password" : undefined
              }
            >
              <Input.Password
                placeholder={
                  credential ? "Enter new password to change" : "Enter password"
                }
                prefix={<KeyOutlined />}
              />
            </Form.Item>
        ...
        {(serviceType === "azure" || serviceType === "o365") && (
          <>
            <Alert
              ...
              description={
                <Space direction="vertical" style={{ width: "100%" }}>
                  ...
                  <div>
                    1. Use OAuth authentication (recommended) - Sign in with
                    your Microsoft account
                  </div>
                  <div>
                    2. Use app-only authentication - Enter Client ID and Client
                    Secret
                  </div>
                  <Divider style={{ margin: "8px 0" }} />
                  <div>
                    <strong>Note:</strong> Your Azure AD account can be
                    different from your portal login account. The Azure AD
                    credentials will be securely stored and associated with your
                    portal user.
                  </div>
                </Space>
              }
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: !credential && !hasOAuthToken,
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: !credential && !hasOAuthToken,
                  message: "Client Secret is required if not using OAuth",
                },
              ]}
              extra={
                credential
                  ? "Leave blank to keep current secret"
                  : "Required only if not using OAuth authentication"
              }
            >
              <Input.Password
                placeholder={
