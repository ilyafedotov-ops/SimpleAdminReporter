...
interface ReportErrorDetails extends Record<string, unknown> {
  type?: string;
  code?: string;
  message?: string;
...
export const ReportViewer: React.FC<ReportViewerProps> = ({
  ...
  const tableColumns = useMemo((): EnhancedColumn[] => {
    ...
    if (fields.length > 0) {
      ...
      const mappedColumns: EnhancedColumn[] = fieldsWithData.map((field) => ({
  ...
  const tableData = useMemo(() => {
    const dataToShow =
      mode === "preview" ? data.results.slice(0, 100) : data.results;
            ...
            children: (
              <Row gutter={[16, 16]}>
                ...
                {debugInfo.attributes && (
                  <Col span={24}>
                    <Text strong>
                      Selected Attributes ({debugInfo.attributes.length}):
                    </Text>
                    <div style={{ marginTop: 8 }}>
                      {debugInfo.attributes.map((attr) => (
                        <Tag key={attr} style={{ marginBottom: 4 }}>
                          {attr}
                        </Tag>
                      ))}
                    </div>
                ...
                {debugInfo.filterDetails &&
                  debugInfo.filterDetails.length > 0 && (
                    <Col span={24}>
                      <Text strong>
                        Active Filters ({debugInfo.filterCount || 0}):
                      </Text>
                      <div style={{ marginTop: 8 }}>
                      ...
                      message={
                        <Space>
                          ...
                          {debugInfo.errorDetails.type && (
                            <Tag color="red">
                              {String(debugInfo.errorDetails.type)}
                            </Tag>
                          )}
                        </Space>
                      }
                      description={
                        <div>
                          <div style={{ marginBottom: 8 }}>
                            ...
                            {String(debugInfo.errorDetails.message)}
                          </div>

                          {debugInfo.errorDetails.code && (
                            <div style={{ marginBottom: 8 }}>
                              <Text strong>Error Code:</Text>{" "}
                              {String(debugInfo.errorDetails.code)}
                            </div>
                          )}
  ...
  if (error) {
    ...
    const canRetry =
      errorDetails?.canRetry && retryCount < maxRetries && onRetry;
    ...
    return (
      <div className={className} style={style}>
        <Alert
          ...
          description={
            <div>
              <div style={{ marginBottom: showRecovery ? 12 : 0 }}>{error}</div>
              ...
              {enableRecovery && errorDetails && (
                <div style={{ marginBottom: 12 }}>
                  ...
                  >
                    {getRecoverySuggestions(errorDetails)
                      .slice(0, 3)
                      .map((suggestion, idx) => (
                        <li key={idx} style={{ marginBottom: 2 }}>
                          {suggestion}
      ...
      <Card
        title={
          <Space>
            <EyeOutlined />
            <span>
              {mode === "preview" ? "Query Results Preview" : "Report Results"}
            </span>
            <Text type="secondary">
              ({data.resultCount || data.results.length} records)
            </Text>
        ...
        {data.results.length > 0 ? (
          <>
            <EnhancedDataTable
              ...
              onExport={
                mode === "full" && onDownload
                  ? (_exportData, format) =>
                      onDownload(format as "csv" | "excel" | "pdf")
                  : undefined
              }
              rowKey={(record) =>
                (record as any).key ||
