...
export const ReportExecutionModal: React.FC<ReportExecutionModalProps> = ({
  ...
  useEffect(() => {
    const fetchCredentials = async () => {
      console.log(
        "Fetching credentials for dataSource:",
      ...
      if (
        ...
        console.log(
          "No credentials needed for postgres or undefined dataSource",
        );
      ...
      try {
        const response = await credentialsAPI.getCredentials(
          queryDefinition.dataSource as "ad" | "azure" | "o365",
        );
        ...
        if (
          ...
          const activeCredentials = (response as any).data.filter(
          ...
          const defaultCred = activeCredentials.find(
            (cred: ServiceCredential) => cred.isDefault,
          );
          if (defaultCred) {
  ...
  const _handleExecute = async () => {
    ...
    try {
      ...
      if (!previewResult.success || !previewResult.data) {
        throw new Error(
          previewResult.error?.message || "Preview execution failed",
        );
      }
      ...
      const execResult: QueryExecutionResult = {
        ...
        result: {
          ...
          metadata: {
            ...
            executionTime:
              previewResult.data.executionTime || Date.now() - startTime,
            cached: false,
  ...
  const _handleParameterChange = (key: string, value: unknown) => {
    const param = queryDefinition.parameters?.find(
  ...
  const handleExport = async (format: "csv" | "excel" | "pdf" = "csv") => {
    if (
      ...
      (executionResult.result?.data as any[])?.length === 0
    ) {
    ...
    try {
      if (format === "csv") {
        ...
        downloadCSV(
          csvContent,
          `${queryDefinition.name.replace(/[^a-zA-Z0-9]/g, "_")}_${new Date().toISOString().split("T")[0]}.csv`,
        );
        ...
        onClose();
        message.info(
          `Navigate to the results view to export as ${format.toUpperCase()}`,
        );
      }
    } catch {
        ...
        >
            ...
            <>
              {isGraphQuery ? (
                ...
                <>
                  ...
                  {(() => {
                    const reportParams = Object.entries(
                      ...
                      <div style={{ marginBottom: "24px" }}>
                        {queryDefinition.dataSource !== "postgres" ? (
                          <>
                            ...
                            >
                              Select which credentials to use for this report.
                              If not selected, the default credentials will be
                              used.
                            </p>
                            <select
                              value={credentialId || ""}
                            ...
                            {credentials.length === 0 &&
                              !loadingCredentials && (
                                ...
                                >
                                  Please configure credentials in Settings to
                                  run this report.
                                </p>
                              )}
                          </>
                          ...
                          <>
                            ...
                            >
                              This report uses the default database connection.
          ...
          {executionResult && (
            <>
              {executionResult.result?.data &&
                (executionResult.result?.data as any[])?.length > 0 &&
                Object.keys((executionResult.result?.data as any[])[0]).length >
                  6 && (
                  <div
                    style={{
                      padding: "8px 16px",
              ...
              >
                <ReportViewer
                  ...
                  data={{
                    ...
                    resultCount:
                      executionResult.result?.metadata?.rowCount ||
                      executionResult.result?.data?.length ||
                      0,
                ...
                <button
                  onClick={async () => {
                    ...
                    if (executionResult?.isPreview && onExecute) {
                      try {
                        const fullResult = await onExecute(
                          queryDefinition.id,
                          parameters,
                          {
                            skipCache,
                        ...
                        const executed = fullResult as { executionId?: string };
                        if (executed.executionId) {
                          navigate(`/reports/history/${executed.executionId}`);
                          onClose();
                        } else {
                      ...
                      message.error(
                        "No execution ID available. Please try running the report again.",
                      );
                    }
                  }}
                  style={{
            ...
            <>
              ...
              <button
                onClick={() => {
                  if (
                    isGraphQuery ||
                    Object.keys(queryDefinition.parameters || {}).length > 0
                  ) {
