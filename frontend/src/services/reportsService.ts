...
export class ReportsService {
  ...
  async getReportTemplates(
    options?: { category?: string; source?: string; _t?: number } | undefined,
  ): Promise<
    ApiResponse<{ definitions: QueryDefinition[]; totalCount: number }>
  > {
    let params: any = {};
    ...
    if (options?.category === "azure" || options?.source === "azure") {
      ...
      if (
        ...
        const graphDefinitions = (
          (graphResponse.data as any).queries || []
        ).map((q: any) => ({
          ...q,
          dataSource: "azure",
          isSystem: true,
        ...
        return {
          ...
          data: {
            definitions: [
              ...((templatesResponse.data as any).definitions || []),
              ...graphDefinitions,
            ],
            totalCount:
              ((templatesResponse.data as any).totalCount || 0) +
              graphDefinitions.length,
          },
        };
      }
    }
  ...
  async previewTemplate<T = Record<string, unknown>>(
    ...
    const response = await apiService.post(
      `/reports/templates/${templateId}/preview`,
      {
        parameters: parameters || {},
        limit: limit || 10,
      },
  ...
  async previewCustomReport<T = Record<string, unknown>>(
    ...
    const response = await apiService.post(
      `/reports/custom/${reportId}/preview`,
      {
        parameters: parameters || {},
        limit: limit || 10,
      },
  ...
  // Field Discovery
  async getAvailableFields(source: "ad" | "azure" | "o365"): Promise<
    ApiResponse<{
      source: string;
      categories?: any[];
  ...
  async searchFields(
    ...
    if (response.success && response.data) {
      let fields: FieldMetadata[] = response.data.fields || [];
      ...
      if (category) {
        fields = fields.filter(
          (field: FieldMetadata) => field.category === category,
        );
      }
  ...
  async getFieldCategories(
    ...
    if (response.success && response.data) {
      ...
      if (categories.length === 0 && response.data.fields) {
        const uniqueCategories = new Set<string>();
        response.data.fields.forEach((field: FieldMetadata) => {
          if (field.category) {
  ...
  async getReportResults(id: string): Promise<
    ApiResponse<{
      historyId: string;
      results: any[];
      resultCount: number;
  ...
  async downloadReportResult(
    ...
    await apiService.downloadFile(
      `/reports/export/history/${id}?format=${format}`,
    );
  }

  async exportHistoryResults(
    historyId: string,
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "/api"}/reports/export/history/${historyId}?${params.toString()}`,
        {
          method: "GET",
          credentials: isCookieAuthEnabled() ? "include" : "same-origin",
      ...
      const filename = filenameMatch
        ? filenameMatch[1]
        : `report_${historyId}.${format === "excel" ? "xlsx" : "csv"}`;
  ...
  async bulkDeleteReports(
    ...
    try {
      const response = await (apiService as any).client.delete(
        "/reports/history/bulk",
        {
          data: { ids },
        },
      );
      ...
      throw new Error(
        error.response?.data?.message || "Failed to bulk delete reports",
      );
    }
  }
  ...
  async removeFromFavorites(
    ...
    return apiService.delete(
      `/reports/favorites/${reportId}?isCustom=${isCustom}`,
    );
  }
  ...
  async shareReport(
    ...
    return apiService.post(`/reports/custom/${reportId}/share`, {
      users,
      permissions,
    });
  }
  ...
  async clearQueryCache(queryId?: string): Promise<
    ApiResponse<{
      cleared: boolean;
