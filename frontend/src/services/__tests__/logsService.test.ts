...
describe("LogsService", () => {
  ...
  describe("getLogStats", () => {
    it("should fetch log statistics with default hours", async () => {
      const mockStats: LogStats = {
        ...
        errorTrends: [{ hour: "2025-01-15T10:00:00Z", error_count: 3 }],
        period: "24h",
      };
    ...
    it("should handle API errors for stats", async () => {
      ...
      await expect(logsService.getLogStats()).rejects.toThrow(
        "Stats API Error",
      );
    });
  });
  ...
  describe("getLogDetails", () => {
    ...
    it("should handle log details API errors", async () => {
      ...
      await expect(logsService.getLogDetails("999", "audit")).rejects.toThrow(
        "Log not found",
      );
    });
  });
  ...
  describe("exportLogs", () => {
    ...
    it("should export logs in CSV format with default parameters", async () => {
      ...
      const mockResponse = {
        ...
        headers: {
          get: vi
            .fn()
            .mockReturnValue('attachment; filename="logs_export.csv"'),
        },
    ...
    it("should export logs with filters and JSON format", async () => {
      ...
      const mockResponse = {
        ...
        headers: {
          get: vi
            .fn()
            .mockReturnValue('attachment; filename="audit_logs.json"'),
        },
        blob: vi.fn().mockResolvedValue(mockBlob),
      };
      mockFetch.mockResolvedValue(mockResponse);
    ...
    it("should handle export API errors", async () => {
      ...
      await expect(logsService.exportLogs()).rejects.toThrow(
  ...
  describe("utility methods", () => {
    ...
    describe("getLogIcon", () => {
      it("should return correct icons for audit log event types", () => {
        ...
        expect(
          logsService.getLogIcon({ ...auditLog, event_type: "auth" }),
        ).toBe("Key");
        expect(
          logsService.getLogIcon({ ...auditLog, event_type: "access" }),
        ).toBe("Shield");
        expect(
          logsService.getLogIcon({ ...auditLog, event_type: "admin" }),
        ).toBe("Settings");
        expect(
          logsService.getLogIcon({ ...auditLog, event_type: "security" }),
        ).toBe("Lock");
        expect(
          logsService.getLogIcon({ ...auditLog, event_type: "data" }),
        ).toBe("Database");
        expect(
          logsService.getLogIcon({ ...auditLog, event_type: "system" }),
        ).toBe("Server");
        expect(
          logsService.getLogIcon({ ...auditLog, event_type: "unknown" }),
        ).toBe("FileText");
      });
      ...
      it("should return correct icons for system log levels", () => {
        ...
        expect(logsService.getLogIcon({ ...systemLog, level: "error" })).toBe(
          "AlertCircle",
        );
        expect(logsService.getLogIcon({ ...systemLog, level: "warn" })).toBe(
          "AlertTriangle",
        );
        expect(logsService.getLogIcon({ ...systemLog, level: "info" })).toBe(
          "Info",
        );
        expect(logsService.getLogIcon({ ...systemLog, level: "debug" })).toBe(
          "Bug",
        );
        expect(logsService.getLogIcon({ ...systemLog, level: "verbose" })).toBe(
          "MessageSquare",
        );
        expect(logsService.getLogIcon({ ...systemLog, level: "unknown" })).toBe(
          "FileText",
        );
      });
    });
