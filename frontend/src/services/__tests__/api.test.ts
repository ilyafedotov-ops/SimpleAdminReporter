  ...
  class MockAdapter {
    ...
    constructor(axiosInstance: unknown) {
      ...
      axiosInstance.get = vi
        .fn()
        .mockImplementation(async (url: string, config: unknown = {}) => {
...
describe("ApiService", () => {
  ...
  describe("request interceptors", () => {
    it("should add authorization header when token exists", async () => {
      ...
      (apiService as unknown).client.get = vi
        .fn()
        .mockImplementation(async (url: string, config: unknown = {}) => {
  ...
  describe("getPaginated", () => {
    ...
    it("should use cache for paginated requests", async () => {
      ...
      expect(result).toEqual(cachedData);
      expect(apiCache.createCacheKey).toHaveBeenCalledWith(
        "paginated:/test",
        undefined,
      );
    });
  ...
  describe("file operations", () => {
    it("should download file successfully", async () => {
      ...
      const createElementSpy = vi
        ...
        .mockReturnValue(mockLink as unknown);
      const appendChildSpy = vi
        .spyOn(document.body, "appendChild")
        .mockImplementation(() => mockLink as unknown);
    ...
    it("should handle file download errors", async () => {
      ...
      await expect(apiService.downloadFile("/download/test")).rejects.toThrow(
        "Test error",
      );
    });
    ...
    it("should handle file upload errors", async () => {
      ...
      await expect(apiService.uploadFile("/upload", mockFile)).rejects.toThrow(
        "Test error",
      );
    });
  });
