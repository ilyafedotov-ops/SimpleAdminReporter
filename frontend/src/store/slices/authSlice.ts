...
// Initial state
// Seed auth synchronously from the auth service so protected deep links survive
// a hard refresh. Waiting for initializeAuth() in App.useEffect causes the
// first render to treat a valid stored cookie-auth session as anonymous,
// redirect to /login, and then bounce authenticated users back to /dashboard.
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          accessToken: (response as { data: { accessToken?: string } }).data
            .accessToken,
          refreshToken: (response as { data: { refreshToken?: string } }).data
            .refreshToken,
        };
      } else {
