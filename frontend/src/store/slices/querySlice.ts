...
export const fetchQueryDefinitionsAsync = createAsyncThunk(
  ...
  async (
    params:
      { dataSource?: string; category?: string; search?: string } | undefined,
    { rejectWithValue },
  ) => {
    try {
      ...
      return rejectWithValue(
        response?.error || "Failed to fetch query definitions",
      );
    } catch (error) {
    ...
    try {
      ...
      if (!options.skipCache) {
        ...
        const cached = cachedEntries.find(
          (entry) => entry.cacheKey === cacheKey && !isStale(entry),
        );
...
export const executeDynamicQueryAsync = createAsyncThunk(
  ...
  async (querySpec: DynamicQuerySpec, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        response.error || "Dynamic query execution failed",
      );
    } catch (error) {
      const appError = parseError(error);
      return rejectWithValue(appError.message);
  ...
  async (
    {
      queryDef,
      parameters,
    }: { queryDef: QueryDefinition; parameters?: Record<string, unknown> },
    { rejectWithValue },
  ) => {
    try {
...
export const fetchGraphDefinitionsAsync = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      return rejectWithValue(
        response.error || "Failed to fetch Graph definitions",
      );
    } catch (error) {
      const appError = parseError(error);
      return rejectWithValue(appError.message);
...
export const executeGraphQueryAsync = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await queryService.executeGraphQuery(
        queryId,
        parameters,
        options,
      );
...
const querySlice = createSlice({
  ...
  reducers: {
    ...
    setBuilderQuery: (
      state,
      action: PayloadAction<DynamicQuerySpec | null>,
    ) => {
    ...
    evictCacheEntry: (
      state,
      action: PayloadAction<{ queryId: string; cacheKey: string }>,
    ) => {
      const { queryId, cacheKey } = action.payload;
      if (state.resultsCache.byQueryId[queryId]) {
        state.resultsCache.byQueryId[queryId] = state.resultsCache.byQueryId[
          queryId
        ].filter((entry) => entry.cacheKey !== cacheKey);
        state.resultsCache.cacheKeys = state.resultsCache.cacheKeys.filter(
          (k) => k !== cacheKey,
        );
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(executeQueryAsync.fulfilled, (state, action) => {
        const { queryId, parameters, result, fromCache, timestamp } =
          action.payload;
        ...
        const executionKey = Object.keys(state.executions.byId).find(
          (key) =>
            state.executions.byId[key].queryId === queryId &&
            state.executions.byId[key].status === "pending" &&
            JSON.stringify(state.executions.byId[key].parameters) ===
        ...
        if (executionKey) {
          ...
          state.executions.activeIds = state.executions.activeIds.filter(
            (id) => id !== executionKey,
          );
        ...
        if (!fromCache) {
          ...
          if (
            state.resultsCache.currentSize > state.resultsCache.maxCacheSize
          ) {
            // Simple LRU: remove oldest entry
            const allEntries = Object.entries(state.resultsCache.byQueryId)
              .flatMap(([qId, entries]) =>
                entries.map((e) => ({ ...e, queryId: qId })),
              )
            ...
            if (allEntries.length > 0) {
              ...
              state.resultsCache.byQueryId[oldest.queryId] =
                state.resultsCache.byQueryId[oldest.queryId].filter(
                  (e) => e.cacheKey !== oldest.cacheKey,
                );
              state.resultsCache.cacheKeys =
                state.resultsCache.cacheKeys.filter(
                  (k) => k !== oldest.cacheKey,
                );
              state.resultsCache.currentSize--;
            }
    ...
    // Clear cache
    builder.addCase(clearQueryCacheAsync.fulfilled, (state, action) => {
      ...
      if (queryId) {
        ...
        // Remove cache keys
        const keysToRemove = Array.from(state.resultsCache.cacheKeys).filter(
          (key) => key.startsWith(`${queryId}:`),
        );
        state.resultsCache.cacheKeys = state.resultsCache.cacheKeys.filter(
    ...
    builder
      ...
      .addCase(fetchGraphDefinitionsAsync.rejected, (state, action) => {
        ...
        state.definitions.error =
          (action.payload as string) || "Failed to fetch Graph definitions";
      });
    ...
    builder
      .addCase(executeDynamicQueryAsync.pending, (state, action) => {
        ...
        const execution: QueryExecution = {
          ...
          parameters: action.meta.arg as unknown as Record<string, unknown>,
          status: "pending",
          startTime: Date.now(),
        };
    ...
    builder
        ...
        while (
          ...
          if (oldestKey) {
            ...
            Object.keys(state.resultsCache.byQueryId).forEach((qId) => {
              state.resultsCache.byQueryId[qId] = state.resultsCache.byQueryId[
                qId
              ].filter((entry) => entry.cacheKey !== oldestKey);
            });
            state.resultsCache.currentSize--;
          }
        }
      ...
      .addCase(executeGraphQueryAsync.rejected, (state, action) => {
        ...
        state.executions.loading[queryId] = false;
        state.executions.errors[queryId] =
          (action.payload as string) || "Graph query execution failed";
...
export const selectQueryDefinitions = createSelector(
  [selectQueryState],
  (queryState) =>
    queryState.definitions.allIds.map((id) => queryState.definitions.byId[id]),
);

export const selectQueryDefinitionById = (id: string) =>
  createSelector(
    [selectQueryState],
...
export const selectActiveExecutions = createSelector(
  [selectQueryState],
  (queryState) =>
    queryState.executions.activeIds.map((id) => queryState.executions.byId[id]),
);

export const selectExecutionHistory = createSelector(
  [selectQueryState],
  (queryState) =>
    queryState.executions.historyIds.map(
      (id) => queryState.executions.byId[id],
    ),
);

export const selectQueryLoadingState = (queryId: string) =>
  createSelector(
    [selectQueryState],
    (queryState) => queryState.executions.loading[queryId] || false,
  );

export const selectQueryError = (queryId: string) =>
  createSelector(
    [selectQueryState],
    (queryState) => queryState.executions.errors[queryId] || null,
  );

export const selectCachedResult = (
  queryId: string,
  parameters: Record<string, unknown>,
) =>
  createSelector([selectQueryState], (queryState) => {
    const cacheKey = generateCacheKey(queryId, parameters);
...
export const selectQueryStatistics = (queryId: string) =>
  createSelector(
    [selectQueryState],
  ...
  (definitions, queryState) => {
    ...
    if (queryState.ui.filterByDataSource) {
      filtered = filtered.filter(
        (def) => def.dataSource === queryState.ui.filterByDataSource,
      );
    }
