...
export function useErrorHandler() {
  ...
  const createRetryHandler = useCallback(
    <T>(asyncFn: () => Promise<T>, maxAttempts = 3) => {
      let attempts = 0;
    ...
    (
      ...
      if (showNotification) {
        if (isRetryableError(appError) && retryCallback && enableAutoRetry) {
          ...
          appError.type === "QUERY_VALIDATION"
        ) {
          ...
          message.error({
            content: React.createElement(
              ...
              React.createElement(
                "div",
                { style: { fontSize: "12px", opacity: 0.8 } },
                "Click to go back and fix the configuration",
              ),
            ),
            duration: 10,
   ...
   */
  const createPreviewRetryHandler = useCallback(
    <T>(
      asyncFn: () => Promise<T>,
      options?: {
        maxAttempts?: number;
      ...
      const retry = async (): Promise<T | null> => {
        ...
        try {
          ...
          if (isRetryableError(appError) && attempts < maxAttempts) {
            ...
            console.log(
              `Retrying ${context} operation in ${delay}ms (attempt ${attempts}/${maxAttempts})`,
            );
          ...
          // Max attempts reached or non-retryable error
          console.error(
            `${context} operation failed after ${attempts} attempts:`,
            appError,
          );
   ...
   */
  const handlePreviewOperation = useCallback(
    async <T>(
      operation: () => Promise<T>,
      options?: {
        context?: string;
      ...
      try {
        ...
        const enhancedError = handlePreviewError(error, {
          showNotification,
          retryCallback: onRetry ? async () => onRetry() : undefined,
          onGoBack,
          context,
          maxRetries,
          enableAutoRetry,
  ...
  const handleFormError = useCallback(
    (error: unknown, form?: any) => {
      ...
      if (appError.details && form && typeof appError.details === "object") {
        const fieldErrors = Object.entries(appError.details).map(
          ([field, error]) => ({
            name: field,
