    ...
    async (refresh = false) => {
      ...
      if (source === "azure") {
        try {
          ...
          for (const entityType of entityTypes) {
            try {
              ...
              const response = await fetch(
                `/api/graph/fields/${entityType}?${params}`,
                {
                  method: "GET",
              ...
              console.warn(
                `Failed to load fields for ${entityType}:`,
                entityError,
              );
            }
          }
          ...
          const categories: FieldCategory[] = Array.from(
            categoryMap.entries(),
          ).map(([key, fields]) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/reports/fields/${source}?${params}`,
          {
            method: "GET",
            headers: getAuthHeaders({
              "Content-Type": "application/json",
        ...
        if (result.success && (result as any)?.data) {
          ...
          const allFields = categoriesData.reduce<FieldMetadata[]>(
            (acc, category) => {
  ...
  const discoverSchema = useCallback(
    async (refresh = false) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/reports/schema/${source}/discover?${params}`,
          {
            method: "GET",
            headers: getAuthHeaders({
              "Content-Type": "application/json",
        ...
        if (!response.ok) {
          if (response.status === 401) {
            throw new Error(
              "Authentication required. Please ensure you have valid AD credentials.",
            );
          }
        ...
        if (result.success && (result as any)?.data) {
          ...
          if (schemaData?.allFields) {
            ...
            setTotalFields(
              schemaData?.totalAttributes || schemaData?.allFields?.length || 0,
            );
          }

          // If we have common attributes, we could highlight them
          console.log(
            `Discovered ${schemaData?.totalAttributes || 0} attributes from ${source}`,
          );
