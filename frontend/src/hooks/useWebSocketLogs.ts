...
import { getSocketAuthOptions } from "@/utils/authToken";
...
export const useWebSocketLogs = (
  ...
  const [connectionStatus, setConnectionStatus] =
    useState<UseWebSocketLogsReturn["connectionStatus"]>("disconnected");
  ...
  const setFilters = useCallback(
    (newFilters: UseWebSocketLogsOptions["filters"]) => {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const socketAuth = getSocketAuthOptions();
      const socket = io(`${socketUrl}/socket/logs`, {
        ...socketAuth,
