...
test.describe("Authentication - Login Flows", () => {
  ...
  test.describe("LDAP Authentication", () => {
    ...
    test("should show error message for invalid AD credentials", async ({
      page,
    }) => {
    ...
    test("should handle different username formats for AD", async ({
      page,
    }) => {
      const user = TEST_USERS.AD_USER;
    ...
    test("should remember user login when remember me is checked", async ({
      page,
    }) => {
      const user = TEST_USERS.AD_USER;
      await ApiHelper.mockAuthEndpoints(page, user);
  ...
  test.describe("Azure AD Authentication", () => {
    test("should initiate Azure AD OAuth flow", async ({ page }) => {
      ...
      expect(
        await page
          .locator('button[type="submit"]:has-text("Sign in with Microsoft")')
          .isVisible(),
      ).toBe(true);
    });

    test("should leave raw OAuth callback codes to the MSAL callback handler", async ({
      page,
    }) => {
      // The React /login route must not treat an arbitrary authorization code in
      // the query string as an authenticated session. The real Azure callback is
      // handled by MSAL/the dedicated callback page and then synced with backend.
    ...
    test("should handle Azure AD OAuth errors", async ({ page }) => {
      ...
      await page.goto(
        "/login?error=access_denied&error_description=User%20cancelled",
      );
    ...
    test("should show appropriate UI for Azure AD authentication", async ({
      page,
    }) => {
      await loginPage.selectAuthSource("azure");

      // Username/password fields should be hidden for Azure
      const hasUsernameField = await loginPage.usernameInput.isVisible();
      const hasPasswordField = await loginPage.passwordInput.isVisible();

      expect(hasUsernameField).toBe(false);
      expect(hasPasswordField).toBe(false);

      // Should show Microsoft-specific branding
      const azureButton = await page.locator(
  ...
  test.describe("Form Validation and UX", () => {
    test("should show loading state during authentication", async ({
      ...
      // Verify loading state is shown through the submit button. Ant Design may
      // render the spinner as an SVG instead of a stable .ant-spin element.
    ...
    test("should disable login button while request is in progress", async ({
      page,
    }) => {
      const user = TEST_USERS.AD_USER;
      await page.unroute("**/api/auth/login");
    ...
    test("should clear form when switching authentication sources", async ({
      page,
    }) => {
      // Fill AD credentials
      await loginPage.selectAuthSource("ad");
    ...
    test("should show proper placeholders for different auth sources", async ({
      page,
    }) => {
      ...
      await page.route("**/api/auth/login", (route) => {
        ...
        route.fulfill({
          ...
          body: JSON.stringify({
            ...
            data: {
              user: {
                id: TEST_USERS.AD_USER.username,
                username: TEST_USERS.AD_USER.username,
                displayName: TEST_USERS.AD_USER.displayName,
                email: TEST_USERS.AD_USER.email,
                authSource: TEST_USERS.AD_USER.authSource,
                roles: TEST_USERS.AD_USER.roles,
                permissions: TEST_USERS.AD_USER.permissions,
                isActive: true,
              },
              accessToken: "token",
              refreshToken: "mock-refresh-token",
  ...
  test.describe("Accessibility", () => {
    test("should be keyboard navigable", async ({ page }) => {
      ...
      // Should submit the form via keyboard and complete login.
    ...
    test("should have proper ARIA labels and roles", async ({ page }) => {
      ...
      const usernameLabel = await page
        .locator('label[for*="username"], [aria-label*="username"]')
        .count();
      const passwordLabel = await page
        .locator('label[for*="password"], [aria-label*="password"]')
        .count();
      ...
      const errorElements = await page
        .locator('[role="alert"], .ant-form-item-explain-error')
        .count();
