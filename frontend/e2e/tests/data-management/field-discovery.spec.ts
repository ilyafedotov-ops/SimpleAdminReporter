...
// Field discovery is not exposed in the current Settings UI. Keep this legacy
// coverage disabled until a dedicated field-discovery surface is restored.
test.describe.skip("Data Management - Field Discovery", () => {
  ...
  test.describe("Field Discovery Process", () => {
    test("should discover AD fields successfully", async ({ page }) => {
              ...
              fields: [
                {
                  name: "sAMAccountName",
                  type: "string",
                  description: "Account name (pre-Windows 2000)",
                },
                {
                  name: "displayName",
                  type: "string",
              ...
              fields: [
                {
                  ...
                  type: "integer",
                  description: "User account control flags",
                },
                {
                  name: "lastLogon",
                  type: "datetime",
                ...
                },
                {
                  name: "pwdLastSet",
                  type: "datetime",
                  description: "Password last set time",
                },
                {
                  name: "accountExpires",
                  type: "datetime",
            ...
            },
            {
              ...
              fields: [
                ...
                { name: "title", type: "string", description: "Job title" },
                {
                  name: "manager",
                  type: "dn",
                  description: "Manager distinguished name",
                },
                {
                  name: "memberOf",
                  type: "array",
      ...
      expect(categories.some((cat) => cat.name === "Basic Information")).toBe(
        true,
      );
      expect(categories.some((cat) => cat.name === "Security")).toBe(true);
      expect(categories.some((cat) => cat.name === "Organizational")).toBe(
        true,
      );

      // Check field counts
      const basicInfoCategory = categories.find(
        (cat) => cat.name === "Basic Information",
      );
    ...
    test("should discover Azure AD fields successfully", async ({ page }) => {
              ...
              fields: [
                {
                  name: "signInActivity",
                  type: "object",
                  description: "Sign-in activity information",
                },
                {
            ...
            },
            {
              ...
              fields: [
                ...
                },
                {
                  name: "usageLocation",
                  type: "string",
                  description: "Usage location for licensing",
                },
              ],
            },
          ],
      ...
      const categories = await settingsPage.getFieldCategories();
      expect(categories.length).toBe(3);
      expect(categories.some((cat) => cat.name === "User Properties")).toBe(
        true,
      );
      expect(categories.some((cat) => cat.name === "Authentication")).toBe(
        true,
      );
    });
          ...
          categories: [
            {
              ...
              fields: [
                ...
                },
                {
                  name: "totalItemSize",
                  type: "integer",
                  description: "Total mailbox size in bytes",
                },
                {
                  name: "itemCount",
                  type: "integer",
                ...
                },
                {
                  name: "lastLogonTime",
                  type: "datetime",
                  description: "Last mailbox logon",
                },
              ],
            },
            {
              ...
              fields: [
                {
                  name: "storageUsed",
                  type: "integer",
                  description: "OneDrive storage used in bytes",
                },
                {
                  name: "storageQuota",
                  type: "integer",
                ...
                },
                {
                  ...
                  type: "datetime",
                  description: "Last OneDrive activity",
                },
              ],
            },
            {
              ...
              fields: [
                {
                  name: "teamsActivities",
                  type: "array",
                  description: "Teams usage activities",
                },
                {
                  name: "meetingsOrganized",
                  type: "integer",
                  description: "Number of meetings organized",
                },
              ],
            },
          ],
    ...
    test("should show discovery progress during field discovery", async ({
      page,
    }) => {
      ...
      const discoverButton = page.locator(
        'button:has-text("Discover"), .discover-fields-btn',
      );
      await discoverButton.click();

      // Should show loading/progress indicator
      const loadingIndicator = page.locator(
        '.ant-spin, .discovery-progress, [data-testid="loading"]',
      );
      expect(await loadingIndicator.isVisible()).toBe(true);

      // Should show progress message
      const progressMessage = page.locator(
        ".progress-message, .discovery-status",
      );
  ...
  test.describe("Field Categories and Details", () => {
    test("should display field details within categories", async ({ page }) => {
      ...
      const fields =
        await settingsPage.getFieldsInCategory("Basic Information");
      expect(fields.length).toBe(3);

      const samAccountField = fields.find(
        (field) => field.name === "sAMAccountName",
      );
    ...
    test("should support field search and filtering", async ({ page }) => {
      ...
      const searchInput = page.locator(
        '.field-search, input[placeholder*="search"]',
      );
    ...
    test("should show field usage statistics", async ({ page }) => {
      ...
      if (fieldCount > 0) {
        ...
        const usageIndicator = firstField.locator(
          ".usage-count, .popularity-badge",
        );
  ...
  test.describe("Field Metadata Management", () => {
    test("should refresh field metadata cache", async ({ page }) => {
      ...
      const refreshResults = page.locator(
        ".refresh-results, .ant-message-success",
      );
    ...
    test("should handle cache corruption gracefully", async ({ page }) => {
      ...
      const rebuildButton = page.locator(
        'button:has-text("Rebuild"), .rebuild-cache-btn',
      );
    ...
    test("should validate field metadata consistency", async ({ page }) => {
      ...
      const validateButton = page.locator(
        'button:has-text("Validate"), .validate-fields-btn',
      );
      if (await validateButton.isVisible()) {
        ...
        const validationResults = page.locator(
          ".validation-results, .field-validation",
        );
  ...
  test.describe("Field Discovery Performance", () => {
    ...
    test("should support progressive field loading", async ({ page }) => {
      ...
      const fieldContainer = page.locator(
        ".field-categories, .fields-container",
      );
  ...
  test.describe("Field Discovery Integration", () => {
    test("should integrate with report builder field selection", async ({
      page,
    }) => {
      await settingsPage.discoverFields("ad");
              ...
              fields: [
                {
                  name: "newField",
                  type: "string",
                  description: "Newly discovered field",
                },
              ],
            },
          ],
    ...
    test("should support custom field annotations", async ({ page }) => {
      ...
      if (await fieldItem.isVisible()) {
        ...
        const editButton = fieldItem.locator(
          '.edit-field, button:has-text("Edit")',
        );

        if (await editButton.isVisible()) {
          await editButton.click();

          const annotationModal = page.locator(
            ".field-annotation-modal, .ant-modal",
          );
          if (await annotationModal.isVisible()) {
            // Should allow custom descriptions or tags
            const descriptionInput = annotationModal.locator(
    ...
    test("should track field discovery history", async ({ page }) => {
      const historyButton = page.locator(
        'button:has-text("History"), .discovery-history-btn',
      );
      ...
      if (await historyButton.isVisible()) {
        ...
        const historyModal = page.locator(
          ".discovery-history-modal, .ant-modal",
        );
  ...
  test.describe("Error Handling and Edge Cases", () => {
    test("should handle credential permission errors", async ({ page }) => {
      ...
      const permissionError = page.locator(
        ".permission-error, .ant-result-403",
      );
    ...
    test("should handle network timeouts during discovery", async ({
      page,
    }) => {
      // Mock timeout
      await page.route("**/api/fields/discover*", (route) => {
    ...
    test("should handle partial discovery failures", async ({ page }) => {
              ...
              fields: [
                {
                  name: "workingField",
                  type: "string",
                  description: "This field was discovered",
                },
              ],
            },
          ],
      ...
      const warningMessage = page.locator(
        ".ant-alert-warning, .discovery-warnings",
      );
    ...
    test("should handle schema version changes", async ({ page }) => {
      ...
      await ApiHelper.mockApiResponse(
        ...
        "**/api/fields/discover*",
        {
          ...
          details:
            "Server schema version 2.3 does not match cached version 2.1",
      ...
      const versionError = page.locator(
        ".schema-version-error, .ant-alert-warning",
      );
      if (await versionError.isVisible()) {
        ...
        const clearCacheButton = page.locator(
          'button:has-text("Clear Cache"), .clear-cache-btn',
        );
  ...
  test.describe("Multi-tenancy and Data Source Support", () => {
    test("should discover fields for multiple AD domains", async ({ page }) => {
              ...
              categories: [
                {
                  ...
                  fields: [
                    ...
                    },
                    {
                      ...
                      type: "string",
                      description: "Custom domain 2 attribute",
                    },
                  ],
                },
              ],
      ...
      if (await domainTabs.isVisible()) {
        const domains = await domainTabs
          .locator(".domain-tab, .domain-option")
          .count();
