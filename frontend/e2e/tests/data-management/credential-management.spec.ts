...
test.describe("Data Management - Credential Management", () => {
  ...
  test.describe("Credential Creation and Management", () => {
    test("should add new AD credential successfully", async ({ page }) => {
      ...
      expect(
        credentials.some((cred) => cred.name === credentialData.name),
      ).toBe(true);
    ...
    test("should add new Azure AD credential successfully", async ({
      page,
    }) => {
      await ApiHelper.mockApiResponse(page, "**/api/credentials", {
        success: true,
    ...
    test("should add new O365 credential successfully", async ({ page }) => {
      ...
      const credentials = await settingsPage.getCredentialsList();
      expect(
        credentials.some((cred) => cred.name === o365Credential.name),
      ).toBe(true);
    });

    test("should validate required fields when adding credential", async ({
      page,
    }) => {
      ...
      expect(
        errors.some((error) => error.toLowerCase().includes("required")),
      ).toBe(true);
    ...
    test("should handle credential creation errors gracefully", async ({
      page,
    }) => {
      // Mock API error
  ...
  test.describe("Credential Testing and Validation", () => {
    test("should test AD credential connection successfully", async ({
      page,
    }) => {
      ...
      const testResult =
        await settingsPage.testCredential("Test AD Connection");
      expect(testResult).toBe("success");
    ...
    test("should handle failed credential connection test", async ({
      page,
    }) => {
      await ApiHelper.mockApiResponse(page, "**/api/credentials*", [
        {
      ...
      const testResult = await settingsPage.testCredential(
        "Failing AD Connection",
      );
    ...
    test("should test Azure AD credential with OAuth flow", async ({
      page,
    }) => {
      await ApiHelper.mockApiResponse(page, "**/api/credentials*", [
        {
    ...
    test("should show credential test history", async ({ page }) => {
      ...
      if (await credentialItem.isVisible()) {
        ...
        const historySection = page.locator(
          ".test-history, .connection-history",
        );
        if (await historySection.isVisible()) {
          const historyItems = await historySection
  ...
  test.describe("Credential Editing and Updates", () => {
    test("should edit existing credential", async ({ page }) => {
      ...
      // Click edit button
      const credentialItem = page.locator(
        '.credential-item:has-text("Editable Credential")',
      );
      const editButton = credentialItem.locator(
      ...
      if (await editButton.isVisible()) {
        ...
        await modal
          .locator('input[name="server"]')
          .fill("new-server.domain.local");
        await modal
          .locator('input[name="username"]')
          .fill("new-user@domain.local");
    ...
    test("should handle credential update conflicts", async ({ page }) => {
      ...
      if (await editButton.isVisible()) {
        ...
        await modal
          .locator('input[name="username"]')
          .fill("updated-user@domain.local");
        await modal.locator('button:has-text("Save")').click();

        // Should show conflict error
        const errorMessage = page.locator(
          ".ant-message-error, .conflict-error",
        );
        const hasError = await errorMessage.isVisible();
  ...
  test.describe("Credential Deletion and Cleanup", () => {
    test("should delete credential successfully", async ({ page }) => {
      ...
      expect(
        credentialsAfter.some((cred) => cred.name === "Credential To Delete"),
      ).toBe(false);
    });
    ...
    test("should prevent deletion of credentials in use", async ({ page }) => {
      ...
      const credentialItem = page.locator(
        '.credential-item:has-text("Active Credential")',
      );
      ...
      if (await deleteButton.isVisible()) {
        ...
        const credentials = await settingsPage.getCredentialsList();
        expect(
          credentials.some((cred) => cred.name === "Active Credential"),
        ).toBe(true);
      }
    });
    ...
    test("should show confirmation dialog before deletion", async ({
      ...
      const credentialItem = page.locator(
        '.credential-item:has-text("Confirm Delete Credential")',
      );
      const deleteButton = credentialItem.locator('button:has-text("Delete")');
      ...
      if (await deleteButton.isVisible()) {
        ...
        const credentials = await settingsPage.getCredentialsList();
        expect(
          credentials.some((cred) => cred.name === "Confirm Delete Credential"),
        ).toBe(true);
      }
    });
  });
  ...
  test.describe("Credential Security and Encryption", () => {
    test("should mask sensitive credential information", async ({ page }) => {
      ...
      const credentialItem = page.locator(
        '.credential-item:has-text("Secure Credential")',
      );

      // Password should be masked in display
      const passwordField = credentialItem.locator(
        ".password-display, .credential-password",
      );
    ...
    test("should encrypt credentials before storage", async ({ page }) => {
      ...
      const credentialRequestPromise = page.waitForRequest(
        (request) =>
          request.url().includes("/api/credentials") &&
          request.method() === "POST",
      );
      await settingsPage.addCredential(credentialData);
      requestBody = await (await credentialRequestPromise).postDataJSON();

      // Verify that sensitive data is sent using the current credential DTO shape.
      expect(requestBody).toBeTruthy();
      expect(requestBody.credentialName ?? requestBody.name).toBe(
        credentialData.name,
      );
    ...
    test("should validate credential permissions", async ({ page }) => {
      ...
      const permissionError = page.locator(
        ".ant-result-403, .permission-error",
      );
  ...
  test.describe("Credential Import and Export", () => {
    test("should support credential import from file", async ({ page }) => {
      const importButton = page.locator(
        'button:has-text("Import"), .import-credentials-btn',
      );
      ...
      if (await importButton.isVisible()) {
        ...
        if (await fileInput.isVisible()) {
          ...
          const importResults = page.locator(
            ".import-results, .import-summary",
          );
    ...
    test("should support credential export", async ({ page }) => {
      ...
      const exportButton = page.locator(
        'button:has-text("Export"), .export-credentials-btn',
      );
  ...
  test.describe("Credential Usage Monitoring", () => {
    test("should show credential usage statistics", async ({ page }) => {
      ...
      const credentialItem = page.locator(
        '.credential-item:has-text("Monitored Credential")',
      );

      // Should show usage information
      const usageInfo = credentialItem.locator(
        ".usage-info, .credential-usage",
      );
    ...
    test("should link to reports using credential", async ({ page }) => {
        ...
        reportsUsing: [
          {
            name: "Inactive Users",
            id: "inactive_users",
            lastExecuted: "2025-01-07T12:00:00Z",
          },
          {
            name: "Password Expiry",
            id: "password_expiry",
            lastExecuted: "2025-01-07T10:30:00Z",
          },
        ],
        scheduledReports: [
      ...
      const credentialItem = page.locator(".credential-item").first();
      const usageLink = credentialItem.locator(
        '.usage-link, button:has-text("View Usage")',
      );
  ...
  test.describe("Performance and Reliability", () => {
    test("should handle large numbers of credentials efficiently", async ({
      page,
    }) => {
      ...
      await page.route("**/api/credentials**", async (route) => {
        return route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            data: largeCredentialList.map((credential) => ({
              ...credential,
              credentialName: credential.name,
              serviceType: credential.type,
              username: `user${credential.id}@domain.local`,
              isActive: true,
            })),
          }),
        });
      ...
      const rowCount = await page
        .locator(".credential-list .ant-table-tbody tr.ant-table-row")
        .count();
    ...
    test("should support credential pagination", async ({ page }) => {
      const paginationControls = page.locator(
        ".ant-pagination, .credentials-pagination",
      );
    ...
    test("should handle credential operations during high load", async ({
      page,
    }) => {
