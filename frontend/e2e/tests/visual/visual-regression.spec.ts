...
// Screenshot baselines are not committed for this project, so this suite is
// intentionally disabled in CI-style full E2E runs. Re-enable only together
// with curated snapshots or an explicit --update-snapshots workflow.
test.describe.skip("Visual Regression Tests", () => {
  ...
  test.describe("Dashboard Visual Tests", () => {
    ...
    test("should match navigation menu states", async ({ page }) => {
      ...
      if (await navMenu.isVisible()) {
        ...
        const collapseButton = page.locator(
          ".ant-layout-sider-trigger, .nav-collapse",
        );
        if (await collapseButton.isVisible()) {
          await collapseButton.click();
          await page.waitForTimeout(500);
          await expect(navMenu).toHaveScreenshot(
            "navigation-menu-collapsed.png",
          );
        }
      }
    });
    ...
    test("should match notification panel", async ({ page }) => {
      ...
      const notificationBell = page.locator(
        '[data-testid="notifications"], .notification-bell',
      );
      if (await notificationBell.isVisible()) {
        await notificationBell.click();
        await page.waitForSelector(".notifications-panel, .ant-dropdown-menu", {
          state: "visible",
        });
        ...
        await expect(notificationsPanel).toHaveScreenshot(
  ...
  test.describe("Reports Page Visual Tests", () => {
    ...
    test("should match data source tabs", async ({ page }) => {
      ...
      if (await tabsContainer.isVisible()) {
        await expect(tabsContainer).toHaveScreenshot(
          "reports-data-source-tabs.png",
        );
        ...
        await page.waitForTimeout(500);
        await expect(tabsContainer).toHaveScreenshot(
          "reports-tabs-ad-selected.png",
        );

        await reportsPage.selectDataSource("azure");
        await page.waitForTimeout(500);
        await expect(tabsContainer).toHaveScreenshot(
          "reports-tabs-azure-selected.png",
        );

        await reportsPage.selectDataSource("o365");
        await page.waitForTimeout(500);
        await expect(tabsContainer).toHaveScreenshot(
          "reports-tabs-o365-selected.png",
        );
      }
    });
    ...
    test("should match report execution modal", async ({ page }) => {
      ...
      if (await parameterForm.isVisible()) {
        await expect(parameterForm).toHaveScreenshot(
          "report-parameters-form.png",
        );
      }
      ...
      if (await resultsTable.isVisible()) {
        await expect(resultsTable).toHaveScreenshot(
          "report-execution-results.png",
        );
      }
    });
    ...
    test("should match report cards layout", async ({ page }) => {
      ...
      const firstReportCard = page
        .locator(".report-item, .ant-list-item, .report-card")
        .first();
      if (await firstReportCard.isVisible()) {
        await expect(firstReportCard).toHaveScreenshot(
          "single-report-card.png",
        );
      }
    });
  });
  ...
  test.describe("Report Builder Visual Tests", () => {
    ...
    test("should match field explorer panel", async ({ page }) => {
      ...
      const fieldsPanel = page.locator(
        '.fields-panel, [data-testid="fields-panel"]',
      );
    ...
    test("should match query builder components", async ({ page }) => {
      ...
      const queryBuilder = page.locator(
        '.query-builder, [data-testid="query-builder"]',
      );
      if (await queryBuilder.isVisible()) {
        await expect(queryBuilder).toHaveScreenshot(
          "query-builder-with-fields.png",
        );
      }

      // Screenshot selected fields area
      const selectedFields = page.locator(
        '.selected-fields, [data-testid="selected-fields"]',
      );
      if (await selectedFields.isVisible()) {
        await expect(selectedFields).toHaveScreenshot(
          "selected-fields-area.png",
        );
      }

      // Screenshot filters area
      const filtersArea = page.locator(
        '.filters-area, [data-testid="filters-area"]',
      );
    ...
    test("should match query preview panel", async ({ page }) => {
      ...
      await ApiHelper.mockApiResponse(page, "**/api/reports/preview*", {
        success: true,
        query:
          "SELECT sAMAccountName, displayName FROM users WHERE enabled = true",
        estimatedRows: 1500,
      });
      ...
      const previewPanel = page.locator(
        '.preview-panel, [data-testid="preview-panel"]',
      );
  ...
  test.describe("Settings Page Visual Tests", () => {
    ...
    test("should match credentials management section", async ({ page }) => {
      ...
      const credentialsSection = page.locator(
        ".credentials-section, .settings-content",
      );
      await expect(credentialsSection).toHaveScreenshot(
        "credentials-management-section.png",
      );
    });
    ...
    test("should match field discovery section", async ({ page }) => {
      ...
      const fieldDiscoverySection = page.locator(
        ".field-discovery-section, .settings-content",
      );
      await expect(fieldDiscoverySection).toHaveScreenshot(
        "field-discovery-section.png",
      );
    });
  });
  ...
  test.describe("Responsive Visual Tests", () => {
    ...
    test("should match mobile navigation menu", async ({ page }) => {
      ...
      const mobileMenu = page.locator(
        ".mobile-menu, .hamburger-menu, .ant-drawer-trigger",
      );
      if (await mobileMenu.isVisible()) {
        await mobileMenu.click();
        await page.waitForSelector(".mobile-nav, .ant-drawer", {
          state: "visible",
        });
  ...
  test.describe("Cross-browser Visual Consistency", () => {
    test("should match layout across different zoom levels", async ({
      page,
    }) => {
      await AuthHelper.login(page, "AD_USER");
    ...
    test("should match components with different font sizes", async ({
      page,
    }) => {
      await AuthHelper.login(page, "AD_USER");
