...
// This legacy suite targets the old pre-built report catalogue UI with
// data-source tabs and execution cards. The current Reports page is a report
// results/history surface, so disable these stale interactions until the
// catalogue UI is restored or the suite is rewritten for the current flow.
test.describe.skip("Report Generation - Pre-built Reports", () => {
  ...
  test.describe("Report Selection and Navigation", () => {
    test("should display available reports for each data source", async ({
      page,
    }) => {
      // Mock reports list
    ...
    test("should search and filter reports correctly", async ({ page }) => {
      ...
      expect(
        searchResults.some((report) => report.name.includes("Password")),
      ).toBe(true);
  ...
  test.describe("Report Execution", () => {
    ...
    test("should handle report execution with different parameters", async ({
      page,
    }) => {
      await ApiHelper.mockApiResponse(page, "**/api/reports/execute/*", {
        success: true,
        data: {
          results: [
            {
              username: "expired1",
              displayName: "Expired User 1",
              passwordExpiry: "2025-02-01",
            },
            {
              username: "expired2",
              displayName: "Expired User 2",
    ...
    test("should handle report execution errors gracefully", async ({
      page,
    }) => {
    ...
    test("should validate report parameters before execution", async ({
      page,
    }) => {
      await reportsPage.selectDataSource("ad");
      await reportsPage.selectReport("Inactive Users");
  ...
  test.describe("Report Results Management", () => {
    ...
    test("should export reports in different formats", async ({ page }) => {
      await ApiHelper.mockApiResponse(page, "**/api/reports/execute/*", {
        success: true,
        data: {
          results: [{ username: "user1", displayName: "User One" }],
          totalRecords: 1,
          executionTime: "0.2s",
        },
      });
    ...
    test("should save report as custom template", async ({ page }) => {
      ...
      await reportsPage.saveAsTemplate(
        "My Custom Inactive Users Report",
        "Custom report with 60-day threshold",
      );
  ...
  test.describe("Report History and Audit", () => {
    test("should track report execution history", async ({ page }) => {
      ...
      const historyButton = page.locator(
        'button:has-text("History"), .report-history',
      );
    ...
    test("should allow re-execution from history", async ({ page }) => {
      ...
      if (await historyButton.isVisible()) {
        ...
        const rerunButton = page
          .locator('button:has-text("Run Again"), .rerun-button')
          .first();
  ...
  test.describe("Cross-browser and Responsive", () => {
    test("should work correctly on different screen sizes", async ({
      page,
    }) => {
  ...
  test.describe("Performance and Reliability", () => {
    ...
    test("should handle network interruption during execution", async ({
      page,
    }) => {
      let requestCount = 0;
