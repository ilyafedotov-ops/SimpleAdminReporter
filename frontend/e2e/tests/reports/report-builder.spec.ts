...
// This legacy suite targets the pre-refactor visual query-builder DOM
// (fields panel/query builder/drop zones). The current /reports/builder page
// exposes a simplified launcher-first builder surface, so keep these detailed
// interaction checks disabled until the full visual builder UI is restored.
test.describe.skip("Report Generation - Custom Report Builder", () => {
  ...
  test.describe("Builder Interface and Navigation", () => {
    ...
    test("should select data source and load fields", async ({ page }) => {
      ...
      expect(fields.some((field) => field.name === "sAMAccountName")).toBe(
        true,
      );
    });
    ...
    test("should search fields effectively", async ({ page }) => {
        ...
        categories: [
          {
            ...
            fields: [
              ...
              },
              {
                ...
                type: "integer",
                description: "Account Control Flags",
              },
            ],
          },
        ],
      ...
      searchResults.forEach((field) => {
        expect(
          field.name.toLowerCase() + field.description.toLowerCase(),
        ).toContain("account");
      });
    });
  });
  ...
  test.describe("Query Building - Filters", () => {
    ...
    test("should add multiple filters with different operators", async ({
      page,
    }) => {
      await reportBuilderPage.selectDataSource("ad");
      ...
      expect(
        filters.some(
          (f) => f.field === "lastLogon" && f.operator === "older_than",
        ),
      ).toBe(true);
      expect(
        filters.some(
          (f) => f.field === "department" && f.operator === "equals",
        ),
      ).toBe(true);
    });
    ...
    test("should handle complex filter combinations", async ({ page }) => {
      ...
      await reportBuilderPage.addFilter(
        "sAMAccountName",
        "not_contains",
        "admin",
      );
  ...
  test.describe("Query Building - Advanced Features", () => {
    ...
    test("should build complex queries with all components", async ({
      page,
    }) => {
      ...
      if (complexQuery.orderBy) {
        await reportBuilderPage.addOrderBy(
          complexQuery.orderBy.field,
          complexQuery.orderBy.direction as "asc" | "desc",
        );
      }
  ...
  test.describe("Query Execution and Results", () => {
    test("should preview query before execution", async ({ page }) => {
      ...
      await ApiHelper.mockApiResponse(page, "**/api/reports/preview*", {
        success: true,
        query:
          "SELECT sAMAccountName, displayName FROM users WHERE enabled = ?",
        estimatedRows: 1500,
    ...
    test("should handle query validation errors", async ({ page }) => {
      ...
      const errorElements = await page
        .locator(".ant-alert-error, .validation-error")
        .count();
  ...
  test.describe("User Experience and Interaction", () => {
    ...
    test("should validate field types for operations", async ({ page }) => {
      ...
      const validationWarning = page.locator(
        ".ant-form-item-explain-error, .validation-warning",
      );
      const hasWarning = await validationWarning.isVisible();

      if (hasWarning) {
        expect(await validationWarning.textContent()).toMatch(
          /(type|invalid|operator)/i,
        );
      }
    });

    test("should handle drag and drop interactions smoothly", async ({
      page,
    }) => {
      await ApiHelper.mockApiResponse(page, "**/api/reports/fields*", {
        categories: [
    ...
    test("should provide helpful tooltips and hints", async ({ page }) => {
      ...
      const field = page
        .locator('[data-field-name="sAMAccountName"], .field-item')
        .first();
  ...
  test.describe("Responsive and Mobile Support", () => {
    ...
    test("should handle touch gestures for query building", async ({
      page,
    }) => {
    ...
    test("should optimize layout for different screen sizes", async ({
      page,
    }) => {
      const viewports = [
  ...
  test.describe("Error Handling and Edge Cases", () => {
    ...
    test("should handle malformed API responses", async ({ page }) => {
      ...
      if (hasError) {
        expect(await errorAlert.textContent()).toMatch(
          /(error|invalid|parse)/i,
        );
      }
    });
    ...
    test("should prevent invalid query configurations", async ({ page }) => {
      ...
      const hasValidationError = await page
        .locator(".ant-form-item-explain-error, .validation-error")
        .isVisible();
