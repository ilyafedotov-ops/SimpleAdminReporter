...
export class AuthHelper {
  private static async selectAuthSource(
    ...
    const optionLabels = {
      ...
      local: "Local Account",
    } satisfies Record<typeof authSource, string>;

    await page.locator(".ant-select-selector").click();
    await page.waitForSelector(".ant-select-dropdown", { state: "visible" });
    await page
      .locator(`.ant-select-item-option[label="${optionLabels[authSource]}"]`)
      .click();
    await page.waitForSelector(".ant-select-dropdown", { state: "hidden" });
  }

  private static usernameInput(page: Page) {
    return page
      .locator('[placeholder*="username" i], [placeholder*="domain" i]')
      .first();
  }

  private static passwordInput(page: Page) {
  ...
  static async login(
    ...
    await this.selectAuthSource(page, user.authSource);

    // Fill credentials (skip for Azure OAuth)
    if (user.authSource !== "azure") {
      await this.usernameInput(page).fill(user.username);
      await this.passwordInput(page).fill(user.password);
    ...
    // Wait for successful login and the protected dashboard shell to finish
    // mounting. The app initializes auth state asynchronously, so URL alone can
    // be momentarily true before route guards settle.
    await page.waitForURL(
      (url) => /\/(dashboard|reports)(\/|$)/.test(url.pathname),
      { timeout: TEST_CONFIG.DEFAULT_TIMEOUT },
    );
    await page
      .getByRole("heading", {
        name: /Reporting Dashboard|Report Results|Recent Reports/i,
      })
      .first()
      .waitFor({ timeout: TEST_CONFIG.DEFAULT_TIMEOUT });
  }
  ...
  static async loginWithCredentials(
    ...
    const matchingUser = Object.values(TEST_USERS).find(
      (user) => user.username === username && user.authSource === authSource,
    ) as TestUser | undefined;

    await ApiHelper.mockAuthEndpoints(page, matchingUser ?? TEST_USERS.AD_USER);
    await page.goto("/login");
    await page.waitForLoadState("networkidle");

    await this.selectAuthSource(page, authSource);

    if (authSource !== "azure") {
      await this.usernameInput(page).fill(username);
      await this.passwordInput(page).fill(password);
    }
  ...
  static async logout(page: Page): Promise<void> {
    // Open user menu
    await page
      .locator('[data-testid="user-profile"], .ant-dropdown-trigger')
      .click();
...
export class ApiHelper {
  ...
  static async mockApiResponse(
    ...
    const isCredentialsRoute =
      typeof urlPattern === "string" && urlPattern.includes("/api/credentials");
    const normalizeCredential = (credential: any) => ({
      ...credential,
      credentialName: credential.credentialName ?? credential.name,
      serviceType: credential.serviceType ?? credential.type,
      isActive: credential.isActive ?? true,
    });
    await page.route(urlPattern, (route) => {
      let body = response;
      if (isCredentialsRoute && Array.isArray(response)) {
        body = { success: true, data: response.map(normalizeCredential) };
      } else if (
        isCredentialsRoute &&
        response &&
        ...
        body = { success: status < 400, data: normalizeCredential(response) };
      }
      route.fulfill({
        status,
      ...
      (response) => {
        ...
        const matchesUrl =
          typeof urlPattern === "string"
            ...
            : urlPattern.test(url);
        return (
          matchesUrl && response.request().method() === method.toUpperCase()
        );
      },
      { timeout },
    );
  ...
  static async mockAllCommonEndpoints(page: Page): Promise<void> {
    ...
    // Mock notification stats that dashboard layout polls after login.
    ...
    // Mock report stats cards loaded by the dashboard shell. Register this after
    // the broad reports route because Playwright evaluates newer routes first.
      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            reportsBySource: { ad: 15, azure: 7, o365: 3 },
            executionsByStatus: { success: 22, failed: 3 },
          },
        }),
      });
    });

    // Mock service credentials endpoints used by settings/data-management E2E.
    await page.route("**/api/credentials**", async (route) => {
      ...
      if (method === "POST" && /\/api\/credentials\/?(?:\?.*)?$/.test(url)) {
...
export class FormHelper {
   ...
   */
  static async getFormErrors(
    page: Page,
    formSelector: string = "form",
  ): Promise<string[]> {
    const form = page.locator(formSelector);
    const errorElements = form.locator(
      ".ant-form-item-explain-error, .error-message",
    );
  ...
  static async submitFormAndValidate(
    ...
    if (expectedResult === "success") {
      ...
      await Promise.race([
        ...
        page
          .locator(".ant-message-success, .success-message")
...
export class TableHelper {
   ...
   */
  static async sortTableByColumn(
    page: Page,
    columnName: string,
    tableSelector: string = "table",
  ): Promise<void> {
  ...
  static async filterTable(
    ...
    const searchInput = page
      .locator('.ant-input-search, input[placeholder*="search"]')
      .first();
  ...
  static async getPerformanceMetrics(page: Page): Promise<any> {
    return await page.evaluate(() => {
      const navigation = window.performance.getEntriesByType(
        "navigation",
      )[0] as window.PerformanceNavigationTiming;
      return {
        domContentLoaded:
        ...
        largestContentfulPaint:
          window.performance.getEntriesByName("largest-contentful-paint")[0]
            ?.startTime || 0,
      };
    });
  }
