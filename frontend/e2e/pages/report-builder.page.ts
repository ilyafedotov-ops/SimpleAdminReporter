...
export class ReportBuilderPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.dataSourceSelector = page
      .locator('[data-testid="data-source-select"], .data-source-select')
      .first();
    this.fieldsPanel = page
      .locator('[data-testid="fields-panel"], .fields-panel')
      .first();
    this.queryBuilder = page
      .locator('[data-testid="query-builder"], .query-builder')
      .first();
    this.previewPanel = page
      .locator('[data-testid="preview-panel"], .preview-panel')
      .first();

    // Field explorer
    this.fieldCategories = page
      .locator('[data-testid="field-categories"], .field-categories')
      .first();
    this.availableFields = page
      .locator('[data-testid="available-fields"], .available-fields')
      .first();
    this.selectedFields = page
      .locator('[data-testid="selected-fields"], .selected-fields')
      .first();
    this.fieldSearch = page
      .locator('input[placeholder*="Search fields"], .field-search')
      .first();

    // Query builder components
    this.selectFieldsArea = page
      .locator('[data-testid="select-fields"], .select-fields-area')
      .first();
    this.filtersArea = page
      .locator('[data-testid="filters-area"], .filters-area')
      .first();
    this.groupByArea = page
      .locator('[data-testid="group-by"], .group-by-area')
      .first();
    this.orderByArea = page
      .locator('[data-testid="order-by"], .order-by-area')
      .first();

    // Filter builder
    this.filterBuilder = page
      .locator('[data-testid="filter-builder"], .filter-builder')
      .first();
    this.addFilterButton = page
      .locator('button:has-text("Add Filter"), .add-filter-btn')
      .first();
    this.filterRows = page
      .locator('[data-testid="filter-row"], .filter-row')
      .first();

    // Actions
    this.previewButton = page
      .locator('button:has-text("Preview"), .preview-btn')
      .first();
    this.executeButton = page
      .locator('button:has-text("Execute"), button:has-text("Run Query")')
      .first();
    this.saveTemplateButton = page
      .locator('button:has-text("Save Template"), .save-template-btn')
      .first();
    this.clearButton = page
      .locator('button:has-text("Clear"), .clear-btn')
      .first();
    this.exportQueryButton = page
      .locator('button:has-text("Export Query"), .export-query-btn')
      .first();

    // Preview and results
    this.previewResults = page
      .locator('[data-testid="preview-results"], .preview-results')
      .first();
    this.queryPreview = page
      .locator('[data-testid="query-preview"], .query-preview')
      .first();
    this.executionTime = page
      .locator('[data-testid="execution-time"], .execution-time')
      .first();
  ...
  async isLoaded(): Promise<boolean> {
    try {
      await this.page
        .getByRole("heading", { name: /Custom Query Builder|Report Builder/i })
  ...
  async selectDataSource(source: "ad" | "azure" | "o365"): Promise<void> {
    ...
    const option = this.page.locator(
      `.ant-select-item:has-text("${source}"), [data-value="${source}"]`,
    );
    await option.click();
  ...
  async getFieldCategories(): Promise<string[]> {
    ...
    if (await this.fieldCategories.isVisible()) {
      const categoryItems = await this.fieldCategories
        .locator(".category-item, .ant-collapse-header")
        .all();
  ...
  async expandFieldCategory(categoryName: string): Promise<void> {
    const category = this.fieldCategories.locator(
      `.category-item:has-text("${categoryName}"), .ant-collapse-header:has-text("${categoryName}")`,
    );
  ...
  > {
    ...
    if (await this.availableFields.isVisible()) {
      const fieldItems = await this.availableFields
        .locator('.field-item, [data-testid="field-item"]')
        .all();
  ...
  async dragFieldToSelect(fieldName: string): Promise<void> {
    const sourceField = this.availableFields.locator(
      `[data-field-name="${fieldName}"], .field-item:has-text("${fieldName}")`,
    );
  ...
  async addFieldToQuery(fieldName: string): Promise<void> {
    const field = this.availableFields.locator(
      `[data-field-name="${fieldName}"], .field-item:has-text("${fieldName}")`,
    );
  ...
  async removeFieldFromQuery(fieldName: string): Promise<void> {
    const selectedField = this.selectedFields.locator(
      `[data-field-name="${fieldName}"], .selected-field:has-text("${fieldName}")`,
    );

    // Look for remove button (X icon)
    const removeButton = selectedField.locator(
      ".remove-field, .ant-tag-close-icon",
    );
  ...
  async addFilter(
    ...
    // Select field
    const fieldSelect = filterRow.locator(
      '.field-select, [data-testid="filter-field"]',
    );
    await fieldSelect.click();
    await this.page.locator(`.ant-select-item:has-text("${field}")`).click();

    // Select operator
    const operatorSelect = filterRow.locator(
      '.operator-select, [data-testid="filter-operator"]',
    );
    await operatorSelect.click();
    await this.page.locator(`.ant-select-item:has-text("${operator}")`).click();

    // Enter value
    const valueInput = filterRow.locator(
      '.value-input, [data-testid="filter-value"]',
    );
  ...
  async removeFilter(index: number): Promise<void> {
    ...
    if (filterRows[index]) {
      const removeButton = filterRows[index].locator(
  ...
  async addOrderBy(
    ...
    const sortDirectionButton = this.orderByArea.locator(
      `[data-field="${fieldName}"] .sort-direction`,
    );
    if (await sortDirectionButton.isVisible()) {
      // Click to toggle between asc/desc until we get the desired direction
      const currentDirection =
        await sortDirectionButton.getAttribute("data-direction");
  ...
  async saveAsTemplate(
    ...
    if (category) {
      ...
      await this.page
        .locator(`.ant-select-item:has-text("${category}")`)
        .click();
    }
  ...
  async clearQuery(): Promise<void> {
    ...
    const confirmDialog = this.page.locator(
      ".ant-modal-confirm, .ant-popconfirm",
    );
  ...
  async exportQuery(format: "sql" | "json" = "sql"): Promise<void> {
    ...
    if (await exportDropdown.isVisible()) {
      await exportDropdown
        .locator(`button:has-text("${format.toUpperCase()}")`)
        .click();
    }
  }
  ...
  async getSelectedFields(): Promise<string[]> {
    ...
    if (await this.selectedFields.isVisible()) {
      const fieldItems = await this.selectedFields
        .locator(".selected-field, .ant-tag")
        .all();
   ...
   * Get applied filters
   */
  async getAppliedFilters(): Promise<
  ...
  > {
    ...
    if (await this.filtersArea.isVisible()) {
      ...
      for (const row of filterRows) {
        const field = await row
          .locator(".field-select .ant-select-selection-item")
          .textContent();
        const operator = await row
          .locator(".operator-select .ant-select-selection-item")
          .textContent();
  ...
  async verifyPageElements(): Promise<{
    ...
    return {
      ...
      hasActionButtons:
        (await this.previewButton.isVisible()) &&
        (await this.executeButton.isVisible()),
    };
  }
}
